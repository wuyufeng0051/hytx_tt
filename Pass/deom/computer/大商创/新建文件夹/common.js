...
function addToCart(goodsId, parentId,event,obj,divId,store_id,end_time,store_mobile)
{
	...
	if($(".goods_info_attr").length > 0){
		if(attr_list == is_selected){
			...
			get_goods_prompt_message(Product_spec_prompt);
		}
	}else{
		Ajax.call('flow.php?step=add_to_cart', 'goods=' + $.toJSON(goods), addToCartResponse, 'POST', 'JSON');
	}
	...
	$(".attr_list .item li").click(function(){
		...
		if(type == "checkbox")
		{
			...
			if($(this).hasClass("selected"))
			{
				...
				if(length<1){
					get_user_prompt_message(Select_attr);
...
function get_collect_store(type, ru_id){
	...
	Ajax.call('merchants_store.php?act=ajax_collect_store', 'merchant_id=' + merchant_id + '&type=' + type + '&execute=' + execute, function(res){
		if(res.error == 1){
			message = Focus_prompt_one;
			foot = false;
		}else if(res.error == 2){
			message = Focus_prompt_login;
			ftx_div = '<div class="ftx-03">'+Focus_prompt_two+'</div>';
			foot = true;
		}
		...
		if(res.error > 0 && res.error < 3){
			...
			if(type == 0){
				message = Focus_prompt_three;
			}else if(type == 1){
				message = Focus_prompt_four;
			}else if(type == 2){
                                
				message = Focus_prompt_five;
			}
			var content = '<div id="' + divId + '">' + 
							'<div class="tip-box icon-box">' +
