 ...
 * 模板标记风格
 *
 * @version        $Id: mytagTemp.php 2014-5-13 下午16:48:10 $
...
//新增标记
	...
	if($submit == "提交"){
		...
		$act = $action == "article" ? "a" : "p";
		$archives = $dsql->SetQuery("INSERT INTO `#@__mytag_temp` (`title`, `module`, `type`, `litpic`, `css`, `isSystem`, `state`, `pubdate`) VALUES ('$title', '$action', '".$act."list', '$litpic', '$css', '$isSystem', '$state', '".GetMkTime(time())."')");
...
//删除标记
	...
	if($id != ""){
		foreach($each as $val){
			...
			unlinkFile(HUONIAOROOT."/templates/mytag/".$val.".html");
