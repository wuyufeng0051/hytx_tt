 ...
 * 建材品牌
 *
 * @version        $Id: buildBrand.php 2014-2-10 下午21:32:58 $
...
checkPurview("buildBrand");

if($dopost != ""){
	$templates = "buildBrandAdd.html";

	//js
	$jsFile = array(
		'ui/jquery.dragsort-0.5.1.min.js',
		'publicUpload.js',
		'admin/build/buildBrandAdd.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}else{
	$templates = "buildBrand.html";

	//js
	$jsFile = array(
		'ui/bootstrap.min.js',
		'ui/jquery-ui-selectable.js',
		'admin/build/buildBrand.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}
...
if($dopost == "getList"){
	...
	if(count($results) > 0){
		...
		if(count($list) > 0){
			echo '{"state": 100, "info": '.json_encode("获取成功").', "pageInfo": {"totalPage": '.$totalPage.', "totalCount": '.$totalCount.'}, "buildBrandList": '.json_encode($list).'}';
		}else{
...
//修改
	...
	if($id != ""){
		foreach($each as $val){
			...
			delPicFile($results[0]['logo'], "delBrand", "build");
...
if(file_exists($tpl."/".$templates)){
	...
	$huoniaoTag->assign('typeListArr', json_encode(getTypeList(0, "build_industry")));
