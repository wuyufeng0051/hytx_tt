 ...
 * 建材资讯
 *
 * @version        $Id: buildNews.php 2016-6-19 下午19:03:10 $
...
if($dopost != ""){
	$templates = "buildNewsAdd.html";

	//js
	$jsFile = array(
		'ui/jquery.dragsort-0.5.1.min.js',
		'publicUpload.js',
		'admin/build/buildNewsAdd.js'
	);
	...
	$templates = "buildNews.html";

	//js
	$jsFile = array(
		'ui/bootstrap.min.js',
		'ui/jquery-ui-selectable.js',
		'admin/build/buildNews.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}
...
if($dopost == "getList"){
	...
	if(count($results) > 0){
		...
		if(count($list) > 0){
			echo '{"state": 100, "info": '.json_encode("获取成功").', "pageInfo": {"totalPage": '.$totalPage.', "totalCount": '.$totalCount.'}, "buildNewsList": '.json_encode($list).'}';
		}else{
...
//新增
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`title`, `typeid`, `litpic`, `weight`, `body`, `arcrank`, `pubdate`) VALUES ('$title', $typeid, '$litpic', $weight, '$body', $arcrank, $pubdate)");
...
//修改
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `title` = '$title', `typeid` = $typeid, `litpic` = '$litpic', `weight` = '$weight', `body` = '$body', `arcrank` = '$arcrank' WHERE `id` = ".$id);
		$return = $dsql->dsqlOper($archives, "update");
...
if(file_exists($tpl."/".$templates)){
	...
	$huoniaoTag->assign('typeListArr', json_encode($dsql->getTypeList(0, "build_news_type")));
