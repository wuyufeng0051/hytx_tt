 ...
 * 添加建材商品
 *
 * @version        $Id: buildProductAdd.php 2014-3-1 下午23:59:19 $
...
$templates = "buildProductAdd.html";

$tab = "build_product";
$dopost = $dopost ? $dopost : "save";        //操作类型 save添加 edit修改
if($dopost == "edit"){
	$pagetitle = "修改建材商品";
	checkPurview("buildProductEdit");
}else{
	$pagetitle = "添加建材商品";
	checkPurview("buildProductAdd");
}
...
//获取品牌
	...
	if(!empty($id)){
		$typeSql = $dsql->SetQuery("SELECT `parentid` FROM `#@__build_industry` WHERE `id` = ".$id);
		$typeResult = $dsql->dsqlOper($typeSql, "results");
		if($typeResult){
			$brandSql = $dsql->SetQuery("SELECT `id`, `title` FROM `#@__build_brand` WHERE `type` = ".$typeResult[0]['parentid']." ORDER BY `weight`");
...
if(file_exists($tpl."/".$templates)){
	...
	$jsFile = array(
		...
		'ui/bootstrap-datetimepicker.min.js',
		'publicUpload.js',
		'admin/build/buildProductAdd.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));

	$huoniaoTag->assign('dopost', $dopost);
	$huoniaoTag->assign('pagetitle', $pagetitle);
	require_once(HUONIAOINC."/config/build.inc.php");
	global $customUpload;
	if($customUpload == 1){
	...
	$archives = $dsql->SetQuery("SELECT `id`, `company` FROM `#@__build_store` ORDER BY `weight`");
	$results = $dsql->dsqlOper($archives, "results");
	if($results){
	...
	$archives = $dsql->SetQuery("SELECT * FROM `#@__build_industry` WHERE `parentid` = 0 ORDER BY `weight`");
	$results = $dsql->dsqlOper($archives, "results");
	if($results){
		foreach($results as $key => $val){
			$archives_ = $dsql->SetQuery("SELECT * FROM `#@__build_industry` WHERE `parentid` = ".$val['id']." ORDER BY `weight`");
	...
	$brandOption = array();
	if(!empty($type)){
		$typeSql = $dsql->SetQuery("SELECT `parentid` FROM `#@__build_industry` WHERE `id` = ".$type);
		$typeResult = $dsql->dsqlOper($typeSql, "results");
		if($typeResult){
			$brandSql = $dsql->SetQuery("SELECT `id`, `title` FROM `#@__build_brand` WHERE `type` = ".$typeResult[0]['parentid']." ORDER BY `weight`");
			$brandResult = $dsql->dsqlOper($brandSql, "results");
