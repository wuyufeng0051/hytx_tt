 ...
 * 建材促销
 *
 * @version        $Id: buildSale.php 2014-03-01 下午16:55:08 $
...
if($dopost != ""){
	$templates = "buildSaleAdd.html";
	
	//js
	$jsFile = array(
		'admin/build/buildSaleAdd.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}else{
	$templates = "buildSale.html";
	
	//js
	$jsFile = array(
		'ui/bootstrap.min.js',
		'ui/jquery-ui-selectable.js',
		'admin/build/buildSale.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}
...
if($dopost == "getList"){
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			//公司
			$companySql = $dsql->SetQuery("SELECT `company` FROM `#@__build_store` WHERE `id` = ". $value["company"]);
		...
		if(count($list) > 0){
			echo '{"state": 100, "info": '.json_encode("获取成功").', "pageInfo": {"totalPage": '.$totalPage.', "totalCount": '.$totalCount.'}, "buildSale": '.json_encode($list).'}';
		}else{
...
if(file_exists($tpl."/".$templates)){
	...
	$huoniaoTag->assign('companyArr', json_encode(getCompanyList("build_store")));
