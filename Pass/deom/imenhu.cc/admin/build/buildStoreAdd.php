 ...
 * 添加建材公司
 *
 * @version        $Id: buildStoreAdd.php 2016-4-26 下午16:44:20 $
...
$templates = "buildStoreAdd.html";

$tab = "build_store";
$dopost = $dopost ? $dopost : "save";        //操作类型 save添加 edit修改
if($dopost == "edit"){
	$pagetitle = "修改建材店铺";
	checkPurview("buildStoreEdit");
}else{
	$pagetitle = "添加建材店铺";
	checkPurview("buildStoreAdd");
}
...
if($_POST['submit'] == "提交"){
	...
	if($domaintype != 0){
		...
		//验证域名是否被使用
		if(!operaDomain('check', $domain, 'build', $tab, $id, GetMkTime($domainexp)))
...
if($dopost == "save" && $submit == "提交"){
	...
	if($aid){
		//域名操作
		operaDomain('update', $domain, 'build', $tab, $aid, GetMkTime($domainexp), $domaintip);

		adminLog("添加建材店铺", $title);
	...
	if($submit == "提交"){
		...
		if($results == "ok"){
			//域名操作
			operaDomain('update', $domain, 'build', $tab, $id, GetMkTime($domainexp), $domaintip);

			adminLog("修改建材店铺", $title);
	...
	if(!empty($id)){
		...
		if(!empty($results)){
			...
			//获取域名信息
			$domainInfo   = getDomain('build', $tab, $id);
...
if(file_exists($tpl."/".$templates)){
	...
	//获取域名信息
	$domainInfo = getDomain('build', 'config');
	...
	$huoniaoTag->assign('typeidListArr', json_encode(getTypeList(0, "build_industry")));
