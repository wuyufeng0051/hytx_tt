 ...
 * 管理汽车经销商试驾管理
 *
 * @version        $Id: carDealerBooking.php 2014-9-16 下午14:21:11 $
 * @package        HuoNiao.Car
...
checkPurview("carDealerBooking");
$dsql  = new dsql($dbo);
$tpl   = dirname(__FILE__)."/../templates/car";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录

$tab = "car_dealer_booking";

if($dopost != ""){
	$templates = "carDealerBookingAdd.html";
	
	//js
	$jsFile = array(
		'admin/car/carDealerBookingAdd.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}else{
	$templates = "carDealerBooking.html";
	
	//js
	$jsFile = array(
		'ui/bootstrap.min.js',
		'ui/jquery-ui-selectable.js',
		'admin/car/carDealerBooking.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}
...
if($dopost == "getList"){
	...
	if(count($results) > 0){
		...
		if(count($list) > 0){
			echo '{"state": 100, "info": '.json_encode("获取成功").', "pageInfo": {"totalPage": '.$totalPage.', "totalCount": '.$totalCount.'}, "carDealerBooking": '.json_encode($list).'}';
		}else{
...
//新增
	...
	checkPurview("carDealerBookingAdd");
...
//修改
	...
	checkPurview("carDealerBookingEdit");
...
//删除
	...
	if(!testPurview("carDealerBookingDel")){
	...
	if($id != ""){
		...
		if(!empty($error)){
			...
			adminLog("删除汽车经销商试驾信息", join(", ", $title));
