 ...
 * 汽车法规
 *
 * @version        $Id: carFagui.php 2014-9-4 下午22:50:41 $
 * @package        HuoNiao.Car
...
if($dopost != ""){
	$templates = "carFaguiAdd.html";
	
	//js
	$jsFile = array(
		'ui/bootstrap.min.js',
		'admin/car/carFaguiAdd.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}else{
	$templates = "carFaguiList.html";
	
	//js
	$jsFile = array(
		'ui/bootstrap.min.js',
		'ui/jquery-ui-selectable.js',
		'admin/car/carFaguiList.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}
...
if($dopost == "getList"){
	...
	if(count($results) > 0){
		...
		if(count($list) > 0){
			echo '{"state": 100, "info": '.json_encode("获取成功").', "pageInfo": {"totalPage": '.$totalPage.', "totalCount": '.$totalCount.'}, "carFagui": '.json_encode($list).'}';
		}else{
