 ...
 * 管理家具行业分类
 *
 * @version        $Id: furnitureIndustry.php 2014-2-28 下午13:40:21 $
...
checkPurview("furnitureIndustry");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__)."/../templates/furniture";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "furnitureIndustry.html";

$action = "furniture_industry";
	...
	foreach ($idsArr as $kk => $id) {
		...
		if($results){
			...
			if($results[0]['parentid'] == 0){
				...
				//查询此分类下的品牌
				$archives = $dsql->SetQuery("SELECT `id`, `logo` FROM `#@__furniture_brand` WHERE `type` = ".$id);
				$results = $dsql->dsqlOper($archives, "results");

				//删除Logo
				if(count($results) > 0){
					foreach($results as $key => $val){
						delPicFile($val['logo'], "delBrand", "furniture");
					}
				}
		...
		$archives = $dsql->SetQuery("DELETE FROM `#@__furniture_brand` WHERE `type` = ".$id);
...
if(file_exists($tpl."/".$templates)){
	...
	$jsFile = array(
		...
		'admin/furniture/furnitureIndustry.js'
	);
