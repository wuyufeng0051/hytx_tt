 ...
 * 管理家具商品订单
 *
 * @version        $Id: furnitureOrder.php 2016-5-4 上午11:51:15 $
...
checkPurview("furnitureOrder");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__)."/../templates/furniture";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "furnitureOrder.html";

$action = "furniture_order";
...
if($dopost == "getList"){
	...
	if($sKeyword != ""){
		...
		//商品
		$proSql = $dsql->SetQuery("SELECT pp.`orderid` FROM `#@__furniture_product` p LEFT JOIN `#@__furniture_order_product` pp ON pp.`proid` = p.`id` WHERE p.`title` like '%$sKeyword%'");
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			//商家
			$userSql = $dsql->SetQuery("SELECT `title` FROM `#@__furniture_store` WHERE `id` = ". $value["store"]);
		...
		if(count($list) > 0){
			echo '{"state": 100, "info": '.json_encode("获取成功").', "pageInfo": {"totalPage": '.$totalPage.', "totalCount": '.$totalCount.', "state0": '.$state0.', "state1": '.$state1.', "state3": '.$state3.', "state4": '.$state4.', "state6": '.$state6.', "state7": '.$state7.', "state10": '.$state10.'}, "furnitureOrder": '.json_encode($list).'}';
		}else{
...
//删除
	...
	if(!testPurview("furnitureOrderDel")){
...
/**
	...
	*/
	...
	if(!testPurview("furnitureOrderEdit")){
		die('{"state": 200, "info": '.json_encode('对不起，您无权使用此功能！').'}');
	}
	if(!empty($id)){
		...
		if($results){
			...
			if($orderstate == 0){
				...
				//订单商品
				$sql = $dsql->SetQuery("SELECT `id`, `proid`, `price`, `count`, `logistic`, `discount` FROM `#@__furniture_order_product` WHERE `orderid` = ".$id);
				...
				if($res){
					...
					foreach ($res as $key => $value) {
						...
						global $handler;
						$handler = true;
						$detailHandels = new handlers("furniture", "detail");
					...
					if($userResult){
						...
						if($userResult[0]['money'] > $orderprice){
							...
							//更新订单支付方式
							$sql = $dsql->SetQuery("UPDATE `#@__furniture_order` SET `paytype` = '管理员支付' WHERE `id` = ".$id);
							...
							foreach ($opArr as $key => $value) {
								...
								//更新订单实付金额
								$sql = $dsql->SetQuery("UPDATE `#@__furniture_order_product` SET `point` = 0, `balance` = 0, `payprice` = '$_price' WHERE `id` = ".$_id);
								$dsql->dsqlOper($sql, "update");

								//更新已购买数量
								$sql = $dsql->SetQuery("UPDATE `#@__furniture_product` SET `sales` = `sales` + $_count, `inventory` = `inventory` - $_count WHERE `id` = ".$_proid);
								$dsql->dsqlOper($sql, "update");
							...
							//记录消费日志
							$logs = $dsql->SetQuery("INSERT INTO `#@__member_money` (`userid`, `amount`, `type`, `info`, `date`) VALUES (".$userid.", ".$orderprice.", 0, '家具消费：".$ordernum."', ".GetMkTime(time()).")");
	...
	if(!empty($id)){
		...
		if($results){
			...
			if($orderstate == 1 || $orderstate == 4 || $orderstate == 6){
				...
				$sql = $dsql->SetQuery("SELECT `proid`, `count`, `point`, `balance`, `payprice` FROM `#@__furniture_order_product` WHERE `orderid` = ".$id);
				$ret = $dsql->dsqlOper($sql, "results");
				if($ret){
				...
				if($userResult){
					...
					if(!empty($totalPoint)){
						...
						//保存操作日志
						$archives = $dsql->SetQuery("INSERT INTO `#@__member_point` (`userid`, `type`, `amount`, `info`, `date`) VALUES ('$userid', '1', '$totalPoint', '家具订单退回：$ordernum', ".GetMkTime(time()).")");
						$dsql->dsqlOper($archives, "update");
					}
					...
					if($totalMoney > 0){
						...
						//记录退款日志
						$logs = $dsql->SetQuery("INSERT INTO `#@__member_money` (`userid`, `amount`, `type`, `info`, `date`) VALUES (".$userid.", ".$totalMoney.", 1, '家具订单退款：".$ordernum."', ".GetMkTime(time()).")");
						$dsql->dsqlOper($logs, "update");
					}
...
if(file_exists($tpl."/".$templates)){
	...
	$jsFile = array(
		...
		'admin/furniture/furnitureOrder.js'
	);
