 ...
 * 查看修改家具订单信息
 *
 * @version        $Id: furnitureOrderEdit.php 2016-05-04 上午12:13:22 $
...
checkPurview("furnitureOrderEdit");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__)."/../templates/furniture";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "furnitureOrderEdit.html";
	...
	if($submit == "提交"){
		...
		if(!empty($id)){
			...
			if(!empty($results)){
				...
				//商家
				$userSql = $dsql->SetQuery("SELECT `title` FROM `#@__furniture_store` WHERE `id` = ". $storeid);
				...
				$sql = $dsql->SetQuery("SELECT `proid`, `price`, `count`, `logistic`, `discount`, `point`, `balance`, `payprice` FROM `#@__".$action."_product` WHERE `orderid` = ".$id);
				...
				if($ret){
					foreach ($ret as $k => $v) {
						...
						$proName = "产品名称";
						$proImg = "";
						$sql = $dsql->SetQuery("SELECT `title`, `litpic` FROM `#@__furniture_product` WHERE `id` = ".$v['proid']);
...
if(file_exists($tpl."/".$templates)){
	...
	$jsFile = array(
		...
		'admin/furniture/furnitureOrderEdit.js'
	);
