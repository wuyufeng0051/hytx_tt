 ...
 * 家具促销
 *
 * @version        $Id: furnitureSale.php 2014-03-01 下午16:55:08 $
...
if($dopost != ""){
	$templates = "furnitureSaleAdd.html";

	//js
	$jsFile = array(
		'admin/furniture/furnitureSaleAdd.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}else{
	$templates = "furnitureSale.html";

	//js
	$jsFile = array(
		'ui/bootstrap.min.js',
		'ui/jquery-ui-selectable.js',
		'admin/furniture/furnitureSale.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}
...
if($dopost == "getList"){
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			//公司
			$companySql = $dsql->SetQuery("SELECT `company` FROM `#@__furniture_store` WHERE `id` = ". $value["company"]);
		...
		if(count($list) > 0){
			echo '{"state": 100, "info": '.json_encode("获取成功").', "pageInfo": {"totalPage": '.$totalPage.', "totalCount": '.$totalCount.'}, "furnitureSale": '.json_encode($list).'}';
		}else{
...
if(file_exists($tpl."/".$templates)){
	...
	$huoniaoTag->assign('companyArr', json_encode(getCompanyList("furniture_store")));
