 ...
 * 添加家具公司
 *
 * @version        $Id: furnitureStoreAdd.php 2016-4-26 下午16:44:20 $
...
$templates = "furnitureStoreAdd.html";

$tab = "furniture_store";
$dopost = $dopost ? $dopost : "save";        //操作类型 save添加 edit修改
if($dopost == "edit"){
	$pagetitle = "修改家具店铺";
	checkPurview("furnitureStoreEdit");
}else{
	$pagetitle = "添加家具店铺";
	checkPurview("furnitureStoreAdd");
}
...
if($_POST['submit'] == "提交"){
	...
	if($domaintype != 0){
		...
		//验证域名是否被使用
		if(!operaDomain('check', $domain, 'furniture', $tab, $id, GetMkTime($domainexp)))
...
if($dopost == "save" && $submit == "提交"){
	...
	if($aid){
		//域名操作
		operaDomain('update', $domain, 'furniture', $tab, $aid, GetMkTime($domainexp), $domaintip);

		adminLog("添加家具店铺", $title);
	...
	if($submit == "提交"){
		...
		if($results == "ok"){
			//域名操作
			operaDomain('update', $domain, 'furniture', $tab, $id, GetMkTime($domainexp), $domaintip);

			adminLog("修改家具店铺", $title);
	...
	if(!empty($id)){
		...
		if(!empty($results)){
			...
			//获取域名信息
			$domainInfo   = getDomain('furniture', $tab, $id);
...
if(file_exists($tpl."/".$templates)){
	...
	$jsFile = array(
		...
		'publicUpload.js',
		'admin/furniture/furnitureStoreAdd.js'
	);
	...
	//获取域名信息
	$domainInfo = getDomain('furniture', 'config');
	...
	$huoniaoTag->assign('typeidListArr', json_encode(getTypeList(0, "furniture_industry")));
