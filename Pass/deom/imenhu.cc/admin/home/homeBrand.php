 ...
 * 家居品牌
 *
 * @version        $Id: homeBrand.php 2014-2-10 下午21:32:58 $
...
checkPurview("homeBrand");

if($dopost != ""){
	$templates = "homeBrandAdd.html";

	//js
	$jsFile = array(
		'ui/jquery.dragsort-0.5.1.min.js',
		'publicUpload.js',
		'admin/home/homeBrandAdd.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}else{
	$templates = "homeBrand.html";

	//js
	$jsFile = array(
		'ui/bootstrap.min.js',
		'ui/jquery-ui-selectable.js',
		'admin/home/homeBrand.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}
...
if($dopost == "getList"){
	...
	if(count($results) > 0){
		...
		if(count($list) > 0){
			echo '{"state": 100, "info": '.json_encode("获取成功").', "pageInfo": {"totalPage": '.$totalPage.', "totalCount": '.$totalCount.'}, "homeBrandList": '.json_encode($list).'}';
		}else{
...
//修改
	...
	if($id != ""){
		foreach($each as $val){
			...
			delPicFile($results[0]['logo'], "delBrand", "home");
...
if(file_exists($tpl."/".$templates)){
	...
	$huoniaoTag->assign('typeListArr', json_encode(getTypeList(0, "home_industry")));
