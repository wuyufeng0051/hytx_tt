 ...
 * 管理家居评论
 *
 * @version        $Id: homeCommon.php 2016-05-04 下午13:00:15 $
...
checkPurview("homeCommon");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__)."/../templates/home";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "homeCommon.html";
...
if($dopost == "getDetail"){
	...
	if(count($results) > 0){
		...
		$typeSql = $dsql->SetQuery("SELECT `id`, `title` FROM `#@__home_product`  WHERE `id` = ". $results[0]["pid"]);
...
//获取评论列表
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			$typeSql = $dsql->SetQuery("SELECT `id`, `title` FROM `#@__home_product` WHERE `id` = ". $value["pid"]);
			$typename = $dsql->getTypeName($typeSql);
