 ...
 * 管理家居行业分类
 *
 * @version        $Id: homeIndustry.php 2014-2-28 下午13:40:21 $
...
checkPurview("homeIndustry");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__)."/../templates/home";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "homeIndustry.html";

$action = "home_industry";
	...
	foreach ($idsArr as $kk => $id) {
		...
		if($results){
			...
			if($results[0]['parentid'] == 0){
				...
				//查询此分类下的品牌
				$archives = $dsql->SetQuery("SELECT `id`, `logo` FROM `#@__home_brand` WHERE `type` = ".$id);
				$results = $dsql->dsqlOper($archives, "results");

				//删除Logo
				if(count($results) > 0){
					foreach($results as $key => $val){
						delPicFile($val['logo'], "delBrand", "home");
					}
				}
		...
		$archives = $dsql->SetQuery("DELETE FROM `#@__home_brand` WHERE `type` = ".$id);
