 ...
 * 查看修改家居订单信息
 *
 * @version        $Id: homeOrderEdit.php 2016-05-04 上午12:13:22 $
...
checkPurview("homeOrderEdit");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__)."/../templates/home";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "homeOrderEdit.html";
	...
	if($submit == "提交"){
		...
		if(!empty($id)){
			...
			if(!empty($results)){
				...
				//商家
				$userSql = $dsql->SetQuery("SELECT `title` FROM `#@__home_store` WHERE `id` = ". $storeid);
				...
				if($ret){
					foreach ($ret as $k => $v) {
						...
						$proName = "产品名称";
						$proImg = "";
						$sql = $dsql->SetQuery("SELECT `title`, `litpic` FROM `#@__home_product` WHERE `id` = ".$v['proid']);
