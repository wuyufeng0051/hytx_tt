 ...
 * 添加家居商品
 *
 * @version        $Id: homeProductAdd.php 2014-3-1 下午23:59:19 $
...
$templates = "homeProductAdd.html";

$tab = "home_product";
$dopost = $dopost ? $dopost : "save";        //操作类型 save添加 edit修改
if($dopost == "edit"){
	$pagetitle = "修改家居商品";
	checkPurview("homeProductEdit");
}else{
	$pagetitle = "添加家居商品";
	checkPurview("homeProductAdd");
}
...
//获取品牌
	...
	if(!empty($id)){
		$typeSql = $dsql->SetQuery("SELECT `parentid` FROM `#@__home_industry` WHERE `id` = ".$id);
		$typeResult = $dsql->dsqlOper($typeSql, "results");
		if($typeResult){
			$brandSql = $dsql->SetQuery("SELECT `id`, `title` FROM `#@__home_brand` WHERE `type` = ".$typeResult[0]['parentid']." ORDER BY `weight`");
...
if(file_exists($tpl."/".$templates)){
	...
	$archives = $dsql->SetQuery("SELECT `id`, `company` FROM `#@__home_store` ORDER BY `weight`");
	$results = $dsql->dsqlOper($archives, "results");
	if($results){
	...
	$archives = $dsql->SetQuery("SELECT * FROM `#@__home_industry` WHERE `parentid` = 0 ORDER BY `weight`");
	$results = $dsql->dsqlOper($archives, "results");
	if($results){
		foreach($results as $key => $val){
			$archives_ = $dsql->SetQuery("SELECT * FROM `#@__home_industry` WHERE `parentid` = ".$val['id']." ORDER BY `weight`");
	...
	$brandOption = array();
	if(!empty($type)){
		$typeSql = $dsql->SetQuery("SELECT `parentid` FROM `#@__home_industry` WHERE `id` = ".$type);
		$typeResult = $dsql->dsqlOper($typeSql, "results");
		if($typeResult){
			$brandSql = $dsql->SetQuery("SELECT `id`, `title` FROM `#@__home_brand` WHERE `type` = ".$typeResult[0]['parentid']." ORDER BY `weight`");
			$brandResult = $dsql->dsqlOper($brandSql, "results");
