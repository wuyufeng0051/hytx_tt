...
if($submit == "预览"){
	...
	$handels = new handlers($action, "hlist");
...
//新增标记
	...
	if($submit == "提交"){
		...
		$archives = $dsql->SetQuery("INSERT INTO `#@__mytag` (`name`, `module`, `type`, `start`, `end`, `config`, `expbody`, `state`, `pubdate`) VALUES ('$name', '$action', 'hlist', '$start', '$end', '$data', '$expbody', '$state', '".GetMkTime(time())."')");
