...
if($dopost == "save" && $submit == "提交"){
	//保存到表
	$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`action`, `loupan`, `title`, `litpic`, `room`, `hall`, `guard`, `area`, `direction`, `high`, `note`, `weight`, `pubdate`) VALUES ('$action', '$loupanid', '$title', '$litpic', '$room', '$hall', '$guard', '$area', '$direction', '$high', '$note', '$weight', '".GetMkTime(time())."')");
	...
	if($submit == "提交"){
		//保存到表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `title` = '$title', `litpic` = '$litpic', `room` = '$room', `hall` = '$hall', `guard` = '$guard', `area` = '$area', `direction` = '$direction', `high` = '$high', `note` = '$note', `weight` = '$weight' WHERE `id` = ".$id);
