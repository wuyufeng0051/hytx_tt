 ...
 * 管理黄页评论
 *
 * @version        $Id: huangyeCommon.php 2013-7-7 上午10:33:36 $
...
checkPurview("huangyeCommon");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__)."/../templates/huangye";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "huangyeCommon.html";
...
//获取评论列表
	...
	if($sKeyword != ""){
		...
		//按信息标题搜索
			...
			$archives = $dsql->SetQuery("SELECT `id` FROM `#@__".$action."list` WHERE `title` like '%$sKeyword%'");
			$results = $dsql->dsqlOper($archives, "results");
			if(count($results) > 0){
				$list = array();
...
if(file_exists($tpl."/".$templates)){
	...
	$jsFile = array(
		...
		'admin/huangye/huangyeCommon.js'
	);
