 ...
 * 管理分类举报信息
 *
 * @version        $Id: infoComplain.php 2015-6-13 下午21:47:11 $
...
checkPurview("huangyeComplain");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__)."/../templates/huangye";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "huangyeComplain.html";
...
if($dopost == "getDetail"){
	...
	if(count($results) > 0){
		...
		$typeSql = $dsql->SetQuery("SELECT `title` FROM `#@__huangyelist` WHERE `id` = ". $results[0]['aid']);
...
//获取举报列表
	...
	$archives = $dsql->SetQuery("SELECT `id` FROM `#@__".$action."` WHERE `module` = 'huangye'");
	...
	$archives = $dsql->SetQuery("SELECT `id`, `aid`, `type`, `userid`, `pubdate`, `ip`, `ipaddr`, `state` FROM `#@__".$action."` WHERE `module` = 'huangye'".$where);
	$results = $dsql->dsqlOper($archives, "results");
...
if(file_exists($tpl."/".$templates)){
	...
	$jsFile = array(
		...
		'admin/huangye/huangyeComplain.js'
	);
