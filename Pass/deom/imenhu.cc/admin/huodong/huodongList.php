...
if($dopost == "getList"){
	...
	if($sAddr != ""){
		if($dsql->getTypeList($sAddr, "huodong_addr")){
			$lower = arr_foreach($dsql->getTypeList($sAddr, "huodong_addr"));
	...
	$archives = $dsql->SetQuery("SELECT `id`, `typeid`, `uid`, `title`, `began`, `end`, `addrid`, `feetype`, `pubdate`, `ip`, `state` FROM `#@__".$tab."` WHERE 1 = 1".$where);
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			$addrSql = $dsql->SetQuery("SELECT `typename` FROM `#@__huodong_addr` WHERE `id` = ". $value["addrid"]);
...
if(file_exists($tpl."/".$templates)){
	...
	$huoniaoTag->assign('addrListArr', json_encode($dsql->getTypeList(0, "huodong_addr")));
