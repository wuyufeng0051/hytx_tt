 ...
 * 添加信息
 *
 * @version        $Id: imageAdd.php 2016-1-18 下午16:43:15 $
...
if($dopost == "edit"){
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("UPDATE `#@__".$action."list` SET `title` = '$title', `subtitle` = '$subtitle', `flag` = '$flags', `redirecturl` = '$redirecturl', `weight` = '$weight', `litpic` = '$litpic', `source` = '$source', `sourceurl` = '$sourceurl', `writer` = '$writer', `typeid` = '$typeid', `keywords` = '$keywords', `description` = '$description', `click` = '$click', `color` = '$color', `arcrank` = '$arcrank', `pubdate` = '$pubdate' WHERE `id` = ".$id);
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("INSERT INTO `#@__".$action."list` (`title`, `subtitle`, `flag`, `redirecturl`, `weight`, `litpic`, `source`, `sourceurl`, `writer`, `typeid`, `keywords`, `description`, `click`, `color`, `arcrank`, `pubdate`, `admin`) VALUES ('$title', '$subtitle', '$flags', '$redirecturl', '$weight', '$litpic', '$source', '$sourceurl', '$writer', '$typeid', '$keywords', '$description', '$click', '$color', '$arcrank', '$pubdate', '$adminid')");
