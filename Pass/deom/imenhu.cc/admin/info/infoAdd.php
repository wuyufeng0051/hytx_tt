...
if($dopost == "edit"){
	...
	if($submit == "提交"){
		...
		if(count($itemResults) > 0){
			foreach ($itemResults as $key=>$value) {
				if($value["required"] == 1 && $$value["field"] == ""){
		...
		if($ret){
			...
			if($arcrank != $arcrank_){
				...
				updateMemberNotice($userid, "会员-发布信息审核通知", $param, array("username" => $username, "title" => $title, "status" => $status, "date" => date("Y-m-d H:i:s", $pubdate)));
		...
		//保存到主表
		$archives = $dsql->SetQuery("UPDATE `#@__".$action."list` SET `typeid` = ".$typeid.", `title` = '".$title."', `color` = '".$color."', `weight` = ".$weight.", `valid` = ".$valid.", `addr` = ".$addr.", `body` = '".$body."', `mbody` = '".$mbody."', `person` = '".$person."', `tel` = '".$tel."', `teladdr` = '".$teladdr."', `qq` = '".$qq."', `click` = ".$click.", `arcrank` = ".$arcrank.", `rec` = '$rec', `fire` = '$fire', `top` = '$top' WHERE `id` = ".$id);
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("INSERT INTO `#@__".$action."list` (`typeid`, `title`, `color`, `weight`, `valid`, `addr`, `body`, `mbody`, `person`, `tel`, `teladdr`, `qq`, `click`, `arcrank`, `ip`, `ipaddr`, `pubdate`, `userid`, `rec`, `fire`, `top`) VALUES (".$typeid.", '".$title."', '".$color."', ".$weight.", ".$valid.", '".$addr."', '".$body."', '".$mbody."', '".$person."', '".$tel."', '".$teladdr."', '".$qq."', ".$click.", ".$arcrank.", '".$ip."', '".$ipAddr."', ".GetMkTime(time()).", ".$adminid.", '$rec', '$fire', '$top')");
