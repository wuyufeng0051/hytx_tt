...
if($dopost == "getList"){
	...
	if($sAddr != ""){
		if($dsql->getTypeList($sAddr, $action."addr")){
			global $arr_data;
			$arr_data = array();
			$lower = arr_foreach($dsql->getTypeList($sAddr, $action."addr"));
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			//地区
			$typeSql = $dsql->SetQuery("SELECT `typename` FROM `#@__".$action."addr` WHERE `id` = ". $value["addr"]);
			$typename = $dsql->getTypeName($typeSql);
