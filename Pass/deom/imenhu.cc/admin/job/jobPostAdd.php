...
if($dopost == "save" && $submit == "提交"){
	//保存到表
	$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`title`, `type`, `company`, `bole`, `sex`, `nature`, `valid`, `number`, `addr`, `experience`, `educational`, `language`, `salary`, `note`, `claim`, `tel`, `email`, `weight`, `click`, `state`, `property`, `pubdate`) VALUES ('$title', '$type', '$comid', '$bole', '$sex', '$nature', '".GetMkTime($valid)."', '$number', '$addr', '$experience', '$educational', '$language', '$salary', '$note', '$claim', '$tel', '$email', '$weight', '$click', '$state', '$property', '".GetMkTime(time())."')");
	...
	if($submit == "提交"){
		//保存到表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `title` = '$title', `type` = '$type', `company` = '$comid', `bole` = '$bole', `sex` = '$sex', `nature` = '$nature', `valid` = '".GetMkTime($valid)."', `number` = '$number', `addr` = '$addr', `experience` = '$experience', `educational` = '$educational', `language` = '$language', `salary` = '$salary', `note` = '$note', `claim` = '$claim', `tel` = '$tel', `email` = '$email', `weight` = '$weight', `click` = '$click', `state` = '$state', `property` = '$property' WHERE `id` = ".$id);
