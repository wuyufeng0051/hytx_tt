 ...
 * 婚尚资讯
 *
 * @version        $Id: marryNews.php 2014-8-4 下午20:53:19 $
...
if($dopost != ""){
	$templates = "marryNewsAdd.html";

	//js
	$jsFile = array(
		'ui/bootstrap.min.js',
		'admin/marry/marryNewsAdd.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}else{
	$templates = "marryNewsList.html";

	//js
	$jsFile = array(
		'ui/bootstrap.min.js',
		'ui/jquery-ui-selectable.js',
		'admin/marry/marryNewsList.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}
...
if($dopost == "getList"){
	...
	if(count($results) > 0){
		...
		if(count($list) > 0){
			echo '{"state": 100, "info": '.json_encode("获取成功").', "pageInfo": {"totalPage": '.$totalPage.', "totalCount": '.$totalCount.'}, "marryNewsList": '.json_encode($list).'}';
		}else{
