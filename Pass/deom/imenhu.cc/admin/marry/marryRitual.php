 ...
 * 管理婚庆礼仪
 *
 * @version        $Id: marryRitual.php 2014-8-2 下午14:20:21 $
...
if($dopost != ""){
	...
	$jsFile = array(
		...
		'admin/marry/marryRitualAdd.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}else{
	$templates = "marryRitual.html";

	//js
	$jsFile = array(
		'ui/bootstrap.min.js',
		'ui/jquery-ui-selectable.js',
		'admin/marry/marryRitual.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}
...
if($submit == "提交"){
	...
	if($dopost == "Add"){
		...
		$userSql = $dsql->SetQuery("SELECT `id` FROM `#@__marry_ritual` WHERE `userid` = '".$userid."'");
		$userResult = $dsql->dsqlOper($userSql, "results");
		if($userResult){
		...
		$userSql = $dsql->SetQuery("SELECT `id` FROM `#@__marry_ritual` WHERE `userid` = '".$userid."' AND `id` != ". $id);
		$userResult = $dsql->dsqlOper($userSql, "results");
		if($userResult){
			echo '{"state": 200, "info": "此会员已授权管理其它公司！"}';
			exit();
	...
	if(empty($address)){
		echo '{"state": 200, "info": "请输入公司详细地址"}';
		exit();
	}

	if(empty($contact)){
		echo '{"state": 200, "info": "请输入公司联系方式"}';
		exit();
	}

	if(empty($people)){
		echo '{"state": 200, "info": "请输入公司详细人"}';
		exit();
	}
}
...
if($dopost == "getList"){
	...
	if(count($results) > 0){
		...
		if(count($list) > 0){
			echo '{"state": 100, "info": '.json_encode("获取成功").', "pageInfo": {"totalPage": '.$totalPage.', "totalCount": '.$totalCount.', "totalGray": '.$totalGray.', "totalAudit": '.$totalAudit.', "totalRefuse": '.$totalRefuse.'}, "marryRitual": '.json_encode($list).'}';
		}else{
...
//新增
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`userid`, `title`, `domaintype`, `levaltype`, `logo`, `worktime`, `addr`, `address`, `lnglat`, `contact`, `people`, `qq`, `website`, `gift`, `content`, `pics`, `click`, `weight`, `state`, `property`, `pubdate`) VALUES ('$userid', '$title', '$domaintype', '$levaltype', '$litpic', '$worktime', '$addr', '$address', '$lnglat', '$contact', '$people', '$qq', '$website', '$gift', '$content', '$imglist', '$click', '$weight', '$state', '$property', '$pubdate')");
...
//修改
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `userid` = '$userid', `title` = '$title', `domaintype` = '$domaintype', `levaltype` = '$levaltype', `logo` = '$litpic', `worktime` = '$worktime', `addr` = '$addr', `address` = '$address', `lnglat` = '$lnglat', `contact` = '$contact', `people` = '$people', `qq` = '$qq', `website` = '$website', `gift` = '$gift', `content` = '$content', `pics` = '$imglist', `click` = '$click', `weight` = '$weight', `state` = '$state', `property` = '$property' WHERE `id` = ".$id);
...
//删除
	...
	if($id != ""){
		...
		foreach($each as $val){
			...
			//删除精品折扣 start
			$archives = $dsql->SetQuery("SELECT `id`, `litpic` FROM `#@__marry_sale` WHERE `action` = 'ritual' AND `cid` = ".$val);
			$results = $dsql->dsqlOper($archives, "results");
			...
			//删除优惠活动 start
			$archives = $dsql->SetQuery("SELECT `id`, `litpic` FROM `#@__marry_deal` WHERE `action` = 'ritual' AND `cid` = ".$val);
			$results = $dsql->dsqlOper($archives, "results");
			foreach($results as $hall){
				//删除缩略图
				delPicFile($hall['litpic'], "delThumb", "marry");
			...
			//删除作品精选 start
			$archives = $dsql->SetQuery("SELECT `id`, `litpic` FROM `#@__marry_works` WHERE `action` = 'ritual' AND `cid` = ".$val);
			$results = $dsql->dsqlOper($archives, "results");
			foreach($results as $hall){
				//删除缩略图
				delPicFile($hall['litpic'], "delThumb", "marry");
			...
			//删除电子优惠券 start
			$archives = $dsql->SetQuery("SELECT `id`, `litpic` FROM `#@__marry_coupon` WHERE `action` = 'ritual' AND `cid` = ".$val);
			$results = $dsql->dsqlOper($archives, "results");
			foreach($results as $hall){
				//删除缩略图
				delPicFile($hall['litpic'], "delThumb", "marry");
...
//更新状态
	...
	if(!testPurview("marryRitualEdit")){
		die('{"state": 200, "info": '.json_encode("对不起，您无权使用此功能！").'}');
	};
	$each = explode(",", $id);
	$error = array();
...
if(file_exists($tpl."/".$templates)){
	...
	if($dopost != ""){
		...
		//公司类型
		$archives = $dsql->SetQuery("SELECT * FROM `#@__marry_item` WHERE `parentid` = 4 ORDER BY `weight` ASC");
		$results = $dsql->dsqlOper($archives, "results");
		$list = array(0 => '请选择');
	...
	$huoniaoTag->assign('typeListArr', json_encode($dsql->getTypeList(4, "marry_item")));
