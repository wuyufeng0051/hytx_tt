 ...
 * 管理作品精选
 *
 * @version        $Id: marryWorks.php 2014-8-4 下午14:17:25 $
...
checkPurview("marry".$action."Works");

$tab = "marry_works";

if($dopost != ""){
	$templates = "marryWorksAdd.html";
	...
	$jsFile = array(
		...
		'admin/marry/marryWorksAdd.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}else{
	$templates = "marryWorks.html";
	
	//js
	$jsFile = array(
		'ui/bootstrap.min.js',
		'ui/jquery-ui-selectable.js',
		'admin/marry/marryWorks.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}
...
if($dopost == "getList"){
	...
	if(count($results) > 0){
		...
		if(count($list) > 0){
			echo '{"state": 100, "info": '.json_encode("获取成功").', "pageInfo": {"totalPage": '.$totalPage.', "totalCount": '.$totalCount.'}, "marryWorks": '.json_encode($list).'}';
		}else{
