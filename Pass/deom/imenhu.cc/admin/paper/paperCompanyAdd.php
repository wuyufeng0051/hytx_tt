...
if($dopost == "save" && $submit == "提交"){
	//保存到表
	$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`title`, `litpic`, `weight`, `state`, `people`, `contact`, `qq`, `pubdate`) VALUES ('$title', '$litpic', '$weight', '$state', '$people', '$contact', '$qq', '".GetMkTime(time())."')");
	...
	if($submit == "提交"){
		//保存到表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `title` = '$title', `litpic` = '$litpic', `weight` = '$weight', `state` = '$state', `people` = '$people', `contact` = '$contact', `qq` = '$qq' WHERE `id` = ".$id);
