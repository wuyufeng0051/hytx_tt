...
if($dopost == "save" && $submit == "提交"){
	//保存到表
	$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`company`, `date`, `title`, `type`, `litpic`, `seotitle`, `keywords`, `description`, `weight`, `state`, `pubdate`) VALUES ('$company', '".GetMkTime($date)."', '$title', '$type', '$litpic', '$seotitle', '$keywords', '$description', '$weight', '$state', '".GetMkTime(time())."')");
	...
	if($submit == "提交"){
		//保存到表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `company` = '$company', `date` = '".GetMkTime($date)."', `title` = '$title', `type` = '$type', `litpic` = '$litpic', `seotitle` = '$seotitle', `keywords` = '$keywords', `description` = '$description', `weight` = '$weight', `state` = '$state' WHERE `id` = ".$id);
...
if(file_exists($tpl."/".$templates)){
	...
	//报刊公司
	$archives = $dsql->SetQuery("SELECT * FROM `#@__paper_company` ORDER BY `weight` DESC, `pubdate` DESC");
	$results = $dsql->dsqlOper($archives, "results");
	$list = array(0 => '请选择');
