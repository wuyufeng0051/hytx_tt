 ...
 * 模板标记风格
 *
 * @version        $Id: mytagTemp.php 2014-5-14 上午12:52:21 $
...
if($dopost == "getList"){
	...
	$archives = $dsql->SetQuery("SELECT `id`, `title`, `module`, `type`, `litpic`, `isSystem`, `state`, `pubdate` FROM `#@__mytag_temp`".$where);
...
//新增标记
	...
	if($submit == "提交"){
		...
		$archives = $dsql->SetQuery("INSERT INTO `#@__mytag_temp` (`title`, `module`, `type`, `litpic`, `css`, `isSystem`, `state`, `pubdate`) VALUES ('$title', '$action', '$do', '$litpic', '$css', '$isSystem', '$state', '".GetMkTime(time())."')");
...
//修改标记
	...
	if($submit == "提交"){
		...
		$archives = $dsql->SetQuery("UPDATE `#@__mytag_temp` SET `title` = '$title', `type` = '$do', `litpic` = '$litpic', `css` = '$css', `isSystem` = '$isSystem', `state` = '$state' WHERE `id` = ".$id);
