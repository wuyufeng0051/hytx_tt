...
if($dopost == "getList"){
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			//地区
			$typeSql = $dsql->SetQuery("SELECT `typename` FROM `#@__renovationaddr` WHERE `id` = ". $value["addrid"]);
			...
			//判断是否过期
			if($value['end'] < GetMkTime(time())){
				// $list[$key]['state'] = "3";
				// $archives = $dsql->SetQuery("UPDATE `#@__renovation_zhaobiao` SET `state` = 3 WHERE `id` = ".$value['id']);
