...
if($_POST['submit'] == "提交"){
	...
	$title = "";
	$archives = $dsql->SetQuery("SELECT `typename` FROM `#@__renovationaddr` WHERE `id` =".$addrid);
...
if($dopost == "save" && $submit == "提交"){
	//保存到表
	$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`title`, `btype`, `budget`, `addrid`, `community`, `address`, `nature`, `area`, `floorplans`, `start`, `end`, `note`, `userid`, `people`, `contact`, `email`, `qq`, `appointment`, `weight`, `click`, `state`, `pubdate`) VALUES ('$title', '$btype', '$budget', '$addrid', '$community', '$address', '$nature', '$area', '$litpic', '".GetMkTime($start)."', '".GetMkTime($end)."', '$note', '$userid', '$people', '$contact', '$email', '$qq', '$appointment', '$weight', '$click', '$state', '".GetMkTime(time())."')");
	...
	if($submit == "提交"){
		//保存到表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `title` = '$title', `btype` = '$btype', `budget` = '$budget', `addrid` = '$addrid', `community` = '$community', `address` = '$address', `nature` = '$nature', `area` = '$area', `floorplans` = '$litpic', `start` = '".GetMkTime($start)."', `end` = '".GetMkTime($end)."', `note` = '$note', `userid` = '$userid', `people` = '$people', `contact` = '$contact', `email` = '$email', `qq` = '$qq', `appointment` = '$appointment', `weight` = '$weight', `click` = '$click', `state` = '$state' WHERE `id` = ".$id);
