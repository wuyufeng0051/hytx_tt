 ...
 * 商城品牌分类
 *
 * @version        $Id: shopBrandType.php 2014-2-10 下午22:15:12 $
...
$templates = "shopBrandType.html";

$tab = "shop_brandtype";

checkPurview("shopBrandType");
	...
	if($id != ""){
		...
		if(!empty($results)){
			...
			if($results != "ok"){
				...
				adminLog("修改商城品牌分类", $action."=>".$typename);
...
//删除分类
	...
	if($id != ""){
		...
		foreach ($idsArr as $kk => $id) {
			...
			//查询此分类下的品牌
			$archives = $dsql->SetQuery("SELECT `id`, `logo` FROM `#@__shop_brand` WHERE `type` = ".$id);
			$results = $dsql->dsqlOper($archives, "results");
			
			//删除Logo
			if(count($results) > 0){
				foreach($results as $key => $val){	
					delPicFile($val['logo'], "delBrand", "shop");
				}
			}
			
		}

		$archives = $dsql->SetQuery("DELETE FROM `#@__shop_brand` WHERE `type` in (".join(",", $idsArr).")");
