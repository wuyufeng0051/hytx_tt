...
if($dopost == "save" && $submit == "提交"){
	//保存到表
	$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`title`, `domaintype`, `company`, `referred`, `addrid`, `address`, `industry`, `project`, `logo`, `userid`, `people`, `contact`, `tel`, `qq`, `note`, `click`, `weight`, `state`, `certi`, `rec`, `pubdate`) VALUES ('$title', '$domaintype', '$company', '$referred', '$addrid', '$address', '$industry', '$project', '$litpic', '$userid', '$people', '$contact', '$tel', '$qq', '$note', '$click', '$weight', '$state', '$certi', '$rec', '".GetMkTime(time())."')");
	...
	if($submit == "提交"){
		//保存到表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `title` = '$title', `domaintype` = '$domaintype', `company` = '$company', `referred` = '$referred', `addrid` = '$addrid', `address` = '$address', `industry` = '$industry', `project` = '$project', `logo` = '$litpic', `userid` = '$userid', `people` = '$people', `contact` = '$contact', `tel` = '$tel', `qq` = '$qq', `note` = '$note', `click` = '$click', `weight` = '$weight', `state` = '$state', `certi` = '$certi', `rec` = '$rec' WHERE `id` = ".$id);
