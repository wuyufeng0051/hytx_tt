...
if($dopost == "edit"){
	...
	if($submit == "提交"){
		...
		//保存到主表
		if($type){
			$archives = $dsql->SetQuery("UPDATE `#@__".$tab."list` SET `class` = ".$class.", `template` = '".$typeid."', `cityid` = '".$cityid."', `title` = '".$title."', `weight` = ".$weight.", `starttime` = ".$starttime.", `endtime` = ".$endtime.", `body` = '".$body."', `state` = '$state' WHERE `id` = ".$id);
		}else{
			$archives = $dsql->SetQuery("UPDATE `#@__".$tab."list` SET `class` = ".$class.", `typeid` = ".$typeid.", `cityid` = '".$cityid."', `title` = '".$title."', `weight` = ".$weight.", `starttime` = ".$starttime.", `endtime` = ".$endtime.", `body` = '".$body."', `state` = '$state' WHERE `id` = ".$id);
		}
...
if(file_exists($tpl."/".$templates)){
	...
	$huoniaoTag->assign('classList', array(1 => '普通广告', 2 => '多图广告', 3 => '伸缩广告', 4 => '对联广告'));
	...
	if($action == "tuan"){
		...
		if($ret){
			...
			foreach ($ret as $key => $value) {
				$addrListArr[$value['cid']] = $value['typename'];
			}
		}
