...
if($dopost == "getList"){
	...
	$archives = $dsql->SetQuery("SELECT `id`, `title`, `color`, `weight`, `arcrank`, `pubdate` FROM `#@__site_noticelist` WHERE 1 = 1".$where);
...
//新增公告
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("INSERT INTO `#@__site_noticelist` (`title`, `color`, `redirecturl`, `weight`, `body`, `arcrank`, `pubdate`) VALUES ('$title', '$color', '$redirecturl', $weight, '$body', $arcrank, $pubdate)");
...
//修改公告
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("UPDATE `#@__site_noticelist` SET `title` = '$title', `color` = '$color', `redirecturl` = '$redirecturl', `weight` = '$weight', `body` = '$body', `arcrank` = '$arcrank' WHERE `id` = ".$id);
		$return = $dsql->dsqlOper($archives, "update");
		...
		if(!empty($id)){
			...
			//主表信息
			$archives = $dsql->SetQuery("SELECT * FROM `#@__site_noticelist` WHERE `id` = ".$id);
			$results = $dsql->dsqlOper($archives, "results");
	...
	if($id != ""){
		...
		foreach($each as $val){
			...
			$archives = $dsql->SetQuery("SELECT * FROM `#@__site_noticelist` WHERE `id` = ".$val);
			$results = $dsql->dsqlOper($archives, "results");
			...
			//删除表
			$archives = $dsql->SetQuery("DELETE FROM `#@__site_noticelist` WHERE `id` = ".$val);
