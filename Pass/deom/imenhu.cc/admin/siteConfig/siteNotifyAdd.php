...
if($dopost == "edit"){
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("UPDATE `#@__".$action."` SET `email_state` = '$email_state', `email_title` = '$email_title', `email_body` = '$email_body', `sms_state` = '$sms_state', `sms_tempid` = '$sms_tempid', `sms_body` = '$sms_body', `wechat_state` = '$wechat_state', `wechat_tempid` = '$wechat_tempid', `wechat_body` = '$wechat_body', `site_state` = '$site_state', `site_title` = '$site_title', `site_body` = '$site_body', `state` = '$state'".$where." WHERE `id` = ".$id);
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("INSERT INTO `#@__".$action."` (`title`, `email_state`, `email_title`, `email_body`, `sms_state`, `sms_tempid`, `sms_body`, `wechat_state`, `wechat_tempid`, `wechat_body`, `site_state`, `site_title`, `site_body`, `state`, `system`) VALUES ('$title', '$email_state', '$email_title', '$email_body', '$sms_state', '$sms_tempid', '$sms_body', '$wechat_state', '$wechat_tempid', '$wechat_body', '$site_state', '$site_title', '$site_body', '$state', '2')");
