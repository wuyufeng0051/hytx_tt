 ...
 * 添加页面元素
 *
 * @version        $Id: pageElementAdd.php 2014-4-26 上午11:24:21 $
...
$templates = "pageElementAdd.html";

$tab = "pageelement";
$dopost = $dopost ? $dopost : "save";        //操作类型 save添加 edit修改
if($dopost == "edit"){
	$pagetitle = "修改页面元素";
	checkPurview("pageElementEdit");
}else{
	$pagetitle = "添加页面元素";
	checkPurview("pageElementAdd");
}
...
if($_POST['submit'] == "提交"){
	...
	//二次验证
	if(empty($sort)){
		echo '{"state": 200, "info": "请选择页面元素分类！"}';
		exit();
	}

	if(empty($title)){
		echo '{"state": 200, "info": "请输入页面元素名称！"}';
		exit();
	}
	...
	if($sort == "apps"){
		...
		if(empty($type)){
			echo '{"state": 200, "info": "请输入页面元素英文名称！"}';
			exit();
		}
	}
...
if(file_exists($tpl."/".$templates)){
	...
	$jsFile = array(
		...
		'ui/bootstrap.min.js',
		'admin/special/pageElementAdd.js'
	);
