...
if($dopost == "save" && $submit == "提交"){
	//保存到表
	$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`title`, `domaintype`, `typeid`, `note`, `head`, `footer`, `litpic`, `weight`, `state`, `pubdate`) VALUES ('$title', '$domaintype', '$typeid', '$note', '$head', '$footer', '$litpic', '$weight', '$state', '".GetMkTime(time())."')");
	...
	if($submit == "提交"){
		//保存到表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `title` = '$title', `domaintype` = '$domaintype', `typeid` = '$typeid', `note` = '$note', `head` = '$head', `footer` = '$footer', `litpic` = '$litpic', `weight` = '$weight', `state` = '$state' WHERE `id` = ".$id);
