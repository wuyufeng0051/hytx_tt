	...
	if($submit == "提交"){
		...
		if(!empty($id)){
			...
			if(!empty($results)){
				...
				$expireddate = $results[0]['expireddate'] == 0 ? "" : date('Y-m-d', $results[0]['expireddate']);
...
if(file_exists($tpl."/".$templates)){
	...
	$huoniaoTag->assign('tuantype', array('0', '1', '2'));
	$huoniaoTag->assign('tuantypenames',array('团购券', '充值卡', '快递'));
