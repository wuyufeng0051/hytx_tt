 ...
 * 模板标记
 *
 * @version        $Id: mytag.php 2014-5-4 下午20:51:21 $
 * @package        HuoNiao.Video
...
$action = empty($action) ? "video" : $action;

//js
$jsFile = array(
	'ui/bootstrap.min.js',
	'ui/jquery-ui-selectable.js',
	'admin/video/mytag.js'
);
...
if($submit == "预览"){
	...
	if(!empty($thumb))   $data["thumb"]   = $thumb;
	if(!empty($orderby)) $data["orderby"] = $orderby;

	$handels = new handlers($action, "alist");
...
//新增标记
	...
	if($submit == "提交"){
		...
		$archives = $dsql->SetQuery("INSERT INTO `#@__mytag` (`name`, `module`, `type`, `start`, `end`, `config`, `expbody`, `state`, `pubdate`) VALUES ('$name', '$action', 'alist', '$start', '$end', '$data', '$expbody', '$state', '".GetMkTime(time())."')");
		...
		if(!empty($config)){
			foreach($config as $key => $val){
				...
				if($key == "typeid"){
					...
					$archives = $dsql->SetQuery("SELECT `typename` FROM `#@__".$action."type` WHERE `id` = ".$val);
					$results = $dsql->dsqlOper($archives, "results");
					if($results){
...
if(file_exists($tpl."/".$templates)){
	...
	$huoniaoTag->assign('typename', empty($typename) ? "不限分类" : $typename);

	//自定义属性
	$huoniaoTag->assign('flagList', array('' => '不限', 'h' => '头条', 'r' => '推荐', 'b' => '加粗', 't' => '跳转'));
