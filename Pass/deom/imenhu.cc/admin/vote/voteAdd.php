...
if($dopost == "edit"){
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("UPDATE `#@__".$action."_list` SET `title` = '".$title."', `color` = '".$color."', `keywords` = '".$keywords."', `description` = '".$description."', `litpic` = '".$litpic."', `began` = '".$began."', `end` = '".$end."', `baomingend` = '".$baomingend."', `ismore` = '".$ismore."', `timelimit` = ".$timelimit.", `voteuser` = '".$voteuser."', `body` = '".$body."', `mbody` = '".$mbody."', `click` = '".$click."', `arcrank` = '$arcrank', `weight` = '".$weight."' WHERE `id` = ".$id);
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("INSERT INTO `#@__".$action."_list` (`title`, `color`, `keywords`, `description`, `litpic`, `began`, `end`, `baomingend`, `ismore`, `timelimit`, `voteuser`, `body`, `mbody`, `click`, `arcrank`, `pubdate`, `admin`, `weight`) VALUES ('".$title."', '".$color."', '".$keywords."', '".$description."', '".$litpic."', ".$began.", ".$end.", '".$baomingend."', '$ismore', '$timelimit', '".$voteuser."', '$body', '".$mbody."', ".$click.", ".$arcrank.", ".GetMkTime(time()).", ".$adminid.", ".$weight.")");
