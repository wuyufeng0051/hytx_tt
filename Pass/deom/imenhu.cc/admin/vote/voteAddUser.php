...
if($dopost == "edit"){
    ...
    if($submit == "提交"){
        ...
        // 查询活动情况
        $arc = $dsql->SetQuery("SELECT `began`,`end`,`baomingend`,`arcrank` FROM `#@__".$action."_list` WHERE `id` = $tid AND `iskill` = 0");
    ...
    $dopost = "save";
    $where = " AND `began` < $now AND `end` > $now AND (`baomingend` = 0 || (`baomingend` <> 0 AND `baomingend` > $now))";
