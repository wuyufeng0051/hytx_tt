...
if($dopost == "save" && $submit == "提交"){
	...
	if($aid){
		...
		//域名操作
		operaDomain('update', $domain, 'website', $tab, $aid, GetMkTime($domainexp), $domaintip);

		adminLog("添加自助建站", $title);
		$RenrenCrypt = new RenrenCrypt();
		$projectid   = base64_encode($RenrenCrypt->php_encrypt($aid));
		echo '{"state": 100, "id": "'.$aid.'", "projectid": "'.$projectid.'"}';
	}else{
	...
	if($submit == "提交"){
		...
		if($results == "ok"){
			//域名操作
			operaDomain('update', $domain, 'website', $tab, $id, GetMkTime($domainexp), $domaintip);
