 ...
 * 自助建站活动中心
 *
 * @version        $Id: websiteEvents.php 2014-06-23 上午10:32:21 $
...
if($dopost != ""){
	$templates = "websiteEventsAdd.html";
	...
	$jsFile = array(
		...
		'admin/website/websiteEventsAdd.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}else{
	$templates = "websiteEvents.html";
	
	//js
	$jsFile = array(
		'ui/bootstrap.min.js',
		'ui/jquery-ui-selectable.js',
		'admin/website/websiteEvents.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}
...
if($dopost == "getList"){
	...
	$archives = $dsql->SetQuery("SELECT `id`, `title`, `click`, `pubdate` FROM `#@__".$tab."` WHERE `website` = $website".$where);
	...
	if(count($results) > 0){
		...
		if(count($list) > 0){
			echo '{"state": 100, "info": '.json_encode("获取成功").', "pageInfo": {"totalPage": '.$totalPage.', "totalCount": '.$totalCount.'}, "websiteEvents": '.json_encode($list).'}';
		}else{
