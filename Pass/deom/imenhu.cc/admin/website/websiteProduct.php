 ...
 * 自助建站产品中心
 *
 * @version        $Id: websiteProduct.php 2014-06-23 上午10:32:21 $
...
if($dopost != ""){
	$templates = "websiteProductAdd.html";
	...
	$jsFile = array(
		...
		'admin/website/websiteProductAdd.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}else{
	$templates = "websiteProduct.html";
	
	//js
	$jsFile = array(
		'ui/bootstrap.min.js',
		'ui/jquery-ui-selectable.js',
		'admin/website/websiteProduct.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}
...
if($dopost == "getList"){
	...
	$archives = $dsql->SetQuery("SELECT `id`, `title`, `typeid`, `litpic`, `click`, `pubdate` FROM `#@__".$tab."` WHERE `website` = $website".$where);
	...
	if(count($results) > 0){
		...
		if(count($list) > 0){
			echo '{"state": 100, "info": '.json_encode("获取成功").', "pageInfo": {"totalPage": '.$totalPage.', "totalCount": '.$totalCount.'}, "websiteProduct": '.json_encode($list).'}';
		}else{
...
//修改应用分类
	...
	$archives = $dsql->SetQuery("SELECT * FROM `#@__".$tab."type` ORDER BY `weight` ASC, `id` ASC");
	$results = $dsql->dsqlOper($archives, "results");
	if($results){
		foreach($results as $key => $val){
