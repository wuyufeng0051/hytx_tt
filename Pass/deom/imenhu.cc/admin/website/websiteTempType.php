 ...
 * 自助建站模板分类
 *
 * @version        $Id: websiteTempType.php 2014-5-27 下午16:36:18 $
...
$templates = "websiteTempType.html";

$tab = "website_temptype";

checkPurview("websiteTempType");
...
//删除分类
	...
	if($id != ""){
		...
		foreach ($idsArr as $kk => $id) {
			...
			//删除分类下模板
			$archives = $dsql->SetQuery("SELECT `id`, `litpic` FROM `#@__website_temp` WHERE `type` = ".$id);
			$results = $dsql->dsqlOper($archives, "results");
			...
			if(count($results) > 0){
				foreach($results as $key => $val){
					...
					//删除模板页面
					$archives = $dsql->SetQuery("DELETE FROM `#@__website_temp_pages` WHERE `tempid` = ".$val['id']);
					$results = $dsql->dsqlOper($archives, "update");
		...
		$archives = $dsql->SetQuery("DELETE FROM `#@__website_temp` WHERE `type` in (".join(",", $idsArr).")");
		$dsql->dsqlOper($archives, "update");

		$archives = $dsql->SetQuery("DELETE FROM `#@__".$tab."` WHERE `id` in (".join(",", $idsArr).")");
		$dsql->dsqlOper($archives, "update");
		
		adminLog("删除自助建站模板分类", join(",", $idsArr));
...
if(file_exists($tpl."/".$templates)){
	...
	$jsFile = array(
		...
		'admin/website/websiteTempType.js'
	);
...
function brandTypeAjax($json, $pid = 0, $tab){
	...
	for($i = 0; $i < count($json); $i++){
		...
		if($id == "" || $id == 0){
			...
			adminLog("添加自助建站模板分类", $model."=>".$name);
		}
