...
if($dopost != ""){
	$templates = "websiteVideoAdd.html";
	...
	$jsFile = array(
		...
		'admin/website/websiteVideoAdd.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}else{
	$templates = "websiteVideo.html";
	
	//js
	$jsFile = array(
		'ui/bootstrap.min.js',
		'ui/jquery-ui-selectable.js',
		'admin/website/websiteVideo.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}
...
if($dopost == "getList"){
	...
	if(count($results) > 0){
		...
		if(count($list) > 0){
			echo '{"state": 100, "info": '.json_encode("获取成功").', "pageInfo": {"totalPage": '.$totalPage.', "totalCount": '.$totalCount.'}, "websiteVideo": '.json_encode($list).'}';
		}else{
...
//新增
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`website`, `title`, `litpic`, `typeid`, `video`, `click`, `keywords`, `description`, `body`, `pubdate`) VALUES ('$website', '$title', '$litpic_', '$typeidArr', '$video', '$click', '$keywords', '$description', '$body', '$pubdate')");
		$return = $dsql->dsqlOper($archives, "update");
		
		if($return == "ok"){
			adminLog("新增自助建站视频", $title);
...
//修改
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `title` = '$title', `litpic` = '$litpic_', `typeid` = '$typeidArr', `video` = '$video', `click` = '$click', `keywords` = '$keywords', `description` = '$description', `body` = '$body' WHERE `id` = ".$id);
		$return = $dsql->dsqlOper($archives, "update");
		
		if($return == "ok"){
			adminLog("修改自助建站视频", $title);
	...
	if($id != ""){
		...
		foreach($each as $val){
			...
			if($results[0]['typeid'] == 0){
				delPicFile($results[0]['video'], "delVideo", "website");
			}
