$(function () {
	...
	$("#btnSubmit").bind("click", function(event){
		...
		$.ajax({
			type: "POST",
			url: "buildBrand.php?dopost="+$("#dopost").val(),
				...
				if(data.state == 100){
					if($("#dopost").val() == "Add"){
						...
						$.dialog({
							...
							ok: function(){
								try{
									$("body",parent.document).find("#nav-buildBrandphp").click();
									//parent.reloadPage($("body",parent.document).find("#body-buildBrandphp")[0].contentWindow);
									parent.reloadPage($("body",parent.document).find("#body-buildBrandphp"));
									$("body",parent.document).find("#nav-buildBrandEdit"+id+" s").click();
								}catch(e){
									location.href = thisPath + "buildBrand.php";
								}
							},
							cancel: false
						});
