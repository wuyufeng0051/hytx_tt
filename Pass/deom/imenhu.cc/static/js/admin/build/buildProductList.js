$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("buildProduct.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("buildProduct.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("buildProductEdit"+id, "build", title, "build/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("buildProduct.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, buildProduct = val.buildProduct;
		...
		if(val.state == "100"){
			...
			for(i; i < buildProduct.length; i++){
				list.push('<tr data-id="'+buildProduct[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row25 left"><img src="'+cfg_attachment+buildProduct[i].litpic+'&type=large" class="litpic" /><span><a href="'+buildProduct[i].url+'" target="_blank">'+buildProduct[i].title+'</a></span></td>');
				var company = '<a href="'+buildProduct[i].companyurl+'" target="_blank">'+buildProduct[i].company+'</a>';
				if(buildProduct[i].companyid == 0){
					company = buildProduct[i].company;
				}
				list.push('  <td class="row20 left">'+company+'</td>');
				list.push('  <td class="row12 left"><a href="javascript:;" data-id="'+buildProduct[i].typeid+'" class="type">'+buildProduct[i].typename+'</td>');
				list.push('  <td class="row7 left">'+buildProduct[i].price+'</td>');
				var state = "";
				switch (buildProduct[i].state) {
					case "0":
				...
				list.push('  <td class="row13">'+buildProduct[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+buildProduct[i].id+'" data-title="'+buildProduct[i].title+'" href="buildProductAdd.php?dopost=edit&id='+buildProduct[i].id+'" title="修改" class="edit">修改</a>');
