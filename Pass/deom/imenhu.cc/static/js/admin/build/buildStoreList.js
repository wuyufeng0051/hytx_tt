$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("buildStoreList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("buildStoreList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("buildStoreEdit"+id, "build", title, "build/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("buildStoreList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, buildStoreList = val.buildStoreList;
		...
		if(val.state == "100"){
			...
			for(i; i < buildStoreList.length; i++){
				list.push('<tr data-id="'+buildStoreList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row20 left"><img src="'+cfg_attachment+buildStoreList[i].logo+'&type=large" class="litpic" /><span><a href="'+buildStoreList[i].url+'" target="_blank">'+buildStoreList[i].company+'</a></span></td>');
				list.push('  <td class="row12"><a href="javascript:;" data-id="'+buildStoreList[i].userid+'" class="userinfo">'+buildStoreList[i].username+'</a></td>');
				list.push('  <td class="row12 left">'+buildStoreList[i].contact+'</td>');
				list.push('  <td class="row10 left">'+buildStoreList[i].addr+'</td>');
				var state = "";
				switch (buildStoreList[i].state) {
					case "0":
				...
				list.push('  <td class="row13">'+buildStoreList[i].pubdate+'</td>');
				list.push('  <td class="row20">');
				list.push('<a data-id="'+buildStoreList[i].id+'" data-title="'+buildStoreList[i].company+'" href="buildStoreAdd.php?dopost=edit&id='+buildStoreList[i].id+'" title="修改" class="edit">修改</a>');
