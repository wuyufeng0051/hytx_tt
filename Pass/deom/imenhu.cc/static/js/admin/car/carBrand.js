$(function(){
	...
	var init = {
			...
			for(var i = 0; i < l; i++){
				(function(){
					...
					if(jsonArray["parentid"] == 0){
						var img = "";
						if(jsonArray["logo"] != ""){
							img = '<img src="'+cfg_attachment+jsonArray["logo"]+'&type=small" style="margin-right:5px;" />';
						}
						typeList.push('  <div class="row3"><a href="javascript:;" class="fold">折叠</a></div>');
						typeList.push('  <div class="row60 left">'+img+'<input type="text" data-id="'+jsonArray["id"]+'" value="'+jsonArray["typename"]+'">'+addType+'</div>');
					}else{
					...
					var editHtml = "";
					if(level == 0){
						editHtml = '<a href="javascript:;" class="edit" title="编辑">编辑</a>';
					}
					typeList.push('  <div class="row17 left">'+editHtml+'<a href="javascript:;" class="del" title="删除">删除编辑</a></div>');
					typeList.push('</div>');
		...
		,quickEdit: function(id, rank){
			if(id == ""){
				huoniao.showTip("warning", "请选择要修改的品牌！", "auto");
			}else{
				huoniao.showTip("loading", "正在获取信息，请稍候...");

				huoniao.operaJson("carBrand.php?dopost=getTypeDetail&action="+action, "id="+id, function(data){
					if(data != null){
						data = data[0];
						huoniao.hideTip();
						...
						$.dialog({
							...
							ok: function(){
								...
								huoniao.operaJson("carBrand.php?dopost=updateType&action="+action, "id="+id+"&"+serialize, function(data){
									if(data.state == 100){
						...
						if(data.logo != ""){
							self.parent.$("#logo").val(data.logo);
							self.parent.$("#logo").siblings("iframe").hide();
							self.parent.$("#logo").siblings(".spic").find(".sholder").html('<img src="'+cfg_attachment+data.logo+'" />');
							self.parent.$("#logo").siblings(".spic").find(".reupload").attr("style", "display:inline-block;");
							self.parent.$("#logo").siblings(".spic").show();
						}
						self.parent.$("#brandStory").val(data.brandStory);
						self.parent.$("#logoStory").val(data.logoStory);
	...
	$("#list").delegate("input", "blur", function(){
		...
		if(id != "" && id != 0){
			huoniao.operaJson("carBrand.php?dopost=updateType&id="+id+"&action="+action, "type=single&typename="+value, function(data){
				if(data.state == 100){
					huoniao.showTip("success", data.info, "auto");
				}else if(data.state == 101){
	...
	$(".root").delegate(".del", "click", function(event){
		...
		if(type.indexOf("编辑") > -1){
			$.dialog.confirm(tip+"删除后无法恢复，请谨慎操作！！！", function(){
				...
				huoniao.operaJson("carBrand.php?dopost=del&action="+action, "id="+id, function(data){
					if(data.state == 100){
						huoniao.showTip("success", data.info, "auto");
						setTimeout(function() {
							location.reload();
			...
			ok: function(){
				...
				if(ids.length <= 0){
					...
					$.dialog.confirm("确定后，此分类下的子级也将同时删除！<br />删除后无法恢复，请谨慎操作！！！", function(){
						...
						huoniao.showTip("loading", "正在删除，，请稍候...");
						huoniao.operaJson("carBrand.php?dopost=del&action="+action, "id="+ids.join(","), function(data){
							if(data.state == 100){
...
function saveOpera(type){
	...
	huoniao.operaJson("carBrand.php?dopost=typeAjax", "data="+json, function(data){
		if(data.state == 100){
			huoniao.showTip("success", data.info, "auto");
			if(type == ""){
				//window.scroll(0, 0);
