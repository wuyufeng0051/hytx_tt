...
$(function () {
	...
	$("#btnSubmit").bind("click", function(event){
		...
		$.ajax({
			type: "POST",
			url: "carFagui.php?dopost="+$("#dopost").val(),
				...
				if(data.state == 100){
					...
					if($("#dopost").val() == "Add"){
						...
						$.dialog({
							...
							ok: function(){
								try{
									$("body",parent.document).find("#nav-carFaguiphp").click();
									//parent.reloadPage($("body",parent.document).find("#body-carFaguiphp")[0].contentWindow);
									parent.reloadPage($("body",parent.document).find("#body-carFaguiphp"));
									$("body",parent.document).find("#nav-carFaguiEdit"+id+" s").click();
								}catch(e){
									location.href = thisPath + "carFagui.php";
								}
							},
							cancel: false
						});
