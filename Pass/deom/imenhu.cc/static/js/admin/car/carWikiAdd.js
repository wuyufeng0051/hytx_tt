...
$(function () {
	...
	$("#btnSubmit").bind("click", function(event){
		...
		$.ajax({
			type: "POST",
			url: "carWiki.php?dopost="+$("#dopost").val(),
				...
				if(data.state == 100){
					...
					if($("#dopost").val() == "Add"){
						...
						$.dialog({
							...
							ok: function(){
								try{
									$("body",parent.document).find("#nav-carWikiphp").click();
									//parent.reloadPage($("body",parent.document).find("#body-carWikiphp")[0].contentWindow);
									parent.reloadPage($("body",parent.document).find("#body-carWikiphp"));
									$("body",parent.document).find("#nav-carWikiEdit"+id+" s").click();
								}catch(e){
									location.href = thisPath + "carWiki.php";
								}
							},
							cancel: false
						});
