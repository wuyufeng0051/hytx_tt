$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("furnitureNews.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#typeManage").bind("click", function(event){
		...
		var href = $(this).attr("href"), type = "furniture";

		try {
			event.preventDefault();
			parent.addPage("furnitureNewsType", type, "家具资讯分类", "furniture/"+href);
		} catch(e) {}
	});
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("furnitureNewsAdd", type, "新增家具资讯", "furniture/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("furnitureNewsEdit"+id, "furniture", title, "furniture/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("furnitureNews.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, furnitureNewsList = val.furnitureNewsList;
		if(val.state == "100"){
			huoniao.hideTip();
			...
			for(i; i < furnitureNewsList.length; i++){
				list.push('<tr data-id="'+furnitureNewsList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row35 left">'+furnitureNewsList[i].title+'</td>');
				list.push('  <td class="row5">'+furnitureNewsList[i].weight+'</td>');
				list.push('  <td class="row15"><a href="javascript:;" data-id="'+furnitureNewsList[i].typeid+'" class="type">'+furnitureNewsList[i].type+'</a></td>');
				list.push('  <td class="row15">'+furnitureNewsList[i].date+'</td>');
				list.push('  <td class="row12">'+furnitureNewsList[i].state+'</td>');
				list.push('  <td class="row15"><a data-id="'+furnitureNewsList[i].id+'" data-title="'+furnitureNewsList[i].title+'" href="furnitureNews.php?dopost=Edit&id='+furnitureNewsList[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
