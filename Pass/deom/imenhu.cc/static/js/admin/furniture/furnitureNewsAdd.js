...
$(function () {
	...
	$("#btnSubmit").bind("click", function(event){
		...
		$.ajax({
			type: "POST",
			url: "furnitureNews.php?dopost="+$("#dopost").val(),
				...
				if(data.state == 100){
					...
					if($("#dopost").val() == "Add"){
						...
						$.dialog({
							...
							ok: function(){
								try{
									$("body",parent.document).find("#nav-furnitureNewsphp").click();
									parent.reloadPage($("body",parent.document).find("#body-furnitureNewsphp"));
									$("body",parent.document).find("#nav-furnitureNewsEdit"+id+" s").click();
								}catch(e){
									location.href = thisPath + "furnitureNews.php";
								}
							},
							cancel: false
						});
