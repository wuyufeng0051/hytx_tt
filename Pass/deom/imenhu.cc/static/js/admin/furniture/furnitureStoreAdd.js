$(function(){
	...
	var init = {
		...
		treeTypeList: function(type){
			var typeList = [], cl = "";
			var l = type == "addr" ? addrListArr : typeidListArr;
			var s = type == "addr" ? addrid : typeid;
	...
	//填充行业
	$("#typeid").html(init.treeTypeList("typeid"));
	...
	function checkGw(t, val, id){
		var flag = false;
		t.addClass("input-loading");
		huoniao.operaJson("../inc/json.php?action=checkStoreUser_2&type=furniture", "key="+val+"&id="+id, function(data){
			t.removeClass("input-loading");
			if(data == 200){
	...
	$("#btnSubmit").bind("click", function(event){
		...
		//异步提交
		huoniao.operaJson("furnitureStoreAdd.php", $("#editform").serialize() + "&submit="+encodeURI("提交"), function(data){
			if(data.state == 100){
				if($("#dopost").val() == "save"){
