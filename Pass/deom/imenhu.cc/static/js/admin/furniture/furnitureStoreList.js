$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("furnitureStoreList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("furnitureStoreList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("furnitureStoreEdit"+id, "furniture", title, "furniture/"+href);
		} catch(e) {}
	});

	//删除
	$("#delBtn").bind("click", function(){
		$.dialog.confirm('确定后将删除此公司下的产品信息<br />此操作不可恢复，您确定要删除吗？', function(){
			init.del();
		});
	});
	...
	$("#list").delegate(".del", "click", function(){
		$.dialog.confirm('确定后将删除此公司下的产品信息<br />您确定要删除吗？', function(){
			init.del();
		});
	});
...
function getList(){
	...
	huoniao.operaJson("furnitureStoreList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, furnitureStoreList = val.furnitureStoreList;
		...
		if(val.state == "100"){
			...
			for(i; i < furnitureStoreList.length; i++){
				list.push('<tr data-id="'+furnitureStoreList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row20 left"><img src="'+cfg_attachment+furnitureStoreList[i].logo+'&type=large" class="litpic" /><span><a href="'+furnitureStoreList[i].url+'" target="_blank">'+furnitureStoreList[i].company+'</a></span></td>');
				list.push('  <td class="row12"><a href="javascript:;" data-id="'+furnitureStoreList[i].userid+'" class="userinfo">'+furnitureStoreList[i].username+'</a></td>');
				list.push('  <td class="row12 left">'+furnitureStoreList[i].contact+'</td>');
				list.push('  <td class="row10 left">'+furnitureStoreList[i].addr+'</td>');
				var state = "";
				switch (furnitureStoreList[i].state) {
					case "0":
				...
				list.push('  <td class="row13">'+furnitureStoreList[i].pubdate+'</td>');
				list.push('  <td class="row20">');
				list.push('<a data-id="'+furnitureStoreList[i].id+'" data-title="'+furnitureStoreList[i].company+'" href="furnitureStoreAdd.php?dopost=edit&id='+furnitureStoreList[i].id+'" title="修改" class="edit">修改</a>');
