...
$(function () {
	...
	$("#btnSubmit").bind("click", function(event){
		...
		$.ajax({
			type: "POST",
			url: "homeNews.php?dopost="+$("#dopost").val(),
				...
				if(data.state == 100){
					...
					if($("#dopost").val() == "Add"){
						...
						$.dialog({
							...
							ok: function(){
								try{
									$("body",parent.document).find("#nav-homeNewsphp").click();
									parent.reloadPage($("body",parent.document).find("#body-homeNewsphp"));
									$("body",parent.document).find("#nav-homeNewsEdit"+id+" s").click();
								}catch(e){
									location.href = thisPath + "homeNews.php";
								}
							},
							cancel: false
						});
