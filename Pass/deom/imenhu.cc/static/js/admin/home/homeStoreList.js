$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("homeStoreList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("homeStoreList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("homeStoreEdit"+id, "home", title, "home/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("homeStoreList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, homeStoreList = val.homeStoreList;
		...
		if(val.state == "100"){
			...
			for(i; i < homeStoreList.length; i++){
				list.push('<tr data-id="'+homeStoreList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row20 left"><img src="'+cfg_attachment+homeStoreList[i].logo+'&type=large" class="litpic" /><span><a href="'+homeStoreList[i].url+'" target="_blank">'+homeStoreList[i].company+'</a></span></td>');
				list.push('  <td class="row12"><a href="javascript:;" data-id="'+homeStoreList[i].userid+'" class="userinfo">'+homeStoreList[i].username+'</a></td>');
				list.push('  <td class="row12 left">'+homeStoreList[i].contact+'</td>');
				list.push('  <td class="row10 left">'+homeStoreList[i].addr+'</td>');
				var state = "";
				switch (homeStoreList[i].state) {
					case "0":
				...
				list.push('  <td class="row13">'+homeStoreList[i].pubdate+'</td>');
				list.push('  <td class="row20">');
				list.push('<a data-id="'+homeStoreList[i].id+'" data-title="'+homeStoreList[i].company+'" href="homeStoreAdd.php?dopost=edit&id='+homeStoreList[i].id+'" title="修改" class="edit">修改</a>');
