...
$(function () {
	...
	thumbnail = function() {
		new SWFUpload({
			upload_url: "/include/upload.inc.php?mod=huangye&type=thumb",
	...
	$("#btnSubmit").bind("click", function(event){
		...
		/*if(!haveBody){
			changePame(1);
			$.dialog.alert("至少添加一个导航及内容！");
			$('.config-nav .btn:eq(1)').click();
			return false;
		}*/
		...
		if(tj){
			...
			$.ajax({
				...
				data: $(this).parents("form").serialize() + "&imglist="+imglist.join(",") + "&detail=" + detail + "&submit=" + encodeURI("提交"),
				dataType: "json",
