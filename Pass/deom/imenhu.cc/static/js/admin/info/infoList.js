	...
	huoniao.operaJson("infoList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < articleList.length; i++){
				...
				list.push('  <td class="row20 left"><a href="'+articleList[i].url+'" target="_blank"'+color+'>'+articleList[i].title+'</a>'+valid+'</td>');
				...
				list.push('  <td class="row13">'+articleList[i].date+'</td>');
