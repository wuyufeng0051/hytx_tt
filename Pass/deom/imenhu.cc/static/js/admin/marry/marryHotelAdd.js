...
$(function(){
	...
	function checkGw(t, val, id){
		var flag = false;
		t.addClass("input-loading");
		huoniao.operaJson("../inc/json.php?action=checkUser_marry&type=hotel", "key="+val+"&id="+id, function(data){
			t.removeClass("input-loading");
			if(data == 200){
				t.siblings(".input-tips").removeClass().addClass("input-tips input-error").html('<s></s>此会员已授权管理其它酒店，一个会员不可以管理多个酒店！');
			}else{
				if(data) {
					for(var i = 0; i < data.length; i++){
				...
				if(flag){
					t.siblings(".input-tips").removeClass().addClass("input-tips input-ok").html('<s></s>如果填写了，则此会员可以管理酒店信息');
				}else{
	...
	$("#btnSubmit").bind("click", function(event){
		...
		if(!huoniao.regex(tables)){
			huoniao.goInput(tables);
			return false;
		}
		...
		//异步提交
		huoniao.operaJson("marryHotel.php", $("#editform").serialize() + "&imglist="+imglist.join(",")+"&submit="+encodeURI("提交"), function(data){
			if(data.state == 100){
				if($("#dopost").val() == "Add"){
