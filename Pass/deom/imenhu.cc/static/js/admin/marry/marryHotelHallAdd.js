$(function(){
	...
	$("#btnSubmit").bind("click", function(event){
		...
		if($.trim(height) == ""){
			$.dialog.alert("请输入宴会厅楼层高度");
			return false;
		}
		
		if($.trim(desk) == ""){
			$.dialog.alert("请输入宴会厅可容纳桌数");
			return false;
		}
		...
		//异步提交
		huoniao.operaJson("marryHotelHall.php", $("#editform").serialize() + "&submit="+encodeURI("提交"), function(data){
			if(data.state == 100){
				if($("#dopost").val() == "Add"){
					$.dialog({
						fixed: true,
					...
					$.dialog({
						...
						ok: function(){
							try{
								$("body",parent.document).find("#nav-marryHotelHall"+$("#hotelid").val()).click();
								//parent.reloadPage($("body",parent.document).find("#body-loupanListphp")[0].contentWindow);
								parent.reloadPage($("body",parent.document).find("#body-marryHotelHall"+$("#hotelid").val()));
								$("body",parent.document).find("#nav-marryHotelHallEdit"+id+" s").click();
							}catch(e){
								location.href = thisPath + "marryHotelHall.php?hotelid="+$("#hotelid").val();
							}
						},
						cancel: false
					});
