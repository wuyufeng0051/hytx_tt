...
$(function(){
	...
	$("#btnSubmit").bind("click", function(event){
		...
		if($.trim(ue.getContent()) == ""){
			$.dialog.alert("请输入折扣详细");
			return false;
		}
		t.attr("disabled", true);	
		
		//异步提交
		huoniao.operaJson("marrySale.php", $("#editform").serialize() + "&submit="+encodeURI("提交"), function(data){
			if(data.state == 100){
				...
				if($("#dopost").val() == "Add"){
					...
					$.dialog({
						...
						ok: function(){
							try{
								$("body",parent.document).find("#nav-marry"+$("#action").val()+"Sale"+$("#cid").val()).click();
								//parent.reloadPage($("body",parent.document).find("#body-loupanListphp")[0].contentWindow);
								parent.reloadPage($("body",parent.document).find("#body-marry"+$("#action").val()+"Sale"+$("#cid").val()));
								$("body",parent.document).find("#nav-marrySaleEdit"+id+" s").click();
							}catch(e){
								location.href = thisPath + "marrySale.php?action="+$("#action").val()+"&cid="+$("#cid").val();
							}
						},
						cancel: false
					});
