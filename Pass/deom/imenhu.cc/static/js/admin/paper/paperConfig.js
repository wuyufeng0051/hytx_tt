$(function(){
	...
	//选择模板
	$("#tplList a.img").bind("click", function(){
		if(!$(this).hasClass("current")){
			$("#tplList a.img").removeClass("current");
			$(this).addClass("current");
			
			$("#tplList .del").show();
			$(this).next("span").find(".del").attr("style", "display:none;");
			$("#articleTemplate").val($(this).attr("data-id"));
			return false;
		}
	});
	
	//模板缩略图放大
	$("#tplList").delegate(".current", "click", function(){
		var url = $(this).find("img").attr("src").replace("preview", "preview_large");
		$.dialog({
			id: 'tplPic',
			title: $(this).next("span").attr("title"),
			lock: true,
			content: '<div style="width: 700px; height: 600px; overflow-y: scroll;"><img src="../'+url+'" width="700" style="display: block; margin: 0 auto;" /></div>',
			padding: 0
		});
	});
	...
	$("#tplList a.del").bind("click", function(){
		...
		$.dialog.confirm('此操作不可恢复，您确定要删除吗？', function(){
			if(!obj.hasClass("current")){
				...
				huoniao.operaJson(formAction+"?action=delTpl", "dopost="+action+"&floder="+encodeURIComponent(floder) + "&token="+$("#token").val(), function(data){
					if(data.state == 100){
