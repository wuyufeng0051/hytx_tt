			...
			if(data.state == 100){
				if($("#dopost").val() == "save"){
					$.dialog({
						...
						ok: function(){
							try{
								$("body",parent.document).find("#nav-paperContent"+forum).click();
								parent.reloadPage($("body",parent.document).find("#body-paperContent"+forum));
								$("body",parent.document).find("#nav-paperContentAdd"+forum+" s").click();
							}catch(e){
								location.href = thisPath + "paperContent.php?forum="+forum;
							}
						},
						cancel: false
					});
					...
					$.dialog({
						...
						ok: function(){
							try{
								$("body",parent.document).find("#nav-paperContent"+forum).click();
								parent.reloadPage($("body",parent.document).find("#body-paperContent"+forum));
								$("body",parent.document).find("#nav-paperContentEdit"+id+" s").click();
							}catch(e){
								location.href = thisPath + "paperContent.php?forum="+forum;
							}
						},
