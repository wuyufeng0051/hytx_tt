$(function(){
	...
	thumbnail = function() {
		new SWFUpload({
			upload_url: "/include/upload.inc.php?mod=paper&type=thumb",
			...
			if(data.state == 100){
				if($("#dopost").val() == "save"){
					...
					$.dialog({
						...
						ok: function(){
							try{
								$("body",parent.document).find("#nav-paperForumphp").click();
								parent.reloadPage($("body",parent.document).find("#body-paperForumphp"));
								$("body",parent.document).find("#nav-paperForumEdit"+id+" s").click();
							}catch(e){
								location.href = thisPath + "paperForum.php";
							}
						},
						cancel: false
					});
