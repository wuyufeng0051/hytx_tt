			...
			if(data.state == 100){
				if($("#dopost").val() == "save"){
					...
					$.dialog({
						...
						ok: function(){
							try{
								$("body",parent.document).find("#nav-renovationCommunityphp").click();
								//parent.reloadPage($("body",parent.document).find("#body-renovationCommunityphp")[0].contentWindow);
								parent.reloadPage($("body",parent.document).find("#body-renovationCommunityphp"));
								$("body",parent.document).find("#nav-communityEdit"+id+" s").click();
							}catch(e){
								location.href = thisPath + "communityList.php";
							}
						},
						cancel: false
					});
