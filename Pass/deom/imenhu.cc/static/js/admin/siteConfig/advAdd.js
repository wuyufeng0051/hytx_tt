...
$(function () {
	...
	//普通广告展现方式切换
	$("input[name=style]").bind("click", function(){
		var val = $(this).val();
			...
			//多图广告
				...
				if(imglistArr1 != ""){
					...
					for(var i = 0; i < imgArray.length; i++){
						...
						picList.push('  </div>');
						picList.push('  <div class="li-input" style="display:block;"><input style="margin:0 10px 10px 0; width:47%; float:left;" class="i-name" placeholder="请输入图片名称" value="'+imgItem[1]+'" /><input style="margin:0 0 10px 0; width:46%; float:left;" class="i-link" placeholder="请输入图片链接" value="'+imgItem[2]+'" /><input class="i-desc" placeholder="请输入图片介绍" value="'+imgItem[3]+'" /></div>');
						picList.push('</li>');
					}
	...
	$("#btnSubmit").bind("click", function(event){
		...
		if(action == "tuan" && cityid == 0){
			$("#addrListArr").siblings(".input-tips").removeClass().addClass("input-tips input-error").attr("style", "display:inline-block");
			tj = false;
			huoniao.goTop();
			return false;
		}else{
			$("#addrListArr").siblings(".input-tips").removeClass().addClass("input-tips input-ok").attr("style", "display:inline-block");
		}
		...
		//多图广告
			...
			content = class2picwidth+'$$'+class2picheight+'$$'+$('#imglist1').val();

		//拉伸广告
		}else if(class_ == 3){
			var showtime  = Number($("#showtime").val()),
				adwidth = Number($("#adwidth").val()),
				extrulink = $("#extrulink").val(),
		...
		//对联广告
			...
			var bodywidth = Number($("#bodywidth").val()),
				...
				leftpiclink = $("#leftpiclink").val(),
				leftpicalt = $("#leftpicalt").val(),
				class4rightpic = $("#class4rightpic").val(),
				rightpiclink = $("#rightpiclink").val(),
				rightpicalt = $("#rightpicalt").val();
			...
			content = bodywidth+'$$'+adwidth+'$$'+adheight+'$$'+topheight+'$$'+class4leftpic+'##'+leftpiclink+'##'+leftpicalt+'$$'+class4rightpic+'##'+rightpiclink+'##'+rightpicalt;
...
function uploadSuccess(obj, file, filetype){
	...
	if(filetype == "swf" || file.split(".")[1] == "swf"){
		media = '<embed src="'+cfg_attachment+file+'" type="application/x-shockwave-flash" quality="high" wmode="transparent">';
	}else{
