$(function(){
	...
	$("#searchBtn").bind("click", function(){
		...
		$("#sCity").html($("#cityBtn").attr("data-id"));
  ...
  $("#keyword").keyup(function (e) {
	...
	$("#cityBtn a").bind("click", function(){
		var id = $(this).attr("data-id"), title = $(this).text();
		$("#cityBtn").attr("data-id", id);
		$("#cityBtn button").html(title+'<span class="caret"></span>');
	});
	...
	$("#pageBtn, #paginationBtn").delegate("a", "click", function(){
		...
		if(obj.attr("id") == "paginationBtn"){
			...
			$("#cityBtn")
				.attr("data-id", "")
				.find("button").html('全部城市<span class="caret"></span>');
	...
	huoniao.operaJson("advList.php?dopost=getList&action="+action+"&type="+atype, data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < adList.length; i++){
				...
				list.push('<tr data-id="'+adList[i].id+'" data-class="'+adList[i].class+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				var id = '';
				//if(action != 'tuan'){
					id = '【'+adList[i].id+'】：';
				//}
				list.push('  <td class="row25 left"><a href="javascript:;" class="preview-ad" title="预览"><i class="icon-eye-open"></i></a>'+id+adList[i].title+'</td>');
				list.push('  <td class="row5">'+adList[i].sort+'</td>');
				list.push('  <td class="row10"><a href="javascript:;" data-id="'+adList[i].typeid+'" class="type">'+adList[i].type+'</a></td>');
				list.push('  <td class="row10">'+adList[i].class+'</td>');
				list.push('  <td class="row13">'+adList[i].start+'</td>');
				list.push('  <td class="row13">'+adList[i].end+'</td>');
				var state = "";
			...
			$("#list .copy").each(function(){
				...
				//团购单独配置
				if(action == "tuan"){
					text.push('动态普通调用：{#myad model="tuan" title="'+tit+'"#}\r\n\r\n');

					if(type == "多图广告"){
						text.push('动态幻灯调用：{#myad model="tuan" title="'+tit+'" type="slide"#}\r\n\r\n');
					}

					text.push('异步普通调用：<script src="/include/json.php?action=adjs&model=tuan&title='+tit+'" language="javascript"></script>\r\n\r\n');

					if(type == "多图广告"){
						text.push('异步幻灯调用：<script src="/include/json.php?action=adjs&model=tuan&title='+tit+'&type=slide" language="javascript"></script>\r\n\r\n');
					}
