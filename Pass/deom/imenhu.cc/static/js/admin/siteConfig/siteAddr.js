$(function(){
		...
		printTypeTree: function(){
			...
			for(var i = 0; i < l; i++){
				(function(){
					...
					typeList.push('  <div class="row60 left"><input type="text" data-type="name" class="input-medium" data-id="'+jsonArray["id"]+'" value="'+jsonArray["typename"]+'" placeholder="名称"><input type="text" data-type="weather_code" class="input-small" data-id="'+jsonArray["id"]+'" value="'+jsonArray["weather_code"]+'" placeholder="城市天气ID" style="margin-left: 20px;"></div>');
	...
	$("#addNew").bind("click", function(){
		...
		html.push('    <div class="row60 left"><input data-id="0" type="text" data-type="name" class="input-medium" placeholder="名称"><input data-id="0" type="text" data-type="weather_code" class="input-small" placeholder="城市天气ID" style="margin-left:20px;"></div>');
...
function saveOpera(type){
	...
	for(var i = 0; i < first.length; i++){
		(function(){
			var html =arguments[0], count = 0, input = $(html).find(".tr input.input-medium"), id = input.attr("data-id"), val = input.val(), weather = $(html).find(".tr input.input-small").val();
			if(val != ""){
				json = json + '{"id": "'+id+'", "name": "'+encodeURIComponent(val)+'", "weather": "'+weather+'"},';
			}
		})(first[i]);
	}
