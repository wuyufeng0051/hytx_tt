	...
	huoniao.operaJson(action+"List.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < infoList.length; i++){
				...
				list.push('  <td class="row30 left"><a href="'+infoList[i].url+'" target="_blank"'+color+'>'+infoList[i].title+'</a></td>');

				var userstr = '<a href="voteAddUser.php?tid='+infoList[i].id+'" class="addUser">添加</a>';
				if(infoList[i].usercount > 0){
					userstr = '已有 <span class="refuse">'+infoList[i].usercount+' </span>&nbsp;&nbsp;'+userstr+'&nbsp;&nbsp;<a href="voteUser.php?tid='+infoList[i].id+'" class="manageUser">管理</a>';
				}
				list.push('  <td class="row15">'+userstr+'</td>');

				var statetime = '';
				switch (infoList[i].statetime) {
					case "0":
						statetime = '<span class="refuse">未开始</span>';
						break;
					case "1":
						statetime = '<span class="audit">进行中</span>';
						break;
					case "2":
						statetime = '<span class="audit">进行中<br><font class="gray">报名结束</font></span>';
						break;
					case "3":
						statetime = '<span class="gray">已结束</span>';
						break;
				}

				list.push('  <td class="row5">'+statetime+'</td>');
				list.push('  <td class="row12">'+infoList[i].began+'</td>');
