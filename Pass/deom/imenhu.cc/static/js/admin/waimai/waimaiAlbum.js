$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("waimaiAlbum.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("waimaiAlbumAdd", "waimai", "添加新相册", "waimai/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("waimaiAlbumEdit"+id, "waimai", title, "waimai/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("waimaiAlbum.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, waimaiAlbum = val.waimaiAlbum;
		...
		if(val.state == "100"){
			...
			for(i; i < waimaiAlbum.length; i++){
				list.push('<tr data-id="'+waimaiAlbum[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row30 left"><img src="'+cfg_attachment+waimaiAlbum[i].path+'&type=small" width="100" class="litpic" />'+waimaiAlbum[i].title+'</td>');
				list.push('  <td class="row10 left">'+waimaiAlbum[i].typeName+'</td>');
				list.push('  <td class="row20 left"><a href="'+waimaiAlbum[i].url+'" target="_blank">'+waimaiAlbum[i].storename+'</a></td>');
				list.push('  <td class="row20 left">'+waimaiAlbum[i].pubdate+'</td>');
				list.push('  <td class="row17">');
				list.push('<a data-id="'+waimaiAlbum[i].id+'" data-title="'+waimaiAlbum[i].title+'" href="waimaiAlbum.php?dopost=edit&id='+waimaiAlbum[i].id+'" title="修改" class="edit">修改</a>');
