...
$(function () {
	...
	$("#btnSubmit").bind("click", function(event){
		...
		$.ajax({
			type: "POST",
			url: "waimaiNews.php?dopost="+$("#dopost").val(),
				...
				if(data.state == 100){
					...
					if($("#dopost").val() == "Add"){
						...
						$.dialog({
							...
							ok: function(){
								try{
									$("body",parent.document).find("#nav-waimaiNewsphp").click();
									//parent.reloadPage($("body",parent.document).find("#body-waimaiNewsphp")[0].contentWindow);
									parent.reloadPage($("body",parent.document).find("#body-waimaiNewsphp"));
									$("body",parent.document).find("#nav-waimaiNewsEdit"+id+" s").click();
								}catch(e){
									location.href = thisPath + "waimaiNews.php";
								}
							},
							cancel: false
						});
