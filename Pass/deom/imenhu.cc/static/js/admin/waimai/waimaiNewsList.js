$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("waimaiNews.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(event){
		...
		var href = $(this).attr("href"), type = "waimai";
		try {
			event.preventDefault();
			parent.addPage("waimaiNewsphpdopostAdd", type, "新增常见问题", "waimai/"+href);
		} catch(e) {}
	});
	...
	$("#typeBtn a").bind("click", function(){
		...
		$("#typeBtn button").html(title+'<span class="waimaiet"></span>');
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("waimaiNewsEdit"+id, "waimai", title, "waimai/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".type", "click", function(event){
		...
		$("#typeBtn")
			.attr("data-id", id)
			.find("button").html(txt+'<span class="waimaiet"></span>');
...
function getList(){
	...
	huoniao.operaJson("waimaiNews.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, waimaiNews = val.waimaiNews;
		if(val.state == "100"){
			huoniao.hideTip();
			
			obj.attr("data-totalpage", val.pageInfo.totalPage);
			
			for(i; i < waimaiNews.length; i++){
				list.push('<tr data-id="'+waimaiNews[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row40 left">'+waimaiNews[i].title+'</td>');
				list.push('  <td class="row15 left"><a href="javascript:;" data-id="'+waimaiNews[i].typeid+'" class="type">'+waimaiNews[i].type+'</a></td>');
				list.push('  <td class="row25">'+waimaiNews[i].date+'</td>');
				list.push('  <td class="row17"><a data-id="'+waimaiNews[i].id+'" data-title="'+waimaiNews[i].title+'" href="waimaiNews.php?dopost=Edit&id='+waimaiNews[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
