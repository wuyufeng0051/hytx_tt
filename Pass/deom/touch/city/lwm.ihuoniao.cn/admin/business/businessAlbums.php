...
if($dopost == "getList"){
	...
	$archives = $dsql->SetQuery("SELECT `id` FROM `#@__".$tab."_albums` WHERE 1 = 1");
	...
	$archives = $dsql->SetQuery("SELECT `id`, `uid`, `typeid`, `litpic`, `pubdate` FROM `#@__".$tab."_albums` WHERE 1 = 1".$where);
...
//删除
	...
	if($id != ""){
		...
		foreach($each as $val){
			...
			$archives = $dsql->SetQuery("SELECT * FROM `#@__".$tab."_albums` WHERE `id` = ".$val);
			$results = $dsql->dsqlOper($archives, "results");
