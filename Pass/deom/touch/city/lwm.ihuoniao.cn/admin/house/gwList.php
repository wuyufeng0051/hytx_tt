...
if($dopost == "getList"){
	...
	$archives = $dsql->SetQuery("SELECT `id`, `type`, `userid`, `stores`, `addr`, `weight`, `state`, `pubdate` FROM `#@__".$tab."` WHERE 1 = 1".$where);
...
//新增
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`type`, `userid`, `stores`, `addr`, `card`, `weight`, `state`, `pubdate`) VALUES ('$typeid', $userid, '$stores', '$addr', '$litpic', $weight, $state, $pubdate)");
...
//修改
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `type` = $typeid, `userid` = $userid, `stores` = '$stores', `addr` = $addr, `card` = '$litpic', `weight` = $weight, `state` = $state WHERE `id` = ".$id);
		$return = $dsql->dsqlOper($archives, "update");
...
if(file_exists($tpl."/".$templates)){
	...
	$huoniaoTag->assign('kfsListArr', json_encode(getKfsList("house_kfs")));
