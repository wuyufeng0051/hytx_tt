 ...
 * 管理房源评论
 *
 * @version        $Id: houseCommon.php 2013-12-13 上午9:55:16 $
...
checkPurview("houseCommonloupan");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__)."/../templates/house";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "houseCommon.html";

$tab = "house";

if($dopost == "getDetail"){
	if($id == "") die;
	$archives = $dsql->SetQuery("SELECT `aid`, `userid`, `content`, `dtime`, `ip`, `ipaddr`, `good`, `bad`, `ischeck` FROM `#@__".$tab."common` WHERE `id` = ".$id);
	$results = $dsql->dsqlOper($archives, "results");

	if(count($results) > 0){

		if($action == "loupan"){
			$archives = $dsql->SetQuery("SELECT `title` FROM `#@__house_loupan` WHERE `id` = ".$results[0]["aid"]);
			$dsqlInfo = $dsql->dsqlOper($archives, "results");
		}else{
			$archives = $dsql->SetQuery("SELECT `title` FROM `#@__house_community` WHERE `id` = ".$results[0]["aid"]);
			$dsqlInfo = $dsql->dsqlOper($archives, "results");
		}
...
//更新评论状态
	...
	$error = array();
	foreach($each as $val){
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."common` SET `ischeck` = $arcrank WHERE `id` = ".$val);
		$results = $dsql->dsqlOper($archives, "update");
		if($results != "ok"){
			$error[] = $val;
		}
...
//删除评论
	...
	$error = array();
	foreach($each as $val){
		$archives = $dsql->SetQuery("DELETE FROM `#@__".$tab."common` WHERE `id` = ".$val);
		$results = $dsql->dsqlOper($archives, "update");
		if($results != "ok"){
			$error[] = $val;
		}
...
//获取评论列表
	...
	if($sKeyword != ""){
		...
		//按信息标题搜索
			...
			if($action == "loupan"){
				$archives = $dsql->SetQuery("SELECT `title` FROM `#@__house_loupan` WHERE `title` like '%$sKeyword%'");
				$results = $dsql->dsqlOper($archives, "results");
			}elseif($action == ""){
				$archives = $dsql->SetQuery("SELECT `title` FROM `#@__house_community` WHERE `title` like '%$sKeyword%'");
				$results = $dsql->dsqlOper($archives, "results");
			}
			if(count($results) > 0){
				$list = array();
	...
	$archives = $dsql->SetQuery("SELECT `id` FROM `#@__".$tab."common`");
	...
	//待审核
	$totalGray = $dsql->dsqlOper($archives." WHERE `action` = '".$action."' AND `ischeck` = 0".$where, "totalCount");
	//已审核
	$totalAudit = $dsql->dsqlOper($archives." WHERE `action` = '".$action."' AND `ischeck` = 1".$where, "totalCount");
	//拒绝审核
	$totalRefuse = $dsql->dsqlOper($archives." WHERE `action` = '".$action."' AND `ischeck` = 2".$where, "totalCount");
	...
	$archives = $dsql->SetQuery("SELECT `id`, `aid`, `userid`, `content`, `dtime`, `ip`, `ipaddr`, `ischeck` FROM `#@__".$tab."common` WHERE `action` = '".$action."'".$where);
	$results = $dsql->dsqlOper($archives, "results");
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			if($action == "loupan"){
				$typeSql = $dsql->SetQuery("SELECT `title` FROM `#@__house_loupan` WHERE `id` = ". $value["aid"]);
			}else{
				$typeSql = $dsql->SetQuery("SELECT `title` FROM `#@__house_community` WHERE `id` = ". $value["aid"]);
			}
