...
if($dopost == "getList"){
	...
	$archives = $dsql->SetQuery("SELECT `id`, `title`, `action`, `type`, `addr`, `person`, `contact`, `weight`, `userid`, `state`, `pubdate` FROM `#@__".$tab."` WHERE 1 = 1".$where);
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			//地区
			$addrSql = $dsql->SetQuery("SELECT `typename` FROM `#@__houseaddr` WHERE `id` = ". $value["addr"]);
...
//新增
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`title`, `note`, `action`, `type`, `addr`, `person`, `contact`, `weight`, `userid`, `state`, `pubdate`) VALUES ('$title', '$note', $action, $type, $addrid, '$person', '$contact', $weight, ".$userLogin->getUserID().", $state, $pubdate)");
...
//修改
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `title` = '$title', `note` = '$note', `action` = '$action', `type` = '$type', `addr` = '$addrid', `person` = '$person', `contact` = '$contact', `weight` = '$weight', `state` = '$state' WHERE `id` = ".$id);
		$return = $dsql->dsqlOper($archives, "update");
...
if(file_exists($tpl."/".$templates)){
	...
	if($dopost != ""){
		...
		$huoniaoTag->assign('actionList', array('1', '2', '3', '4', '5', '6'));
		$huoniaoTag->assign('actionName',array('新房', '二手房', '出租房', '写字楼', '商铺', '厂房/仓库'));
