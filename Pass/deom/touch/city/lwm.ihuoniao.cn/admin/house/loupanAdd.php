...
if($dopost == "save" && $submit == "提交"){
	//保存到表
	$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`title`, `addrid`, `addr`, `longitude`, `latitude`, `subway`, `litpic`, `bussiness`, `deliverdate`, `opendate`, `price`, `ptype`, `views`, `weight`, `state`, `salestate`, `hot`, `rec`, `tuan`, `tuantitle`, `tuanbegan`, `tuanend`, `userid`, `investor`, `protype`, `address`, `tel`, `worktime`, `note`, `buildtype`, `zhuangxiu`, `buildage`, `planarea`, `buildarea`, `planhouse`, `linklocal`, `parknum`, `rongji`, `green`, `floor`, `property`, `proprice`, `config`, `pubdate`) VALUES ('$title', '$addrid', '$addr', '$longitude', '$latitude', '$subway', '$litpic', '$bussiness', '".GetMkTime($deliverdate)."', '".GetMkTime($opendate)."', '$price', '$ptype', '$views', '$weight', '$state', '$salestate', '$hot', '$rec', '$tuan', '$tuantitle', '$tuanbegan', '$tuanend', '$userid', '$investor', '$protype', '$address', '$tel', '$worktime', '$note', '$buildtype', '$zhuangxiu', '$buildage', '$planarea', '$buildarea', '$planhouse', '$linklocal', '$parknum', '$rongji', '$green', '$floor', '$property', '$proprice', '$config', '".GetMkTime(time())."')");
	...
	if($submit == "提交"){
		//保存到表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `title` = '$title', `addrid` = '$addrid', `addr` = '$addr', `longitude` = '$longitude', `latitude` = '$latitude', `subway` = '$subway', `litpic` = '$litpic', `bussiness` = '$bussiness', `deliverdate` = '".GetMkTime($deliverdate)."', `opendate` = '".GetMkTime($opendate)."', `price` = '$price', `ptype` = '$ptype', `views` = '$views', `weight` = '$weight', `state` = '$state', `salestate` = '$salestate', `hot` = '$hot', `rec` = '$rec', `tuan` = '$tuan', `tuantitle` = '$tuantitle', `tuanbegan` = '$tuanbegan', `tuanend` = '$tuanend', `userid` = '$userid', `investor` = '$investor', `protype` = '$protype', `address` = '$address', `tel` = '$tel', `worktime` = '$worktime', `note` = '$note', `buildtype` = '$buildtype', `zhuangxiu` = '$zhuangxiu', `buildage` = '$buildage', `planarea` = '$planarea', `buildarea` = '$buildarea', `planhouse` = '$planhouse', `linklocal` = '$linklocal', `parknum` = '$parknum', `rongji` = '$rongji', `green` = '$green', `floor` = '$floor', `property` = '$property', `proprice` = '$proprice', `config` = '$config', `pubdate` = '".GetMkTime(time())."' WHERE `id` = ".$id);
...
if(file_exists($tpl."/".$templates)){
	...
	$huoniaoTag->assign('ptypeList', array("1" => "元/㎡", "2" => "万元/套"));
