...
if($dopost == "getList"){
	...
	if($sKeyword != ""){
		...
		//模糊匹配会场
		$sql = $dsql->SetQuery("SELECT `id` FROM `#@__job_fairs_center` WHERE `title` like '%$sKeyword%'");
...
//根据关键词查询会场
	...
	$key = $_POST['key'];
	if(!empty($key)){
		$sql = $dsql->SetQuery("SELECT `id`, `title` FROM `#@__job_fairs_center` WHERE `title` like '%$key%' LIMIT 0, 10");
		$result = $dsql->dsqlOper($sql, "results");
		if($result){
