...
if($submit == "提交"){
	...
	if($dopost == "Add"){
		...
		preg_match("/^[A-Za-z0-9_\x{4e00}-\x{9fa5}]{3,15}$/iu", $username, $matchUsername);
		if(!$matchUsername){
...
//获取会员列表
	...
	$archives = $dsql->SetQuery("SELECT `id`, `mtype`, `username`, `discount`, `nickname`, `realname`, `certifyState`, `email`, `emailCheck`, `phone`, `phoneCheck`, `company`, `licenseState`, `photo`, `sex`, `money`, `point`, `regtime`, `regip`, `lastlogintime`, `lastloginip`, `state` FROM `#@__".$db."` WHERE 1 = 1".$where);
...
//新增
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("INSERT INTO `#@__".$db."` (`mtype`, `username`, `password`, `nickname`, `email`, `emailCheck`, `phone`, `phoneCheck`, `company`, `regtime`, `regip`, `state`) VALUES ('$mtype', '$username', '$passwd', '$nickname', '$email', '1', '$phone', '1', '$company', '$regtime', '$regip', '1')");
...
//修改
	...
	if($submit == "提交"){
		...
		$archives = $dsql->SetQuery("UPDATE `#@__".$db."` SET `mtype` = '$mtype'".$passArr.", `discount` = '$discount', `nickname` = '$nickname', `email` = '$email', `emailCheck` = '$emailCheck', `phone` = '$phone', `phoneCheck` = '$phoneCheck', `qq` = '$qq', `photo` = '$photo', `sex` = '$sex', `birthday` = '$birthday', `company` = '$company', `realname` = '$realname', `idcard` = '$idcard', `idcardFront` = '$idcardFront', `idcardBack` = '$idcardBack', `certifyState` = '$certifyState', `certifyInfo` = '$certifyInfo', `addr` = '$addr', `address` = '$address', `license` = '$license', `licenseState` = '$licenseState', `licenseInfo` = '$licenseInfo', `state` = '$state', `stateinfo` = '$stateinfo' WHERE `id` = ".$id);
		...
		if($update == "ok"){
			...
			if($certifyState_ != $certifyState){
				...
				//实名认证通过
				if($certifyState == 1){
					updateMemberNotice($id, "会员-实名认证审核通过", $param, array("email" => $email, "username" => $username));
				}

				//实名认证未通过
				if($certifyState == 2){
					updateMemberNotice($id, "会员-实名认证审核失败", $param, array("email" => $email, "username" => $username, "info" => $certifyInfo));
				}
			...
			if($licenseState_ != $licenseState){
				...
				//营业执照认证通过
				if($licenseState == 1){
					updateMemberNotice($id, "会员-营业执照审核通过", $param, array("email" => $email, "username" => $username));
				}

				//营业执照认证未通过
				if($licenseState == 2){
					updateMemberNotice($id, "会员-营业执照审核失败", $param, array("email" => $email, "username" => $username, "info" => $licenseInfo));
				}
			}
...
//获取帐户日志
	...
	$archives = $dsql->SetQuery("SELECT `id` FROM `#@__".$db."_".$type."` WHERE 1 = 1");
	...
	$atpage = $pagestep*($page-1);
	$where .= " LIMIT $atpage, $pagestep";
	$archives = $dsql->SetQuery("SELECT `id`, `type`, `amount`, `info`, `date` FROM `#@__".$db."_".$type."` WHERE 1 = 1".$where);
	$results = $dsql->dsqlOper($archives, "results");
...
//增加帐户操作记录
	...
	if($aid){
		...
		//余额
		if($action == "money"){
			updateMemberNotice($userid, "会员-帐户资金变动提醒", $param, array("username" => $username, "amount" => $oper.$amount, "money" => $money, "date" => date("Y-m-d H:i:s", $date), "info" => $info));
