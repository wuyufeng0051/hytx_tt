...
if($action == ""){
	...
	//其它属性
	$huoniaoTag->assign('flagopt', array('0', '1', '2', '3'));
	$huoniaoTag->assign('flagnames',array('推荐','特价','热卖','限时抢'));
...
//上架新商品
	...
	//保存到主表
	$archives = $dsql->SetQuery("INSERT INTO `#@__shop_product` (`type`, `title`, `brand`, `property`, `store`, `category`, `mprice`, `price`, `logistic`, `volume`, `weight`, `specification`, `inventory`, `limit`, `litpic`, `sort`, `click`, `state`, `flag`, `btime`, `etime`, `pics`, `body`, `mbody`, `pubdate`) VALUES ('$typeid', '$title', '$brand', '$property', '$store', '$category', '$mprice', '$price', '$logistic', '$volume', '$weight', '$specifival', '$inventory', '$limit', '$litpic', '$sort', '$click', '$state', '$flag', '$btime', '$etime', '$imglist', '$body', '$mbody', ".GetMkTime(time()).")");
