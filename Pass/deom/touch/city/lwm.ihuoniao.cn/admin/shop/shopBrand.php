...
//新增
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`type`, `title`, `logo`, `weight`, `rec`, `pubdate`) VALUES ($typeid, '$title', '$litpic', $weight, $rec, $pubdate)");
...
//修改
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `type` = $typeid, `title` = '$title', `logo` = '$litpic', `weight` = $weight, `rec` = $rec WHERE `id` = ".$id);
		$return = $dsql->dsqlOper($archives, "update");
