...
function typeOpera($arr, $pid = 0, $db, $level){
	...
	for($i = 0; $i < count($arr); $i++){
		...
		//如果ID为空则向数据库插入下级分类
		if($id == "" || $id == 0){
			$archives = $dsql->SetQuery("INSERT INTO `#@__".$db."` (`parentid`, `typename`, `pinyin`, `level`, `weather_code`, `weight`) VALUES ('$pid', '$name', '$pinyin', '$level', '$weather', '$i')");
		...
		//其它为数据库已存在的分类需要验证名称或天气ID是否有改动，如果有改动则UPDATE
			...
			$archives = $dsql->SetQuery("SELECT `typename`, `weather_code`, `weight` FROM `#@__".$db."` WHERE `id` = ".$id);
			$results = $dsql->dsqlOper($archives, "results");
			if(!empty($results)){
				//验证名称
