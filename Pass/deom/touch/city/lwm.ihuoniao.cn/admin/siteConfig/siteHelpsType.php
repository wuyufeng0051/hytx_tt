 ...
 * 帮助信息分类
 *
 * @version        $Id: siteHelpsType.php 2013-11-30 下午23:49:25 $
...
//删除分类
	...
	if($id != ""){
		...
		foreach ($idsArr as $kk => $id) {
			...
			//删除内容图片
			$archives = $dsql->SetQuery("SELECT `body` FROM `#@__".$action."list` WHERE `typeid` = ".$id);
			$results = $dsql->dsqlOper($archives, "results");
			if($results){
...
if(file_exists($tpl."/".$templates)){
	...
	$jsFile = array(
		...
		'admin/siteConfig/siteHelpsType.js'
	);
