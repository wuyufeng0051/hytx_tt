 ...
 * 团购频道配置
 *
 * @version        $Id: tuanConfig.php 2013-12-3 下午15:53:49 $
...
checkPurview("tuanConfig");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__)."/../templates/tuan";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "tuanConfig.html";

$action =  $action == "" ? "tuan" : $action;
...
if($type != ""){
	...
	if($customSubDomain == 2){
		...
		$sql = $dsql->SetQuery("UPDATE `#@__tuan_city` SET `type` = 2 WHERE `type` = 1");
	...
	//推荐返现金额
	$customInc .= "\$recMoney = ".$recMoney.";\r\n";
	$customInc .= "\$singelnum = ".$singelnum.";\r\n";
	//地图
...
if(file_exists($tpl."/".$templates)){
	...
	//推荐返现金额
	$huoniaoTag->assign('recMoney', $recMoney);
	$huoniaoTag->assign('singelnum', $singelnum);
