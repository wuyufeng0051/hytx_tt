 ...
 * 团购常见问题分类
 *
 * @version        $Id: tuanFaqType.php 2013-12-5 下午21:01:21 $
...
checkPurview("tuanFaqType");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__)."/../templates/tuan";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "tuanFaqType.html";
...
//修改分类
	...
	if(!testPurview("editTuanFaqType")){
...
//删除分类
	...
	foreach ($idsArr as $kk => $id) {
		...
		//查询此分类下所有信息ID
		$archives = $dsql->SetQuery("SELECT `id`, `body` FROM `#@__".$action."list` WHERE `typeid` = ".$id);
		$results = $dsql->dsqlOper($archives, "results");
		...
		$archives = $dsql->SetQuery("DELETE FROM `#@__".$action."list` WHERE `typeid` = ".$id);
