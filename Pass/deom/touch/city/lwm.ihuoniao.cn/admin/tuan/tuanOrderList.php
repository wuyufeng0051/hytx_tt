...
if($dopost == "getList"){
	...
	$archives = $dsql->SetQuery("SELECT `id`, `ordernum`, `userid`, `proid`, `orderprice`, `procount`, `propolic`, `orderstate`, `ret-state`, `exp-date`, `usercontact`, `orderdate`, `paytype` FROM `#@__".$action."` WHERE 1 = 1".$where);
...
//删除
	...
	$error = array();
	foreach($each as $val){
		$archives = $dsql->SetQuery("SELECT `id`, `ordernum`, `userid`, `proid`, `procount`, `orderprice`, `orderstate` FROM `#@__".$action."` WHERE `id` = ".$val);
		$results = $dsql->dsqlOper($archives, "results");
		...
		//退款
		if($orderstate != 0 && $orderstate != 3 && $orderstate != 7){
			//团购商品
...
//退款
	...
	if(!empty($id)){
		$archives = $dsql->SetQuery("SELECT o.`ordernum`, o.`point`, o.`balance`, o.`payprice`, o.`userid`, o.`proid`, o.`procount`, o.`orderprice`, o.`propolic`, o.`orderstate`, s.`uid`, l.`title` FROM `#@__".$action."` o LEFT JOIN `#@__tuanlist` l ON l.`id` = o.`proid` LEFT JOIN `#@__tuan_store` s ON s.`id` = l.`sid` WHERE o.`id` = ".$id);
		$results = $dsql->dsqlOper($archives, "results");
