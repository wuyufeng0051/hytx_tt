    ...
    foreach ($shopArr as $key => $value) {
        ...
        if($ret){
            ...
            foreach ($ret as $k => $v) {
                ...
                $foodTotal = $peisongTotal = $dabaoTotal = $addserviceTotal = $discountTotal = $promotionTotal = $firstdiscountTotal = 0;
                ...
                foreach ($priceinfo as $k_ => $v_) {
                    if($v_['type'] == "peisong"){
                        ...
                        $peisongTotalPrice += sprintf("%.2f", $v_['amount']);
                    }
                    if($v_['type'] == "dabao"){
                        $dabaoTotal += sprintf("%.2f", $v_['amount']);
                        $dabaoTotalPrice += sprintf("%.2f", $v_['amount']);
                    }
                    if($v_['type'] == "fuwu"){
                        $addserviceTotal += sprintf("%.2f", $v_['amount']);
                        $addserviceTotalPrice += sprintf("%.2f", $v_['amount']);
                    }
                    if($v_['type'] == "youhui"){
                        $discountTotal += sprintf("%.2f", $v_['amount']);
                        $discountTotalPrice += sprintf("%.2f", $v_['amount']);
                    }
                    if($v_['type'] == "manjian"){
                        $promotionTotal += sprintf("%.2f", $v_['amount']);
                        $promotionTotalPrice += sprintf("%.2f", $v_['amount']);
                    }
                    if($v_['type'] == "shoudan"){
                        $firstdiscountTotal += sprintf("%.2f", $v_['amount']);
                        $firstdiscountTotalPrice += sprintf("%.2f", $v_['amount']);
                    }
                }

                //计算总交易额
                $zjye = sprintf("%.2f", $foodTotal - $discountTotal - $promotionTotal - $firstdiscountTotal + $dabaoTotal + $peisongTotal + $addserviceTotal);
                $turnover += $zjye;

                //计算平台应得金额
                $ptyd = sprintf("%.2f", $foodTotal * $fencheng_foodprice / 100 - $discountTotal * $fencheng_discount / 100 - $promotionTotal * $fencheng_promotion / 100 - $firstdiscountTotal * $fencheng_firstdiscount / 100 + $dabaoTotal * $fencheng_dabao / 100 + $peisongTotal * $fencheng_delivery / 100 + $addserviceTotal * $fencheng_addservice / 100);
                $platform += $ptyd;

                //商家应得
                $business += sprintf("%.2f", $zjye - $ptyd);
            ...
            $ret = $dsql->dsqlOper($sql, "results");
            $delivery += sprintf("%.2f", $ret[0]['total']);
