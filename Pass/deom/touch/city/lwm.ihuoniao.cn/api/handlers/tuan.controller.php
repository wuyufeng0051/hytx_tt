...
function tuan($params, $content = "", &$smarty = array(), &$repeat = array()){
	...
	//获取指定ID的详细信息
	}elseif($action == "detail" || $action == "buy" || $action == "review" || $action == "pic"){
		...
		$action = $action == "pic" || $action == "review" ? "detail" : $action;
		...
		if(!empty($ordernum)){
			...
			if($payDetail){
				...
				foreach ($ids as $key => $value) {
					...
					//查询订单详细信息
					$archives = $dsql->SetQuery("SELECT `id`, `proid`, `procount`, `orderprice`, `useraddr`, `username`, `usercontact` FROM `#@__tuan_order` WHERE `ordernum` = '$value' AND `userid` = $userid");
					$orderDetail  = $dsql->dsqlOper($archives, "results");
					if($orderDetail){
