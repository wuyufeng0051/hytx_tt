...
if(isset($set_modules) && $set_modules == TRUE){
    ...
    /* 回调地址 */
    $login[$i]['callback'] = "http://".$cfg_basehost."/api/login.php?action=back&type=wechat";
...
class Loginwechat {
    ...
    function login($data){
        ...
        //PC端显示二维码
            ...
            //记录生成日志
            $time = GetMkTime(time());
    		$sql = $dsql->SetQuery("INSERT INTO `#@__site_wxlogin` (`state`, `uid`, `date`) VALUES ('$state', '0', '$time')");
            ...
            if(file_exists($tpl.$templates)){
        		...
        		$loginRedirect = $loginRedirect ? $loginRedirect : "http://".$cfg_basehost;
	...
	function back($data, $return){
      ...
      if($return['state'] == $_SESSION['state'] && !empty($return['code'])){
        ...
        if($unionid){
    		$sql = $dsql->SetQuery("SELECT `id` FROM `#@__member` WHERE `".$logincode."_conn` = '$unionid'");
        ...
        if(isset($user_info['errcode']) && $cfg_wechatType == "0"){
            ...
            header("Location:http://$cfg_basehost/api/login.php?type=wechat&qr=".$return['qr']);
            die;
        }
