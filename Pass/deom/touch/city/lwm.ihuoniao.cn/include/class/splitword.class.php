 ...
 *
 * @version        $Id: splitword.class.php 2015-02-13 上午11:31:24 $
...
class splitword
{
    ...
    function __construct($source_charset='utf-8', $target_charset='utf-8', $load_all=TRUE, $source='')
    {
        ...
        if(file_exists(HUONIAOINC.'/'.$this->mainDicFile)) $this->isUnpacked = TRUE;
    ...
    */
    ...
    {
		$this->addonDicFile = HUONIAOINC.'/'.$this->addonDicFile;
		$this->mainDicFile = HUONIAOINC.'/'.$this->mainDicFile;
		$this->mainDicFileZip = HUONIAOINC.'/'.$this->mainDicFileZip;
	...
	function InportDict( $targetfile )
     {
     	...
     	require_once(HUONIAOINC.'/zip.class.php');
     	$zip = new zip();
     	//echo $targetfile;
     	$unpackagefile = array_keys($zip->Extract($targetfile,HUONIAOINC.'/data/'));
     	//exit();
     	$this->MakeDict(HUONIAOINC.'/data/'.$unpackagefile[0]);
     	unlink(HUONIAOINC.'/data/'.$unpackagefile[0]);
     	return true;
     }
}
