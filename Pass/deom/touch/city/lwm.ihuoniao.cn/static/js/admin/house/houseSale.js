	...
	huoniao.operaJson("houseSale.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < houseSale.length; i++){
				...
				list.push('  <td class="row5">'+houseSale[i].weight+'</td>');
				...
				list.push('  <td class="row12 left">'+community+'</td>');

				var type = houseSale[i].usertype == 0 ? "(个人)" : "(中介)";
				var username = "";
				if(houseSale[i].userid != 0){
					username = '<a href="javascript:;" data-id="'+houseSale[i].userid+'">'+houseSale[i].username+'</a>';
				}else{
				...
				list.push('  <td class="row15">'+houseSale[i].pubdate+'</td>');
