...
$(function(){
	...
	$("#btnSubmit").bind("click", function(event){
		...
		if(!init.regex(price)){
			huoniao.goTop();
			$.dialog.alert("请正确输入报价，只能填写数字，支持两位小数！");
			return false;
		};

		if(!init.regex(unitprice)){
			huoniao.goTop();
			$.dialog.alert("请正确输入单价，只能填写数字，支持两位小数！");
			return false;
		};
		...
		//异步提交
		huoniao.operaJson("houseSaleAdd.php", $("#editform").serialize() + "&submit="+encodeURI("提交"), function(data){
			if(data.state == 100){
				if($("#dopost").val() == "save"){
