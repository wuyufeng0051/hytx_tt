	...
	huoniao.operaJson("houseSp.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < houseSp.length; i++){
				...
				list.push('  <td class="row37 left">'+img+'<span><a href="'+houseSp[i].url+'" target="_blank">'+houseSp[i].title+'</a><span class="append">'+append+'</span></span></td>');
				list.push('  <td class="row5">'+houseSp[i].weight+'</td>');

				var type = houseSp[i].usertype == 0 ? "(个人)" : "(中介)";
				var username = "";
				if(houseSp[i].userid != 0){
					username = '<a href="javascript:;" data-id="'+houseSp[i].userid+'">'+houseSp[i].username+'</a>';
				}else{
