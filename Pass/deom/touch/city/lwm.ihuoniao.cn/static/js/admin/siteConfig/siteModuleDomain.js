$(function(){
	...
	var isOper = false, basehost = cfg_basehost.replace("http://", "");
	...
	if(moduleArr.length > 0){
		...
		for(var i = 0; i < moduleArr.length; i++){
			...
			list.push('<td class="row10 left">'+getSelect(moduleArr[i].type)+'</td>');
			list.push('<td class="row25 left">'+getInput(moduleArr[i].type, moduleArr[i].domain)+'</td>');
			list.push('<td class="row50 left"><a href="javascript:;" class="link save">保存</a></td>');
			list.push('</tr>');
		}
