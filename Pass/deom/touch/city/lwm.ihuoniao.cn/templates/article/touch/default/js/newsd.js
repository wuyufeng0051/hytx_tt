$(function(){
    ...
    $(window).scroll(function(){
        var box = $('.recommend_moudule');
        if(box.hasClass('disabled') || loadmoreLock) return;
        var sct = $(window).scrollTop();
        if(sct + $(window).height() + 50 > $('.loadmore').offset().top) {
            box.find('.error').remove();
            $('.loading').css('display','inline-block');
            loadmoreLock = true;
            var url = masterDomain + "/include/ajax.php?service=article&action=alist&group_img=1&flag=r&page=" + page + "&pageSize=" + pageSize + "&orderby=1";
            $.ajax({
                url: url,
                type: "GET",
                dataType: "jsonp",
                    ...
                    if(data && data.state != 200){
                        if(data.state == 101){
                            ...
                            for(var i = 0; i < list.length; i++){
                                ...
                                var time = returnHumanTime(pubdate,1);

                                item.push('<dl class="clearfix">');
                                    item.push('<a href="' + url + '">');
                                        if(img !== undefined && img != ''){
                                            item.push('<dt>')
                                                item.push('<img src="'+staticPath+'images/blank.gif" onerror="this.src=\''+litpic+'\'" data-url="' + img + '" alt="">')
                                            item.push('</dt>')
                                        }
                                        item.push('<dd>')
                                            item.push('<span class="m_news_bt">' + title + '</span>')
                                            item.push('<div class="info clearfix">')
                                                item.push('<time>' + time + '</time>')
                                                item.push('<div class="count"><i class="icon-pl2"></i>' + common + '</div>')
                                        item.push('</dd>')
                                    item.push('</a>');
                                item.push('</dl>');
                            ...
                            if($('.recommend_moudule dl').length >= maxSize || list.length < pageSize) {
    ...
    $("#dashang").bind("click", function(){
        ...
        //非客户端下验证支付类型
        if(appInfo.device == ""){
            if (paytype == "wxpay" && !navigator.userAgent.toLowerCase().match(/micromessenger/)) {
