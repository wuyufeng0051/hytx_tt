				...
				success: function (data) {
					if(data){
						if(data.state == 100){
							...
							$('.sp .choose-local-second').css('width', '60.72%');
					...
					if(data.state == 100){
						...
						if(list.length > 0){
							for(var i = 0; i < list.length; i++){
								...
								html.push('<div class="house-img l"><img src="'+pic+'"></div>');
								html.push('<dl class="l">');

								html.push('<dt>'+list[i].title+'</dt>');

								html.push('<dd class="item-area"><em>'+list[i].addr[0]+'-'+list[i].addr[1]+'-'+list[i].address+'</em></dd>');
								html.push('<dd class="item-type-1"><em>'+list[i].protype+'</em><em>面积：'+list[i].area+'㎡</em><em>'+list[i].bno+'/'+list[i].floor+'层</em></em></dd>');
								html.push('<dd class="item-type-2"><span>'+list[i].zhuangxiu+'</span></dd>');
								html.push('</dl>')
								html.push('</div>')
