$(function(){
		...
		$.ajax({
			...
			success: function (data) {
				if(data && data.state == 100){
					...
					if(!selected){
						$("#industry").val(0);
						$("#industry").closest("dd").find(".tip-inline").removeClass().addClass("tip-inline error").html("<s></s>请选择经营行业");
					}
	...
	$("#submit").bind("click", function(event){
		...
		if($.trim(nature.val()) == "" || nature.val() == 0){
			nature.siblings(".tip-inline").removeClass().addClass("tip-inline error").html("<s></s>请选择公司性质");
			offsetTop = offsetTop == 0 ? $("#selNature").position().top : offsetTop;
		}

		//规模
		if($.trim(scale.val()) == "" || scale.val() == 0){
			scale.siblings(".tip-inline").removeClass().addClass("tip-inline error").html("<s></s>请选择公司规模");
			offsetTop = offsetTop == 0 ? $("#selScale").position().top : offsetTop;
		}

		//行业
		if($.trim(industry.val()) == "" || industry.val() == 0){
			industry.siblings(".tip-inline").removeClass().addClass("tip-inline error").html("<s></s>请选择经营行业");
		...
		if($.trim(contact.val()) == "" || contact.val() == 0){
			contact.siblings(".tip-inline").removeClass().addClass("tip-inline error").html("<s></s>请输入联系电话");
