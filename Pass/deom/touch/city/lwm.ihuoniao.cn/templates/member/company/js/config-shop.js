$(function(){
	...
	$("#selIndustry").delegate("a", "click", function(){
		if($(this).text() != "请选择" && $(this).attr("data-id") != $("#industry").val()){
	...
	$("#submit").bind("click", function(event){
		...
		if($.trim(industry.val()) == "" || industry.val() == 0){
			industry.siblings(".tip-inline").removeClass().addClass("tip-inline error").html("<s></s>请选择所属行业");
		...
		if($.trim(company.val()) == "" || company.val() == 0){
			company.siblings(".tip-inline").removeClass().addClass("tip-inline error").html("<s></s>请输入公司名称");
		...
		if($.trim(referred.val()) == "" || referred.val() == 0){
			referred.siblings(".tip-inline").removeClass().addClass("tip-inline error").html("<s></s>请输入店铺简称");
