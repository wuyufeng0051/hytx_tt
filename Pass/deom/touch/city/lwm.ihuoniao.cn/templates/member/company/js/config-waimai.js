$(function(){
	...
	//开启满减
	var temp = '<div class="input-append input-prepend fn-clear"><span class="add-bef">满</span><input type="text" class="inp j1" name="m1[]" size="5" maxlength="5"><span class="add-aft">减</span><input type="text" class="inp j2" name="m2[]" size="5" maxlength="5"><span class="add-aft"><a href="javascript:;" class="del">删</a></span></div>';
	...
	$("#submit").bind("click", function(event){
		...
		if($.trim(title.val()) == "" || title.val() == 0){
			title.siblings(".tip-inline").removeClass().addClass("tip-inline error").html("<s></s>请输入商铺名称");
		...
		if($("#typeObj input:checked").length == 0){
			...
			dl.find(".tip-inline").removeClass().addClass("tip-inline error").html("<s></s>请选择经营类别");
