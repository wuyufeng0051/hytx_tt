...
$(function(){
	...
	$("#startdate").focus(function(){
		selectDate("startdate", function(dp){
			...
			if(edate.val() != ""){
				...
				if(enDate < snDate){
					edate.val("");
					$("#startdate").closest("dd").find(".tip-inline").removeClass().addClass("tip-inline error").html("<s></s>请选择结束时间");
				}else{
	...
	$("#enddate").focus(function(){
		selectDate("enddate", function(dp){
			...
			if(sdate.val() != ""){
				...
				if(enDate < snDate){
					$("#enddate").closest("dd").find(".tip-inline").removeClass().addClass("tip-inline error").html("<s></s>结束时间必须大于开始时间");
				}else{
					$("#startdate").closest("dd").find(".tip-inline").removeClass().addClass("tip-inline success").html("<s></s>");
				}
			}
	...
	//新增购买须知
	var noticeHtml = '<div class="notice-item fn-hide"><div class="label"><input type="text" placeholder="标题" /></div><div class="dd"><textarea placeholder="内容"></textarea></div><span class="btn move" title="移动"><i></i></span><span class="btn del" title="删除"><i></i></span><span class="btn add" title="添加"><i></i></span></div>';
	...
	$("#notice").delegate(".del", "click", function(){
		...
		if(val1 == "" && val2 == ""){
			...
			$.dialog.confirm("删除之后将无法恢复，确定要删除吗？", function(){
				t.slideUp(300, function(){
					t.remove();
				});
			});
	...
	var packtypeVal = $("input[name=packtype]:checked").val(),
			...
			m = 0,
			manyHtml = function(){
				return '<table class="tab tab'+m+'"><tr><td class="mtit"><input type="text" value="内容" /></td><td class="items"><table><thead><tr><th width="38%"align="left"><a href="javascript:;" class="btn remove" title="删除"><i></i></a>名称</th><th width="15%">单价</th><th width="15%">数量/规格</th><th width="15%">小计</th><th width="17%"style="text-align:center;">操作</th></tr></thead><tbody><tr><td><input type="text"class="tit"/></td><td><input type="text"class="pric"/></td><td><input type="text"class="coun"/></td><td><input type="text"class="tot"/></td><td><span class="btn move"title="移动"><i></i></span><span class="btn del"title="删除"><i></i></span><span title="添加"class="btn add"><i></i></span></td></tr><tr><td><input type="text"class="tit"/></td><td><input type="text"class="pric"/></td><td><input type="text"class="coun"/></td><td><input type="text"class="tot"/></td><td><span class="btn move"title="移动"><i></i></span><span class="btn del"title="删除"><i></i></span><span title="添加"class="btn add"><i></i></span></td></tr><tr><td><input type="text"class="tit"/></td><td><input type="text"class="pric"/></td><td><input type="text"class="coun"/></td><td><input type="text"class="tot"/></td><td><span class="btn move"title="移动"><i></i></span><span class="btn del"title="删除"><i></i></span><span title="添加"class="btn add"><i></i></span></td></tr></tbody></table></td></tr></table>';
			};
	...
	$(".taocon").delegate(".add", "click", function(){
		var t = $(this);
		t.closest("tr").after('<tr><td><input type="text" class="tit"></td><td><input type="text" class="pric"></td><td><input type="text" class="coun"></td><td><input type="text" class="tot"></td><td><span class="btn move" title="移动"><i></i></span><span class="btn del" title="删除"><i></i></span><span title="添加" class="btn add"><i></i></span></td></tr>');
	});
	...
	var regex = {
		...
		//副标题
		,subtitle: function(){
			return this.regexp($("#subtitle"), ".{5,100}", "输入错误，请正确填写，5-100个字！");
		}

		//库存
		,maxnum: function(){
			return this.regexp($("#maxnum"), "[1-9]\\d*", "输入错误，请输入大于0的数字！");
		}

		//购买限制
		,limit: function(){
			return this.regexp($("#limit"), "[1-9]\\d*", "输入错误，请输入大于0的数字！");
		}

		//市场价
		,market: function(){
			return this.regexp($("#market"), "(?!0+(?:.0+)?$)(?:[1-9]\\d*|0)(?:.\\d{1,2})?", "请填写正确的价格，必须为整数，且不能为0，最多支持两位小数点！");
		}

		//团购价
		,price: function(){
			return this.regexp($("#price_"), "(?!0+(?:.0+)?$)(?:[1-9]\\d*|0)(?:.\\d{1,2})?", "请填写正确的价格，必须为整数，且不能为0，最多支持两位小数点！");
		}
	...
	$("#submit").bind("click", function(event){
		...
		if((startdate == "" || enddate == "") && offsetTop <= 0){
			...
			$("#startdate").closest("dd").find(".tip-inline").removeClass().addClass("tip-inline error").html("<s></s>请选择团购时间");
		}else if(offsetTop <= 0){
			var nsdate = huoniao.transToTimes(startdate);
			var nedate = huoniao.transToTimes(enddate);
			if(nedate < nsdate){
				offsetTop = $("#startdate").position().top;
				$("#startdate").closest("dd").find(".tip-inline").removeClass().addClass("tip-inline error").html("<s></s>结束时间必须大于开始时间");
			}
		}
