$(function(){
	...
	$("#amount").bind("blur", function(){
		...
		if (!re.test(val)) {
			...
			$(".verifyAmount dd").html('<strong>'+huoniao.number_format(val, 2)+'</strong>元（可获得 '+Math.round(val*pointRatio)+' '+pointName+'）');
			$(".verifyAmount").show();
	...
	// 		var money = 5869.61;
	// 		$(".verifyAmount dd").html('<strong>'+huoniao.number_format(money, 2)+'</strong>元（可获得 '+(money*pointRatio)+' '+pointName+'）');
	// 		$(".verifyAmount").show();
