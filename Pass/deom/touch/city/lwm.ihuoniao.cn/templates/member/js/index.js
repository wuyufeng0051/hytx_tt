...
$(function(){
	...
	$("#customBanner").bind("click", function(){
		...
		html.push('<h5>自定义封面图片</h5>');
		html.push('<a href="javascript:;" class="close" title="关闭">关闭</a>');
		html.push('</div>');
		html.push('<div class="cont"><p class="loading"><img src="'+staticPath+'images/ajax-loader.gif" />加载中，请稍候...</p></div>');
		html.push('<div class="foot"><input type="button" value="保存" class="save" /><input type="button" value="取消" class="cancel" /></div>');
		html.push('</div>');
...
function getList(){
	...
	banner.find(".custom-cover .cover-list").html('<p class="loading"><img src="'+staticPath+'images/ajax-loader.gif" />加载中，请稍候...</p>');
		...
		success: function (data) {
			if(data && data.state != 200){
				...
				if(loadType){
					...
					typeHtml.push('<li class="curr" data-id="rec"><a href="javascript:;">推荐</a></li>');
