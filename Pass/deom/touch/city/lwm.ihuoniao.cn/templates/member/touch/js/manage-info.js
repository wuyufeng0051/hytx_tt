		...
		success: function (data) {
          ...
          $('.count span').text(0);
                  ...
                  isvalid     = list[i].isvalid,
							...
							html.push('<dd class="item-area"><em>区域：'+address+'</em></dd>');
							html.push('<dd class="item-type-1"><em> 浏览：'+click+'次 </em><em>评论：'+common+'条</em></dd>');
              ...
              if(list[i].arcrank == "1" && !isvalid){
								if(isbid == 1){
    							html.push('<a href="javascript:;" class="bid">预算：'+bid_price+'，加价</a>');
                }else{
                  html.push('<a href="javascript:;" class="bid">竞价</a>');
                }
              }
							html.push('</div>');
							html.push('</div>');
