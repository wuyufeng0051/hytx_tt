...
shopInit = {
		...
		if(!ishas){
			var li = $('<li data-id="'+data.id+'" data-specation="'+data.specation+'" data-count="'+data.count+'" class="fn-hide">'+
						...
						'<div class="info">'+
							...
							'<p><span><strong>&yen;</strong> × <strong class="c">'+data.count+'</strong></span><a href="javascript:;" class="del">删除</a></p>'+
						'</div>'+
					'</li>');
			cartul.append(li);
			li.slideDown();
	...
	,list: function(){
		...
		var thi = this;
		var cartData = $.cookie(cookiePre+"shop_cart");
		if(cartData == null || cartData == ""){
			...
			//异步获取信息详细
			$.ajax({
				url: masterDomain+'/include/ajax.php?service=shop&action=getCartList',
				type: "GET",
				dataType: "jsonp",
