...
if($dopost == "save" && $submit == "提交"){
	//保存到表
	$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`title`, `domaintype`, `nature`, `scale`, `industry`, `logo`, `userid`, `people`, `contact`, `addrid`, `address`, `lnglat`, `postcode`, `email`, `site`, `body`, `pics`, `weight`, `state`, `property`, `seotitle`, `keywords`, `description`, `pubdate`) VALUES ('$title', '$domaintype', '$nature', '$scale', '$industry', '$litpic', '$userid', '$people', '$contact', '$addrid', '$address', '$lnglat', '$postcode', '$email', '$site', '$body', '$pics', '$weight', '$state', '$property', '$seotitle', '$keywords', '$description', '".GetMkTime(time())."')");
	//echo $archives;die;
	...
	if($submit == "提交"){
		//保存到表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `title` = '$title', `domaintype` = '$domaintype', `nature` = '$nature', `scale` = '$scale', `industry` = '$industry', `logo` = '$litpic', `userid` = '$userid', `people` = '$people', `contact` = '$contact', `addrid` = '$addrid', `address` = '$address', `lnglat` = '$lnglat', `postcode` = '$postcode', `email` = '$email', `site` = '$site', `body` = '$body', `pics` = '$pics', `weight` = '$weight', `state` = '$state', `property` = '$property', `seotitle` = '$seotitle', `keywords` = '$keywords', `description` = '$description' WHERE `id` = ".$id);
