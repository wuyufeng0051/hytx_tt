...
if($dopost == "getList"){
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			$typeSql = $dsql->SetQuery("SELECT `typename` FROM `#@__jobaddr` WHERE `id` = ". $value["addr"]);
...
//新增
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`title`, `people`, `mobile`, `tel`, `fax`, `addr`, `address`, `lnglat`, `email`, `qq`, `note`, `traffic`, `pics`, `pubdate`) VALUES ('$title', '$people', '$mobile', '$tel', '$fax', '$addr', '$address', '$lnglat', '$email', '$qq', '$note', '$traffic', '$pics', '".GetMkTime(time())."')");
...
//修改
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `title` = '$title', `people` = '$people', `mobile` = '$mobile', `tel` = '$tel', `fax` = '$fax', `addr` = '$addr', `address` = '$address', `lnglat` = '$lnglat', `email` = '$email', `qq` = '$qq', `note` = '$note', `traffic` = '$traffic', `pics` = '$pics' WHERE `id` = ".$id);
