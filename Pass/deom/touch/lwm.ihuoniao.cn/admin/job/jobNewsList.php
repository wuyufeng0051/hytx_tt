 ...
 * 招聘资讯
 *
 * @version        $Id: jobNews.php 2014-01-18 下午23:44:11 $
 * @package        HuoNiao.job
...
if($dopost != ""){
	...
	$jsFile = array(
		...
		'publicUpload.js',
		'ui/bootstrap.min.js',
		'admin/job/jobNewsAdd.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}else{
	$templates = "jobNewsList.html";

	//js
	$jsFile = array(
		'ui/bootstrap.min.js',
		'ui/jquery-ui-selectable.js',
		'admin/job/jobNewsList.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}
...
if($dopost == "getList"){
	...
	if(count($results) > 0){
		...
		if(count($list) > 0){
			echo '{"state": 100, "info": '.json_encode("获取成功").', "pageInfo": {"totalPage": '.$totalPage.', "totalCount": '.$totalCount.'}, "jobNewsList": '.json_encode($list).'}';
		}else{
...
//新增
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`title`, `typeid`, `litpic`, `click`, `weight`, `source`, `sourceUrl`, `writer`, `keyword`, `description`, `body`, `arcrank`, `pubdate`) VALUES ('$title', '$typeid', '$litpic', '$click', '$weight', '$source', '$sourceUrl', '$writer', '$keyword', '$description', '$body', '$arcrank', '$pubdate')");
...
//修改
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `title` = '$title', `typeid` = $typeid, `litpic` = '$litpic', `click` = '$click', `weight` = '$weight', `source` = '$source', `sourceUrl` = '$sourceUrl', `writer` = '$writer', `keyword` = '$keyword', `description` = '$description', `body` = '$body', `arcrank` = '$arcrank' WHERE `id` = ".$id);
	...
	if($id != ""){
		...
		foreach($each as $val){
			...
			//删除缩略图
			delPicFile($results[0]['litpic'], "delThumb", "job");
