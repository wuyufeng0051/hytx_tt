...
if($dopost == "getList"){
	...
	$archives = $dsql->SetQuery("SELECT `id`, `title`, `people`, `contact`, `state`, `pubdate` FROM `#@__".$tab."` WHERE `type` = ".$type.$where);
...
//更新状态
	...
	if(!testPurview("jobSentenceEdit")){
		die('{"state": 200, "info": '.json_encode("对不起，您无权使用此功能！").'}');
	};
	$each = explode(",", $id);
	$error = array();
