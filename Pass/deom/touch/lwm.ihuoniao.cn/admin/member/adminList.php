...
if($dopost == "del"){
	...
	if($id != ""){
		foreach($each as $val){
			$sql = $dsql->SetQuery("SELECT `username` FROM `#@__".$db."` WHERE `id` = ".$val);
...
//获取管理员列表
	...
	$archives = $dsql->SetQuery("SELECT `id`, `username`, `nickname`, `regtime`, `mgroupid`, `state` FROM `#@__".$db."` WHERE 1 = 1".$where);
