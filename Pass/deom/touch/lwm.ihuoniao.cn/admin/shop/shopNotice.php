 ...
 * 商城公告
 *
 * @version        $Id: shopNotice.php 2014-1-24 下午14:29:12 $
...
if($dopost != ""){
	...
	$templates = "shopNotice.html";

	//js
	$jsFile = array(
		'ui/bootstrap.min.js',
		'ui/jquery-ui-selectable.js',
		'admin/shop/shopNotice.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}
$tab = 'shop_noticelist';
...
if($dopost == "getList"){
	...
	$archives = $dsql->SetQuery("SELECT `id`, `title`, `color`, `weight`, `arcrank`, `pubdate` FROM `#@__".$tab."` WHERE 1 = 1".$where);
...
//新增公告
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`title`, `color`, `redirecturl`, `litpic`, `weight`, `body`, `arcrank`, `pubdate`) VALUES ('$title', '$color', '$redirecturl', '$litpic', $weight, '$body', $arcrank, $pubdate)");
...
//修改公告
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `title` = '$title', `color` = '$color', `redirecturl` = '$redirecturl', `litpic` = '$litpic', `weight` = '$weight', `body` = '$body', `arcrank` = '$arcrank' WHERE `id` = ".$id);
		$return = $dsql->dsqlOper($archives, "update");
