...
if($dopost == "getList"){
	...
	$archives = $dsql->SetQuery("SELECT `id`, `type`, `sitename`, `sitelink`, `litpic`, `weight`, `arcrank` FROM `#@__".$tab."list` WHERE `module` = '$action'".$where);
...
//新增
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."list` (`module`, `type`, `sitename`, `sitelink`, `litpic`, `weight`, `arcrank`, `pubdate`) VALUES ('$action', $typeid, '$sitename', '$sitelink', '$litpic', $weight, $arcrank, $pubdate)");
...
//修改
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."list` SET `type` = $typeid, `sitename` = '$sitename', `sitelink` = '$sitelink', `litpic` = '$litpic', `weight` = $weight, `arcrank` = '$arcrank' WHERE `id` = ".$id);
		$return = $dsql->dsqlOper($archives, "update");
...
if(file_exists($tpl."/".$templates)){
	...
	$huoniaoTag->assign('typeListArr', json_encode(getTypeList(0, $action, $tab."type")));
