...
if($action != "" || $dopost != ""){
	...
	//启用短信帐号
		...
		if($dopost == "add"){
			...
			$archives = $dsql->SetQuery("INSERT INTO `#@__sitesms` (`title`, `username`, `password`, `signCode`, `charset`, `sendUrl`, `sendCode`, `accountUrl`, `accountCode`, `website`, `contact`, `state`) VALUES ('$title', '$username', '$password', '$signCode', '$charset', '$sendUrl', '$sendCode', '$accountUrl', '$accountCode', '$website', '$contact', '$state')");
			$results = $dsql->dsqlOper($archives, "update");
			...
			if($id !== ""){
				...
				$archives = $dsql->SetQuery("UPDATE `#@__sitesms` SET `title` = '$title', `username` = '$username', `password` = '$password', `signCode` = '$signCode', `charset` = '$charset', `sendUrl` = '$sendUrl', `sendCode` = '$sendCode', `accountUrl` = '$accountUrl', `accountCode` = '$accountCode', `website` = '$website', `contact` = '$contact' WHERE `id` = $id");
				$results = $dsql->dsqlOper($archives, "update");
