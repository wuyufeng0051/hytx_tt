 ...
 * 团购常见问题
 *
 * @version        $Id: tuanFaq.php 2013-12-5 下午20:59:19 $
...
if($dopost != ""){
	$templates = "tuanFaqAdd.html";
	
	//js
	$jsFile = array(
		'admin/tuan/tuanFaqAdd.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
	
}else{
	$templates = "tuanFaq.html";
	
	//js
	$jsFile = array(
		'ui/bootstrap.min.js',
		'ui/jquery-ui-selectable.js',
		'admin/tuan/tuanFaq.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}
...
if($dopost == "getList"){
	...
	if(count($results) > 0){
		...
		if(count($list) > 0){
			echo '{"state": 100, "info": '.json_encode("获取成功").', "pageInfo": {"totalPage": '.$totalPage.', "totalCount": '.$totalCount.'}, "tuanFaqList": '.json_encode($list).'}';
		}else{
...
if(file_exists($tpl."/".$templates)){
	...
	$huoniaoTag->assign('typeListArr', json_encode($dsql->getTypeList(0, "tuan_faqtype")));
