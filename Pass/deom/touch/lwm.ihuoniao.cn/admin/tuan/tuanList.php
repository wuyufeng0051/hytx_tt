...
if($dopost == "getList"){
	...
	if($sKeyword != ""){
		...
		$sidArr = array();
		$userSql = $dsql->SetQuery("SELECT `store`.id FROM `#@__tuan_store` store LEFT JOIN `#@__member` user ON `user`.id = `store`.uid WHERE `user`.company like '%$sKeyword%'");
	...
	if($sAddr != ""){
		...
		$addridArr = array();
		$archives = $dsql->SetQuery("SELECT `id` FROM `#@__".$action."_store` WHERE `addrid` in ($lower)");
		$results = $dsql->dsqlOper($archives, "results");
		foreach ($results as $key => $value) {
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			//分类
			$userSql = $dsql->SetQuery("SELECT `stype` FROM `#@__tuan_store` WHERE `id` = ".$value['sid']);
