...
$sql = $dsql->SetQuery("SELECT c.`id`, c.`name`, c.`username`, c.`phone`, c.`age`, c.`sex`, c.`photo`, c.`lng`, c.`lat`, (SELECT count(`id`) FROM `#@__waimai_order` WHERE `peisongid` = c.`id`) as total, (SELECT count(`id`) FROM `#@__waimai_order` WHERE `peisongid` = c.`id` AND `state` = 1) as ok, (SELECT count(`id`) FROM `#@__waimai_order` WHERE `peisongid` = c.`id` AND (`state` = 6 OR `state` = 7)) as failed, c.`state`  FROM `#@__$dbname` c ORDER BY c.`id` ASC");
