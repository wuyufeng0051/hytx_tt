...
function house($params, $content = "", &$smarty = array(), &$repeat = array()){
	...
	if($action == "loupan"){
		...
		if($addrid == 0 && $business != 0){
			$sql = $dsql->SetQuery("SELECT `parentid` FROM `#@__houseaddr` WHERE `id` = ".$business);
			$ret = $dsql->dsqlOper($sql, "results");
			if($ret){
		...
		if(!empty($addrid)){
			global $data;
			$data = "";
			$addrArr = getParentArr("houseaddr", $addrid);
		...
		if(!empty($typeid)){
			...
			if($res){
				$loupan_seotitle .= $res[0]['typename'];
			}
		}
	...
	//小区列表
		...
		if(!empty($typeid)){
			...
			$res = $dsql->dsqlOper($sql, "results");
			if($res){
				$community_seotitle .= $res[0]['typename'];
			}
		}
		$huoniaoTag->assign('typeid', $typeid);
	...
	//二手房列表
		...
		if(!empty($direction)){
			...
			$res = $dsql->dsqlOper($sql, "results");
			if($res){
				$sale_seotitle .= "朝".$res[0]['typename'];
				$huoniaoTag->assign('directionName', "朝".$res[0]['typename']);
			}
		}
		...
		if(!empty($zhuangxiu)){
			...
			$res = $dsql->dsqlOper($sql, "results");
			if($res){
				$sale_seotitle .= $res[0]['typename'];
				$huoniaoTag->assign('zhuangxiuName', $res[0]['typename']);
			}
		}
		$huoniaoTag->assign('zhuangxiu', $zhuangxiu);
	...
	//出租房列表
		...
		if(!empty($price)){
			$priceArr = explode(",", $price);
			if(empty($priceArr[0])){
				$zu_seotitle .= $priceArr[1] . "00元以下";
			}elseif(empty($priceArr[1])){
				$zu_seotitle .= $priceArr[0] . "00元以上";
			}elseif(!empty($priceArr[0]) && !empty($priceArr[1])){
				$zu_seotitle .= $priceArr[0] . "00-" . $priceArr[1] . "00元";
			}
		}
		$huoniaoTag->assign('price', $price);
	...
	//写字楼列表
		...
		if(!empty($protype)){
			...
			$res = $dsql->dsqlOper($sql, "results");
			if($res){
				$xzl_seotitle .= $res[0]['typename'];
				$huoniaoTag->assign('protypeName', $res[0]['typename']);
			}
		}
	...
	//商铺列表
		...
		if(!empty($industry)){
			...
			if($res){
				$sp_seotitle .= $res[0]['typename'];
				$huoniaoTag->assign('industryName', $res[0]['typename']);
			}
		}
