...
class shop {
	...
	public function config(){
		...
		$customChannelDomain = $shopDomain;
	...
	public function addr(){
		...
		$results = $dsql->getTypeList($type, "shopaddr", $son, $page, $pageSize);
		if($results){
			return $results;
		}
	}
	...
	public function notice(){
		...
		$archives = $dsql->SetQuery("SELECT `id`, `title`, `color`, `redirecturl`, `litpic` FROM `#@__shop_noticelist` WHERE `arcrank` = 0 ORDER BY `weight` DESC, `id` DESC");
		//总条数
	...
	public function news(){
		...
		$archives = $dsql->SetQuery("SELECT `id`, `title`, `litpic`, `body`, `pubdate` FROM `#@__shop_news` WHERE `arcrank` = 0".$where." ORDER BY `weight` DESC, `id` DESC");
		//总条数
		$totalCount = $dsql->dsqlOper($archives, "totalCount");
		//总分页数
		$totalPage = ceil($totalCount/$pageSize);
	...
	public function newsDetail(){
		...
		$archives = $dsql->SetQuery("SELECT `title`, `typeid`, `litpic`, `body`, `pubdate` FROM `#@__shop_news` WHERE `arcrank` = 0 AND `id` = ".$this->param);
		$results  = $dsql->dsqlOper($archives, "results");
		if($results){
	...
	public function store(){
		...
		if(!empty($addrid)){
			if($dsql->getTypeList($addrid, "shopaddr")){
				global $arr_data;
				$arr_data = array();
				$lower = arr_foreach($dsql->getTypeList($addrid, "shopaddr"));
		...
		$archives = $dsql->SetQuery("SELECT `id`, `title`, `domaintype`, `company`, `referred`, `addrid`, `address`, `industry`, `project`, `logo`, `userid`, `tel`, `qq`, `click`, `certi`, `rec`, `pubdate` FROM `#@__shop_store` ".$where.$order);
		...
		if($results){
			foreach($results as $key => $val){
				...
				global $data;
				$data = "";
				$addrArr = getParentArr("shopaddr", $val['addrid']);
	...
	public function storeDetail(){
		...
		if($results){
			...
			global $data;
			$data = "";
			$addrName = getParentArr("shopaddr", $results[0]['addrid']);
	...
	public function slist(){
		...
		if(!empty($limited)){
			...
			//未开始
			if($limited == "1"){
				$where .= " AND find_in_set(3, `flag`) AND `btime` > $now";

			//进行中
			}elseif($limited == "2"){
				$where .= " AND find_in_set(3, `flag`) AND `btime` <= $now AND `etime` > $now";

			//已结束
			}elseif($limited == "3"){
				$where .= " AND find_in_set(3, `flag`) AND `etime` < $now";
		...
		//正常情况下不显示未开始和已结束的商品
			...
			$now = GetMkTime(time());
			$where .= " AND ((`btime` <= $now AND `etime` > $now) OR (`btime` = 0 AND `etime` = 0))";
		...
		if(!empty($item)){
			...
			foreach ($item_ as $key => $value) {
				$val = explode(":", $value);
				$itemArr[] = "`property` like '%".$val[0]."#".$val[1]."%'";
			}
			if(!empty($itemArr)){
		...
		if(!empty($specification)){
			...
			foreach($speList as $k => $v){
				$speArr[] = "`specification` like '%".$v.",%' OR `specification` like '%".$v."-%'";
			}
			if(!empty($speArr)){
		...
		if($store != ""){
			...
			if(!empty($storetype)){
				...
				foreach ($sids as $key => $value) {
					array_push($st, "find_in_set(".$value.", `category`)");
				}
				$where .= " AND `store` = ".$store." AND (".join(" OR ", $st).")";
			}else{
		...
		switch ($orderby){
			...
			//销量降序
			case 1:
				$orderby = " ORDER BY `sales` DESC, `sort` DESC, `id` DESC";
				break;
			//销量升序
			case 2:
				$orderby = " ORDER BY `sales` ASC, `sort` DESC, `id` DESC";
				break;
			//价格升序
			case 3:
			...
			//时间降序
			case 5:
				$orderby = " ORDER BY `pubdate` DESC, `sort` DESC, `id` DESC";
				break;
			//人气降序
			case 6:
				$orderby = " ORDER BY `click` DESC, `sort` DESC, `id` DESC";
				break;
			default:
				$orderby = " ORDER BY `sort` DESC, `id` DESC";
				break;
		...
		$archives = $dsql->SetQuery("SELECT " .
									"`id`, `type`, `title`, `store`, `mprice`, `price`, `sales`, `inventory`, `litpic`, `flag`, `btime`, `etime`, `state`, `pubdate`, `specification` " .
									"FROM `#@__shop_product` " .
									"WHERE " .
									"1 = 1".$where);
		...
		if(!empty($cartData)){
			...
			foreach ($cartData as $key => $value) {
				...
				if($detail){
					...
					$param = array("url" => $detail['litpic'], "type" => "small");
	...
	public function confirm_order(){
		...
		foreach($pros as $key => $value){
			...
			if(!is_array($detail)) return array("state" => 200, "info" => '购物车中含有不存在的商品，请刷新页面重试！');
			...
			if(strpos($detail['flag'], '3') !== false){
				if($detail['btime'] > time()) return array("state" => 200, "info" => '【'.$detail['title'].'  '.join("；", $speInfo).'】活动还未开始');
				if($detail['etime'] < time()) return array("state" => 200, "info" => '【'.$detail['title'].'  '.join("；", $speInfo).'】活动已经结束');
			}
			...
			if(($detail['limit'] < $pro[2] && $detail['limit'] != 0) || $inventor < $pro[2]) return array("state" => 200, "info" => '【'.$detail['title'].'  '.join("；", $speInfo).'】库存不足');
	...
	public function checkPayAmount(){
		...
		if(empty($pros) && empty($ordernum)) return array("state" => 200, "info" => "提交失败，商品信息提交失败！");
		...
		if(!empty($ordernum)){
			...
			if($ret){
				...
				return array("state" => 200, "info" => "订单提交失败！");
			}
		}
		...
		if($useBalance == 1 && !empty($balance) && empty($paypwd)) return array("state" => 200, "info" => "请输入支付密码！");
	...
	public function payCheck(){
		...
		if(empty($pros)) return array("state" => 200, "info" => "商品信息传递失败！");
		...
		foreach ($proArr as $key => $value) {
			...
			$detail = $this->detail();
			if(!is_array($detail)){
				$info = count($proArr) > 1 ? "订单中包含不存在或已下架的商品，请确认后重试！" : "提交失败，您要购买的商品不存在或已下架！";
			...
			if(($detail['limit'] < $proInfo[2] && $detail['limit'] != 0) || $inventor < $proInfo[2]) return array("state" => 200, "info" => '【'.$detail['title'].'  '.join("；", $speInfo).'】库存不足');
	...
	public function pay(){
		...
		if(empty($pros)) return array("state" => 200, "info" => '格式错误');
		...
		if(!empty($ordernum)){
			...
			foreach ($orderList as $key => $value) {
				//更新主表
				if(empty($addressid)){
					$sql = $dsql->SetQuery("UPDATE `#@__shop_order` SET `paytype` = '$paytype'");
				}else{
					$sql = $dsql->SetQuery("UPDATE `#@__shop_order` SET `paytype` = '$paytype', `people` = '$person', `address` = '$address', `contact` = '$contact', `note` = '$note'");
				}
		...
		//新订单
			...
			foreach ($orderList as $key => $value) {
				...
				$store = $value["store"];
				$no = $note[$store];
				$sql = $dsql->SetQuery("INSERT INTO `#@__shop_order` (`ordernum`, `store`, `userid`, `orderstate`, `orderdate`, `paytype`, `people`, `address`, `contact`, `note`) VALUES ('$newOrdernum', '$store', '$userid', 0, ".GetMkTime(time()).", '$paytype', '$person', '$address', '$contact', '$no')");
				...
				if($oid){
					...
					return array("state" => 200, "info" => '下单失败1');
				}

				if(!$tr){
					return array("state" => 200, "info" => '下单失败2');
				}
		...
		if(($usePinput && !empty($point)) || ($useBalance && !empty($balance))){
			...
			foreach ($opArr as $key => $value) {
				...
				$oprice = $value['price'] * $value['count'] + $value['logistic'];  //单个订单总价 = 单价 * 数量 + 物流费
		...
		//如果没有使用积分或余额，重置积分&余额等价格信息
			...
			foreach ($opArr as $key => $value) {
				$payprice = $value['price'] * $value['count'] + $value['logistic'];
		...
		if($payTotalAmount <= 0){
			...
			//跳转至第三方支付页面
			createPayForm("shop", join(",", $ordernumArr), $payTotalAmount, $paytype, "商城订单");
		}
	...
	public function paySuccess(){
		...
		if(!empty($param)){
			...
			foreach ($paramArr as $key => $value) {
				...
				//查询订单信息
				$archives = $dsql->SetQuery("SELECT o.`userid`, o.`paydate`, p.`proid`, p.`speid`, p.`count`, p.`point`, p.`balance`, p.`payprice` FROM `#@__shop_order` o LEFT JOIN `#@__shop_order_product` p ON o.`id` = p.`orderid` WHERE o.`ordernum` = '$value'");
				...
				if($res){
					...
					if($paydate == 0){
						...
						if(!empty($upoint) && $upoint > 0){
							...
							//保存操作日志
							$archives = $dsql->SetQuery("INSERT INTO `#@__member_point` (`userid`, `type`, `amount`, `info`, `date`) VALUES ('$userid', '0', '$upoint', '支付商城订单：$value', '$date')");
							$dsql->dsqlOper($archives, "update");
						}
						...
						if(!empty($ubalance) && $ubalance > 0){
							...
							//$archives = $dsql->SetQuery("INSERT INTO `#@__member_money` (`userid`, `type`, `amount`, `info`, `date`) VALUES ('$userid', '0', '$ubalance', '支付商城订单：$value', '$date')");
							//$dsql->dsqlOper($archives, "update");
						...
						//保存操作日志
						$archives = $dsql->SetQuery("INSERT INTO `#@__member_money` (`userid`, `type`, `amount`, `info`, `date`) VALUES ('$userid', '0', '$totalPrice', '商城消费冻结：$value', '$date')");
						$dsql->dsqlOper($archives, "update");
	...
	public function refund(){
		...
		//验证订单
		$archives = $dsql->SetQuery("SELECT o.`id` FROM `#@__shop_order` o WHERE o.`id` = '$id' AND o.`userid` = '$uid' AND (o.`orderstate` = 1 OR o.`orderstate` = 6 OR (o.`orderstate` = 2 AND o.`paydate` != 0)) AND o.`ret-state` = 0");
		$results  = $dsql->dsqlOper($archives, "results");
		if($results){
	...
	public function receipt(){
		...
		//验证订单
		$archives = $dsql->SetQuery("SELECT o.`ordernum`, o.`userid`, s.`userid` uid FROM `#@__shop_order` o LEFT JOIN `#@__shop_store` s ON s.`id` = o.`store` WHERE o.`id` = '$id' AND o.`userid` = '$uid' AND o.`orderstate` = 6");
		$results  = $dsql->dsqlOper($archives, "results");
		if($results){
			...
			if($ret){
				foreach($ret as $key => $val){
					$totalMoney += $val['price'] * $val['count'] + $val['logistic'] + $val['discount'];
			...
			if($totalMoney > 0){
				...
				//保存操作日志
				$now = GetMkTime(time());
				$archives = $dsql->SetQuery("INSERT INTO `#@__member_money` (`userid`, `type`, `amount`, `info`, `date`) VALUES ('$uid', '1', '$totalMoney', '商城订单：$ordernum', '$now')");
				$dsql->dsqlOper($archives, "update");
			}
	...
	public function delivery(){
		...
		//验证订单
		$archives = $dsql->SetQuery("SELECT o.`id` FROM `#@__shop_order` o LEFT JOIN `#@__shop_store` s ON o.`store` = s.`id` LEFT JOIN `#@__member` m ON s.`userid` = m.`id` WHERE o.`id` = '$id' AND m.`id` = '$uid' AND o.`orderstate` = 1");
		$results  = $dsql->dsqlOper($archives, "results");
		if($results){
	...
	public function refundPay(){
		...
		//验证订单
		$archives = $dsql->SetQuery("SELECT o.`id`, o.`ordernum`, o.`userid` FROM `#@__shop_order` o LEFT JOIN `#@__shop_store` s ON o.`store` = s.`id` LEFT JOIN `#@__member` m ON s.`userid` = m.`id` WHERE o.`id` = '$id' AND m.`id` = '$uid' AND o.`orderstate` = 6 AND o.`ret-state` = 1");
		$results  = $dsql->dsqlOper($archives, "results");
		if($results){
			...
			if(!empty($totalPoint)){
				...
				//保存操作日志
				$archives = $dsql->SetQuery("INSERT INTO `#@__member_point` (`userid`, `type`, `amount`, `info`, `date`) VALUES ('$userid', '1', '$totalPoint', '商城订单退回：$ordernum', '$now')");
				$dsql->dsqlOper($archives, "update");
			}
			...
			if($totalMoney > 0){
				...
				//保存操作日志
				$archives = $dsql->SetQuery("INSERT INTO `#@__member_money` (`userid`, `type`, `amount`, `info`, `date`) VALUES ('$userid', '1', '$totalMoney', '商城订单退回：$ordernum', '$now')");
				$dsql->dsqlOper($archives, "update");
	...
	public function refundReply(){
		...
		//验证订单
		$archives = $dsql->SetQuery("SELECT o.`id` FROM `#@__shop_order` o LEFT JOIN `#@__shop_store` s ON o.`store` = s.`id` LEFT JOIN `#@__member` m ON s.`userid` = m.`id` WHERE o.`id` = '$id' AND m.`id` = '$uid' AND o.`orderstate` = 6 AND o.`ret-state` = 1");
		$results  = $dsql->dsqlOper($archives, "results");
		if($results){
	...
	public function sendCommon(){
		...
		if(empty($orderid)) return array("state" => 200, "info" => '订单信息提交失败！');
		...
		if($orderDetail['orderstate'] == 3){
			...
			foreach($orderDetail['product'] as $key => $value){
				...
				if(empty($rat)) return array("state" => 200, "info" => '请选择商品评价！');
				if(empty($sco1)) return array("state" => 200, "info" => '请给商品描述打分');
				if(empty($sco2)) return array("state" => 200, "info" => '请给商家服务打分');
				if(empty($sco3)) return array("state" => 200, "info" => '请给商品质量打分');
				if(empty($not)) return array("state" => 200, "info" => '请输入评价内容！');
			...
			return array("state" => 200, "info" => '订单状态有误！');
		}
	...
	public function orderList(){
		...
		$archives = $dsql->SetQuery("SELECT " .
								"`id`, `ordernum`, `store`, `userid`, `orderstate`, `orderdate`, `paytype`, `ret-state`, `exp-date`, `common` " .
								"FROM `#@__shop_order` " .
								"WHERE".$where);
		...
		if(!empty($results)){
			...
			if($ret){
				...
				foreach ($ret as $key => $value) {
					...
					//如果是未支付的，不计算积分和余额
					$payprice = $results['orderstate'] == 0 ? $value['price'] * $value['count'] + $value['logistic'] + $value['discount'] : $value['payprice'];
	...
	public function storeConfig(){
		...
		if(empty($industry)){
			return array("state" => 200, "info" => '请选择所属行业');
		}
		...
		if(empty($referred)){
			return array("state" => 200, "info" => '请输入店铺简称');
		}
		...
		if(empty($logo)){
			return array("state" => 200, "info" => '请上传店铺LOGO');
		}
		...
		if(empty($tel)){
			return array("state" => 200, "info" => '请输入客服电话');
		}
		...
		if(!$userResult){
			...
			//保存到主表
			$archives = $dsql->SetQuery("INSERT INTO `#@__shop_store` (`userid`, `title`, `company`, `referred`, `addrid`, `address`, `industry`, `project`, `logo`, `people`, `contact`, `tel`, `qq`, `note`, `click`, `weight`, `state`, `certi`, `rec`, `pubdate`) VALUES ('$userid', '$title', '$company', '$referred', '$addrid', '$address', '$industry', '$project', '$logo', '$people', '$contact', '$tel', '$qq', '$body', '1', '1', '0', '0', '0', '$pubdate')");
		...
		//更新商铺信息
			...
			//保存到主表
			$archives = $dsql->SetQuery("UPDATE `#@__shop_store` SET `company` = '$company', `title` = '$title', `referred` = '$referred', `addrid` = '$addrid', `address` = '$address', `industry` = '$industry', `project` = '$project', `logo` = '$logo', `people` = '$people', `contact` = '$contact', `tel` = '$tel', `qq` = '$qq', `note` = '$body', `state` = 0 WHERE `userid` = ".$userid);
		...
		if($ret){
			...
			foreach ($ids as $key => $value) {
				...
				if($ret){
					...
					return array("state" => 200, "info" => '账号验证错误，删除失败！');
				}
	...
	public function updateCategory(){
		...
		if(!is_numeric($id)){
			return array("state" => 200, "info" => '更新失败，请重试！');
		}

		if(empty($typename)){
			return array("state" => 200, "info" => '请输入分类名称！');
		}
		...
		if($ret){
			...
			if($ret){
				...
				if($ret[0]['typename'] == $typename){
					return array("state" => 101, "info" => '没有变化');
				}else{
				...
				return array("state" => 200, "info" => '账号验证错误，更新失败！');
			}
	...
	public function logisticTemplate(){
		...
		if(!empty($id)){
			...
			if($results){
				...
				return array("state" => 101, "info" => '模板不存在！');
			}
		}
	...
	public function put(){
		...
		if($typeid == ""){
			return array("state" => 200, "info" => '分类获取失败，请重新选择分类！');
		}

		if($itemid == ""){
			return array("state" => 200, "info" => '分类属性ID获取失败，请重新选择分类！');
		}
		...
		if($shopResults){
			foreach($shopResults as $key => $val){
				...
				if(is_array($proval)){
					if($r && empty($proval)){
						return array("state" => 200, "info" => "请选择'.$typeName.'！");
					}
					if(!empty($proval)){
		...
		if($title == ""){
			return array("state" => 200, "info" => "请输入商品标题！");
		}

		$category = isset($category) ? join(',',$category) : '';

		if(!preg_match("/^0|\d*\.?\d+$/i", $mprice, $matches)){
			return array("state" => 200, "info" => "市场价不得为空，类型为数字！");
		}

		if(!preg_match("/^0|\d*\.?\d+$/i", $price, $matches)){
			return array("state" => 200, "info" => "一口价不得为空，类型为数字！");
		}

		if(empty($logistic)){
			return array("state" => 200, "info" => "请选择物流运费模板！");
		}
		...
		if(!empty($spearray)){
			...
			foreach($spearray as $key => $val){
				...
				if(!preg_match("/^0|\d*\.?\d+$/i", $spemprice, $matches)){
					return array("state" => 200, "info" => "规格表中价格不得为空，类型为数字！");
				}elseif(!preg_match("/^0|\d*\.?\d+$/i", $speprice, $matches)){
					return array("state" => 200, "info" => "规格表中库存不得为空，类型为数字！");
		...
		if(!empty($specifival)){
			...
			if(!preg_match("/^0|\d*\.?\d+$/i", $inventory, $matches)){
				return array("state" => 200, "info" => "库存不得为空，类型为数字！");
			}
		}
		...
		if(trim($body) == ''){
			return array("state" => 200, "info" => '请输入商品描述');
		}

		//保存到主表
		$archives = $dsql->SetQuery("INSERT INTO `#@__shop_product` (`type`, `title`, `brand`, `property`, `store`, `category`, `mprice`, `price`, `logistic`, `volume`, `weight`, `specification`, `inventory`, `limit`, `sales`, `litpic`, `sort`, `click`, `state`, `pics`, `body`, `mbody`, `pubdate`) VALUES ('$typeid', '$title', '$brand', '$property', '$sid', '$category', '$mprice', '$price', '$logistic', '$volume', '$weight', '$specifival', '$inventory', '$limit', '0', '$litpic', '1', '1', '0', '$imglist', '$body', '$body', '$pubdate')");
		$aid = $dsql->dsqlOper($archives, "lastid");
	...
	public function edit(){
		...
		if($id == ""){
			return array("state" => 200, "info" => '信息提交失败，请重试！');
		}
		...
		//保存到主表
		$archives = $dsql->SetQuery("UPDATE `#@__shop_product` SET `type` = '$typeid', `title` = '$title', `brand` = '$brand', `property` = '$property', `category` = '$category', `mprice` = '$mprice', `price` = '$price', `logistic` = '$logistic', `volume` = '$volume', `weight` = '$weight', `specification` = '$specifival', `inventory` = '$inventory', `limit` = '$limit', `litpic` = '$litpic', `state` = 0, `pics` = '$imglist', `body` = '$body' WHERE `id` = ".$id);
	...
	public function logisticEdit(){
		...
		$id    = $param['id'];
		$title = $param['title'];
		if(empty($id)) die('{"state": 200, "info": "提交失败，请重试！"}');
		if(empty($title)) die('{"state": 200, "info": "请输入模板名称！"}');
