...
class waimai {
	...
	public function paySuccess(){
		...
		if(!empty($param)){
			...
			// 判断是否跑腿订单
				...
				$archives = $dsql->SetQuery("SELECT `id`, `shop`, `paydate`, `amount` FROM `#@__paotui_order` WHERE `ordernum` = '$ordernum'");
				$res = $dsql->dsqlOper($archives, "results");
				if($res){
					$id       = $res[0]['id'];
					$uid      = $res[0]['uid'];
					...
					if($paydate == 0){
						...
						//更新订单状态
						$archives = $dsql->SetQuery("UPDATE `#@__paotui_order` SET `state` = 2, `paytype` = '$paytype', `paydate` = '$date' WHERE `ordernum` = '$ordernum'");
						$dsql->dsqlOper($archives, "update");
	...
	public function courierOrderList(){
		...
		// 跑腿
			...
			$archives = $dsql->SetQuery("SELECT o.`id`, o.`type`, o.`shop`, o.`price`, o.`totime`, o.`gettime`, o.`ordernum`, o.`address`, o.`buyaddress`, o.`person`, o.`tel`, o.`address`, o.`note`, o.`gettel`, o.`getperson`, o.`pubdate`, o.`state` FROM `#@__paotui_order` o".$where);
			...
			if($results){
				foreach ($results as $key => $value) {
					...
					//计算骑手距离取起点多远
					$juliShop = getDistance($clat, $clng, $value['buylng'], $value['buylat']);
					$list[$key]['juliShop'] = $juliShop;

					//计算起点距离终点多远
					$juliPerson = getDistance($value['buylat'], $value['buylng'], $value['lat'], $value['lng']);
					$list[$key]['juliPerson'] = $juliPerson;
				}
				...
				if($ret){
					...
					if($ret == "ok"){
		                ...
		                if($ret_){
		                    ...
		                    updateMemberNotice($uid, "会员-取货提醒", $param, array("ordernum" => $ordernumstore));
		                }
				...
				if($ret){
					...
					if($ret == "ok"){
		                ...
		                if($ret_){
		                    ...
		                    updateMemberNotice($uid, "会员-订单完成通知", $param, array("ordernum" => $ordernum, "date" => date("Y-m-d H:i:s", $okdate)));
		                }
	...
	public function paotuiDeal(){
		...
		if($type == "1"){
			...
			if($buyfrom == 1){
				if(empty($buylng) || empty($buylat) || empty($buyaddress)){
					return array("state" => 200, "info" => '请填写商品购买地址！');
				}
				// 计算距离和价格
				$juli = getDistance($lat, $lng, $buylat, $buylng)/1000;
				$juli = round($juli);

				if($juli > 24){
			      	return array("state" => 200, "info" => "您超出骑手的送货范围了 $juli");
			    }
			...
			if($ret){
				...
				$sql = $dsql->SetQuery("UPDATE `#@__paotui_order` SET `ordernum` = '$newOrdernum', `buyaddress` = '$buyaddress', `buylng` = '$buylng', `buylat` = '$buylat', `totime` = '$totime', `price` = '$price', `tip` = '$tip', `freight` = '$freight', `person` = '$person', `tel` = '$tel', `address` = '$address', `paytype` = '$paytype', `lng` = '$lng', `lat` = '$lat', `amount` = '$amount', `pubdate` = '$pubdate' WHERE `id` = $id");
				...
				$sql = $dsql->SetQuery("INSERT INTO
					...
					VALUES
					('$uid', '$newOrdernum', '$shop', '$buyaddress', '$buylng', '$buylat', '$totime', '$price', '$tip', '$freight', '0', '0', '$person', '$tel', '$address', '$paytype', '$lng', '$lat', '$amount', '$pubdate')
				");
		...
		// 帮我送
			...
			$sql = $dsql->SetQuery("SELECT `id`, `addr1`, `addr2` FROM `#@__paotui_order` WHERE `uid` = $uid AND `type` = 2 AND `state` <= 0 ORDER BY `id` DESC LIMIT 0,1");
			$ret = $dsql->dsqlOper($sql, "results");
			if($ret){
				$data = $ret[0];
				$id = $data['id'];
				...
				$sql = $sql = $dsql->SetQuery("SELECT * FROM `#@__waimai_address` WHERE `uid` = $uid AND (`id` = $addr1 OR `id` = $addr2)");
				$ret = $dsql->dsqlOper($sql, "results");
				if($ret && count($ret) == 2){
					foreach ($ret as $key => $value) {
						if($value['id'] == $addr1){
					...
					// 计算距离和价格
					$juli = getDistance($lat1, $lng1, $lat2, $lng2)/1000;
					$juli = round($juli);
				...
				$sql = $dsql->SetQuery("UPDATE `#@__paotui_order` SET `ordernum` = '$newOrdernum', `gettime` = '$gettime', `totime` = '$totime', `tip` = '$tip', `freight` = '$freight', `buyaddress` = '$address1', `address` = '$address2', `person` = '$person2', `getperson` = '$person1', `tel` = '$tel2', `gettel` = '$tel1', `note` = '$note', `paytype` = '$paytype', `amount` = '$amount', `buylat` = '$lat1', `buylng` = '$lng1', `lat` = '$lat2', `lng` = '$lng2', `pubdate` = '$pubdate', `state` = 0 WHERE `id` = ".$id);
				// echo $sql;die;
				$ret = $dsql->dsqlOper($sql, "update");
			}else{
				return array("state" => 200, "info" => '提交失败！');
	...
	/**
		...
		* @return array
		*/
	public function paotuiSongDeal1(){
		global $dsql;
		global $userLogin;
		$uid = $userLogin->getMemberID();
		...
		// 删除失效订单
		$sql = $dsql->SetQuery("DELETE FROM `#@__paotui_order` WHERE `uid` = $uid AND `state` = -1");
		$dsql->dsqlOper($sql, "update");

		$sql = $dsql->SetQuery("INSERT INTO `#@__paotui_order` (`uid`, `type`, `shop`, `weight`, `price`, `state`) VALUES ('$uid', '2', '$shop', '$weight', '$price', -1)");
	...
	/**
		...
		* @return array
		*/
	public function paotuiSongDeal2(){
		global $dsql;
		global $userLogin;
		$uid = $userLogin->getMemberID();
		...
		if(empty($name) || empty($value)) return array("state" => 200, "info" => '提交失败！');


		$sql = $dsql->SetQuery("SELECT `id` FROM `#@__paotui_order` WHERE `uid` = $uid AND `type` = 2 AND `state` = -1");
		$ret = $dsql->dsqlOper($sql, "results");
		if($ret){
			if(strstr($name, ",")){
				...
				foreach ($namearr as $k => $val) {
					$v = $val == "gettime" ? GetMkTime($valarr[$k]) : $valarr[$k];
					array_push($item, "`$val` = '$v'");
				}
				$sql = $dsql->SetQuery("UPDATE `#@__paotui_order` SET ".join(",", $item)." WHERE `id` = ".$ret[0]['id']);
				// echo $sql;die;
			}else{
				$sql = $dsql->SetQuery("UPDATE `#@__paotui_order` SET `$name` = '$value' WHERE `id` = ".$ret[0]['id']);
			}
			$ret = $dsql->dsqlOper($sql, "update");
			if($ret == "ok"){
				return "提交成功";
			}else{
				return array("state" => 200, "info" => '提交失败1！');
			}
		}else{
			return array("state" => 200, "info" => '操作失败2！');
		}
	...
	public function cancelPaotuiOrder(){
		...
		//验证订单
		$archives = $dsql->SetQuery("SELECT o.`paytype`, o.`paydate`, o.`amount`, l.`ordernum` FROM `#@__paotui_order` o LEFT JOIN `#@__pay_log` l ON l.`body` = o.`ordernum` WHERE o.`id` = '$id' AND o.`uid` = '$uid' AND o.`state` = 2");
		$results  = $dsql->dsqlOper($archives, "results");
		if($results){
			$amount        = $results[0]['amount'];
			$paytype       = $results[0]['paytype'];
			...
			if($r){
				...
				updateMemberNotice($uid, "会员-订单退款成功", $param, array("username" => $user['username'], "order" => $ordernum, 'amount' => $amount));
	...
	public function orderPaotuiDetail(){
		...
		if($userid > -1){
			...
			$sql = $dsql->SetQuery("SELECT * FROM `#@__paotui_order` WHERE `id` = $id AND `peisongid` = $did");
		}
		$ret = $dsql->dsqlOper($sql, "results");
		$return = array();
		if($ret){
