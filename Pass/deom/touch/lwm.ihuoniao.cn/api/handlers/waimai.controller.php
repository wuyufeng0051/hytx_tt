...
function waimai($params, $content = "", &$smarty = array(), &$repeat = array()){
	...
	if($do == "courier" && $action == "detail"){
		...
		if(is_array($detailConfig) && $detailConfig['state'] == 100){
			...
			if(is_array($detailConfig)){
				...
				if($detailConfig['peisongid'] != $did){
					header("location:http://".$cfg_basehost."/404.html?v=1");
				}
				...
				header("location:http://".$cfg_basehost."/404.html?v=2");
			}



		}else{
			header("location:http://".$cfg_basehost."/404.html?v=3");
		}
	...
	//商铺详细
		...
		//验证是否为首单
		$firstOrder = 1;
		$sql = $dsql->SetQuery("SELECT `id` FROM `#@__waimai_order` WHERE `uid` = $userid AND `sid` = $id AND `state` = 1");
		$ret = $dsql->dsqlOper($sql, "results");
		if($ret){
			$firstOrder = 0;
		}
	...
	//订单继续支付
		...
		if($ordertype == "waimai"){
			...
			//查询订单信息
			$sql = $dsql->SetQuery("SELECT `ordernum`, `amount` FROM `#@__waimai_order` WHERE `id` = $orderid AND `state` = 0");
			$ret = $dsql->dsqlOper($sql, "results");
			if($ret){
	...
	// --------------------------------跑腿
	// 帮我买详情页
	}elseif($action == "paotui-buy" || $action == "addressPaotui" ){
		...
		$huoniaoTag->assign('shopmap', $shopmap ? $shopmap : 0);	//直接显示地图
		...
		if($address){
			...
			$sql = $dsql->SetQuery("SELECT * FROM `#@__waimai_address` WHERE `uid` = $userid $where ORDER BY `id` DESC LIMIT 0, 1");
		}
		...
		//送 - 收货地址
		$address_id2 = $address_lng2 = $address_lat2 = 0;
		if($address2){
			$sql = $dsql->SetQuery("SELECT * FROM `#@__waimai_address` WHERE `uid` = $userid AND `id` = $address2");
			$ret = $dsql->dsqlOper($sql, "results");
			if($ret){
				$data = $ret[0];
				$address_id2      = $data['id'];
				$address_person2  = $data['person'];
				$address_tel2     = $data['tel'];
				$address_street2  = $data['street'];
				$address_address2 = $data['address'];
				$address_lng2     = $data['lng'];
				$address_lat2     = $data['lat'];
			}
		}

		$huoniaoTag->assign("cart_address_id2", $address_id2);
		$huoniaoTag->assign("cart_address_person2", $address_person2);
		$huoniaoTag->assign("cart_address_tel2", $address_tel2);
		$huoniaoTag->assign("cart_address_street2", $address_street2);
		$huoniaoTag->assign("cart_address_address2", $address_address2);

		$huoniaoTag->assign("cart_address_lng2", $address_lng2);
		$huoniaoTag->assign("cart_address_lat2", $address_lat2);

		if($action == "addressPaotui"){
			$huoniaoTag->assign("address", (int)$address);
			$huoniaoTag->assign("address2", (int)$address2);
		}
		...
		$huoniaoTag->assign("dopost", (int)$do);
		...
		// 获取已提交的未完成订单
		$archive = $dsql->SetQuery("SELECT * FROM `#@__paotui_order` WHERE `uid` = $userid AND `type` = 2 AND `state` <= 0 ORDER BY `id` DESC LIMIT 0,1");
		...
		if($result){
			foreach ($result[0] as $key => $value) {
				...
				if($key == "addr1" || $key == "addr2"){
					...
					$address_id2 = $address_lng2 = $address_lat2 = 0;
					$address_person2 = $address_tel2 = $address_street2 = $address_address2 = "";

					$sql = $sql = $dsql->SetQuery("SELECT * FROM `#@__waimai_address` WHERE `uid` = $userid AND `id` = $value");
					$ret = $dsql->dsqlOper($sql, "results");
					if($ret){
						$data = $ret[0];
						$address_id      = $data['id'];
					...
					$huoniaoTag->assign("cart_address_id".$no, $address_id);
					$huoniaoTag->assign("cart_address_person".$no, $address_person);
					$huoniaoTag->assign("cart_address_tel".$no, $address_tel);
					$huoniaoTag->assign("cart_address_street".$no, $address_street);
					$huoniaoTag->assign("cart_address_address".$no, $address_address);

					$huoniaoTag->assign("cart_address_lng".$no, $address_lng);
					$huoniaoTag->assign("cart_address_lat".$no, $address_lat);
