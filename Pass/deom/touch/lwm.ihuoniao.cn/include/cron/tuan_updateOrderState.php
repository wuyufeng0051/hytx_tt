...
$time = time();
$sql = $dsql->SetQuery("SELECT o.`id` FROM `#@__tuan_order` o LEFT JOIN `#@__tuanlist` l ON l.`id` = o.`proid` WHERE ((o.`orderstate` = 0 OR o.`orderstate` = 1) AND l.`enddate` < $time) OR (l.`maxnum` - l.`defbuynum` - l.`buynum` - o.`procount` < 0)");
