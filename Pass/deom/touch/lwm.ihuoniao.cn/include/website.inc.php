...
if($type == "template"){
	...
	$archives = $dsql->SetQuery("SELECT `id` FROM `#@__website` WHERE `id` = ".$id);
}
...
?>
{
    "data": {
        ...
        "url": "<?php echo $cfg_clihost."/website/".$projectid;?>"
    },
...
//提交留言
	...
	if(empty($token)) die('{"state": 200, "info": '.json_encode("留言失败-token！").'}');
	if(empty($user)){
	...
	$archives = $dsql->SetQuery("INSERT INTO `#@__website_guest` (`website`, `people`, `contact`, `ip`, `ipaddr`, `note`, `state`, `pubdate`) VALUES ('$id', '$user', '$contact', '".GetIP()."', '".getIpAddr(GetIP())."', '$content', '0', '".GetMkTime(time())."')");
	$results = $dsql->dsqlOper($archives, "update");
	if($results == "ok"){
