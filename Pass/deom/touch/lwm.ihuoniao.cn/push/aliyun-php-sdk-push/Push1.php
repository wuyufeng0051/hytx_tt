...
$accessKeyId = "nkgWOk115DhnQJtE";
$accessKeySecret = "UFJZ7octrtGG0kW1fgT4jFRefl14cj";
$appKey = "24324205";
...
$request->setTargetValue("5"); //根据Target来设定，如Target=device, 则对应的值为 设备id1,设备id2. 多个值使用逗号分隔.(帐号与设备有一次最多100个的限制)
$request->setDeviceType("ANDROID"); //设备类型 ANDROID iOS ALL.
$request->setPushType("NOTICE"); //消息类型 MESSAGE NOTICE
$rand = mt_rand(1,100);
$request->setTitle("title " . $rand); // 消息的标题
$request->setBody("body " . $rand); // 消息的内容
...
// $request->setAndroidOpenUrl("http://www.jindianshenghuo.com/?service=waimai&do=courier&state=4,5");//Android收到推送后打开对应的url,仅当AndroidOpenType="URL"有效
...
$request->setAndroidExtParameters("{\"music\": \"notice\", \"url\":\"http://www.jindianshenghuo.com/?service=waimai&do=courier&state=4,5\"}"); // 设定android类型设备通知的扩展属性
