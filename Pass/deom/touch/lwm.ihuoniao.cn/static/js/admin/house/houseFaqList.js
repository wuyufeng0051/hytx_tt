	...
	huoniao.operaJson("houseFaqList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < houseFaqList.length; i++){
				...
				list.push('  <td class="row15">'+houseFaqList[i].date+'</td>');
				list.push('  <td class="row12">'+houseFaqList[i].state+'</td>');
				list.push('  <td class="row10"><a data-id="'+houseFaqList[i].id+'" data-title="'+houseFaqList[i].title+'" href="javascript:;" title="修改" class="edit">编辑</a><a href="javascript:;" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
