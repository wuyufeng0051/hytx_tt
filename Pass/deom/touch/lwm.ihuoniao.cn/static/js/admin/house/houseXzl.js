	...
	huoniao.operaJson("houseXzl.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < houseXzl.length; i++){
				...
				list.push('  <td class="row12 left">'+houseXzl[i].loupan+'</td>');

				var type = houseXzl[i].usertype == 0 ? "(个人)" : "(中介)";
				var username = "";
				if(houseXzl[i].userid != 0){
					username = '<a href="javascript:;" data-id="'+houseXzl[i].userid+'">'+houseXzl[i].username+'</a>';
				}else{
				...
				list.push('  <td class="row15">'+houseXzl[i].pubdate+'</td>');
