	...
	$("#tab li").bind("click", function(){
		if(!$(this).hasClass("selected")){
			...
			if(model.html() != undefined){
				...
				$("#modelInfo").show();
				$("#module").css({"height": $(".sub-top").height() + $("#modelList").height() + $("#modelInfo").height() + 10});
			}else{
...
//	});
	...
	$(".h-nav").delegate("a", "click", function(event){
		...
		if(id == undefined){
			...
			//id = id[1].split(".")[0];
			id = id[1].replace(/\./g, "").replace(/\=/g, "").replace(/\?/g, "").replace(/\&/g, "");
		}
		...
		if(href != "" && id != undefined){
			...
			if(id == "store"){
				...
				$("#welcome-"+parentId).find("a").each(function() {
					...
					//id_ = id_[1].split(".")[0];
					id_ = id_[1].replace(/\./g, "").replace(/\=/g, "").replace(/\?/g, "").replace(/\&/g, "");
	...
	$(".default-nav").delegate("li", "click", function(e){
		...
		//普通菜单
			...
			if(e.target.nodeName.toLowerCase() == "s"){
				...
				if($(this).hasClass("cur")){
					if($(".default-nav li:eq("+(index-1)+")").attr("data-type") != "store"){
						$("#welcome div").hide();
					}
				...
				if(type != "store"){
					...
					if(id != "index"){
						$("#welcome-"+type+" a").each(function() {
							...
							id_ = href[1].replace(/\./g, "").replace(/\=/g, "").replace(/\?/g, "").replace(/\&/g, "");
							if(id_ == id){
								$(this).addClass("on");
			...
			success: function (d) {
				...
				if(data.length > 0){
					...
					for(var i = 0; i < data.length; i++){
						...
						if(data[i].id == 'waimaiOrderphp' && data[i].count > 0){
							hasWaimai = true;
						}
					}
				...
				//消息提醒
				audio.src = hasWaimai ? "/static/audio/notice02.mp3" : "/static/audio/notice01.mp3";
				if(d.hasnew || hasWaimai){
					//标题闪动
					clearInterval(timer);
...
function fBodyHeight(m){
	...
	$(".navul ul").width(Math.ceil(w+0.5));
