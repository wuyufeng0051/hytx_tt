$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobDownloadList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("jobDownloadListphpdopostAdd", type, "新增招聘资讯", "job/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("jobDownloadList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, jobDownloadList = val.jobDownloadList;
		if(val.state == "100"){
			huoniao.hideTip();

			obj.attr("data-totalpage", val.pageInfo.totalPage);

			for(i; i < jobDownloadList.length; i++){
				list.push('<tr data-id="'+jobDownloadList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row35 left">'+jobDownloadList[i].title+'</td>');
				list.push('  <td class="row5">'+jobDownloadList[i].click+'</td>');
				list.push('  <td class="row15"><a href="javascript:;" data-id="'+jobDownloadList[i].typeid+'" class="type">'+jobDownloadList[i].type+'</a></td>');
				list.push('  <td class="row15">'+jobDownloadList[i].date+'</td>');
				list.push('  <td class="row12">'+jobDownloadList[i].state+'</td>');
				list.push('  <td class="row15"><a data-id="'+jobDownloadList[i].id+'" data-title="'+jobDownloadList[i].title+'" href="jobDownloadList.php?dopost=Edit&id='+jobDownloadList[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
