$(function(){
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			...
			parent.addPage("jobSentenceAdd"+type, "job", "新增信息", "job/"+href);
		} catch(e) {}
	});
	...
	huoniao.operaJson("jobSentence.php?dopost=getList&type="+type, data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < jobSentence.length; i++){
				...
				list.push('  <td class="row15 left">'+jobSentence[i].people+'</td>');
				list.push('  <td class="row15 left">'+jobSentence[i].contact+'</td>');
				var state = "";
