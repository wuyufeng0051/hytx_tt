...
$(function(){
	...
	var init = {
		...
		selectTypeList: function(type){
			var typeList = [], title = type == "addr" ? "选择地区" : "选择分类";
	...
	$("#addrBtn a").bind("click", function(){
		if($(this).parent().hasClass("dropdown-submenu")) return false;
		var id = $(this).attr("data-id"), title = $(this).text();
		$("#addrid").val(id);
	...
	function getCircle(){
		...
		huoniao.operaJson("tuanStoreAdd.php?action=getCircle", "addrid="+addrid, function(data){
	...
	$(".chooseData").bind("click", function(){
		...
		huoniao.operaJson("../siteConfig/siteSubway.php?dopost=getSubway", "addrid="+$("#addrid").val(), function(data){
			huoniao.hideTip();
			...
			if(data.state == 100){
				if($("#dopost").val() == "save"){
					...
					$.dialog({
						...
						ok: function(){
							try{
								$("body",parent.document).find("#nav-tuanStorephp").click();
								parent.reloadPage($("body",parent.document).find("#body-tuanStorephp"));
								$("body",parent.document).find("#nav-tuanStoreEdit"+$("#id").val()+" s").click();
							}catch(e){
								location.href = thisPath + "tuanStore.php";
							}
						},
						cancel: false
					});
