$(function() {
				...
				$.ajax({
			        ...
			        success: function(data){
			          if(data.state == 100){
			            ...
			            var list = data.info;
			            var html = [];
			            html.push('<li data-id="" class="on"><a href="javascript:;">不限</a></li>');
			...
			$.ajax({
				...
				success: function (data) {
					if(data && data.state == 100){
						...
						list.push('<ul>');
						list.push('<li><a href="javascript:;">全部</a></li>');
	...
	// 分类和排序
	$('#scroll-assort-second, #scroll-sort, #scroll-area-second').delegate("li", "click", function(){
	...
	function getList(tr){
		...
		if (!addrid) {
			addrid = $('#scroll-area li.on').attr('data-id');
		}
					...
					if(data.state == 100){
						...
						if(list.length > 0){
							for(var i = 0; i < list.length; i++){
								...
								html.push('<span>经营类目：'+list[i].typename[0]+'</span>&nbsp;&nbsp;<span>人均消费：'+list[i].amount+'</span>');
								html.push('</p>');
								html.push('<p class="per">');
								html.push('<span>联系人：'+list[i].member['company']+'</span></span>');
								html.push('</p>');
								html.push('<p class="address grey"><span>所在地：'+list[i].addrname[0]+'</span><span>'+list[i].addrname[1]+'</span></p>');
								html.push('</div>');
								html.push('</a>');
								html.push('</li>');
							}
