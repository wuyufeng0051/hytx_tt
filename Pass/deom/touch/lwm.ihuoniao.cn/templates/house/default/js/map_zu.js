$(function(){
	...
	var map, filterData, list = $(".list"), ids = 0, init = {
		...
		,setSjRange: function(val){
			var minTxt = maxTxt = "元", min = val[0], max = val[1];
		...
		,confirmFilter: function(){
			...
			$(".filter-confirm").bind("click", function(){
				...
				if(sjMin != 0 || sjMax != g_conf.sjMax){
					filterTitle.push(sjMin + "-" + sjMax + "元");
				}
	...
	var bubbleMapSize = {
		...
		,bubbleTemplate = {
			...
			//区域
			1 : '<div class="bubble bubble-1" data-longitude="${longitude}" data-latitude="${latitude}" data-id="${id}"><p class="name" title="${name}区">${name}区</p><p class="num">${avg_price}元/月</p><p><span class="count">${house_count}</span>套</p></div>',

			//区域
			2 : '<div class="bubble bubble-1 bubble-2" data-longitude="${longitude}" data-latitude="${latitude}" data-id="${id}"><p class="name" title="${name}区">${name}区</p><p class="num">${avg_price}元/月</p><p><span class="count">${house_count}</span>套</p></div>',
