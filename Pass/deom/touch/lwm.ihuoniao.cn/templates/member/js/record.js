			...
			if(data && data.state != 200){
				if(data.state == 101){
					...
					if(list.length > 0){
						...
						objId.html('<table><thead class="thead"><tr><th class="fir"></th><th>来源/用途</th><th style="padding-left: 30px;">余额变化</th><th>时间</th></tr></thead>'+html.join("")+'</table>');
					...
					$("#total label").html("&nbsp;&nbsp;（总收入 <strong style='color:#30992e;'>"+pageInfo.totalAdd+"</strong>&nbsp;&nbsp;|&nbsp;&nbsp;总支出 <strong style='color:#b1000f;'>"+pageInfo.totalLess+"</strong>）");
					showPageInfo();
				}
			}else{
				objId.html("<p class='loading'>暂无相关信息！</p>");
