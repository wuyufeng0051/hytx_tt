$(function(){
	...
	$("#chnickname").bind("click", function(){
		modifyFun("修改登录昵称", "chnicknameEdit", "nick", 500, "确定修改", function(){
			var name = $("#name");
			return "name="+name.val();
		}, function(){
			var name = $("#name");
			if($.trim(name.val()) == ""){
				popTip("请输入新的昵称", "error");
				name.focus();
				return "false";
			}
		});
	...
	$("#chCertify").bind("click", function(){
		modifyFun("实名认证", "chCertifyAdd", "certify", 580, "提交认证", function(){
	...
	$("#shCertify").bind("click", function(){
		modifyFun("实名认证信息", "chCertify", "certify", 580, false);
