$(function(){
	...
	function mysub(){
		...
		save.addClass('disabled').text('请稍后');
		...
		$.ajaxFileUpload({
			...
			success: function(m, l) {
				if (m.state == "SUCCESS") {
					picid = m.url;
					save.removeClass('disabled').text('保存');
	...
	function saveFile(data){
		...
		if(picid != 0){
			...
			save.addClass("disabled").text("保存中");
			resbtn.addClass('disabled');
			...
			$.ajax({
				...
				success: function (data) {
					if(data && data.state == 100){
						resbtn.remove();
						save.removeClass("disabled").text("保存成功");
						setTimeout(function(){
	...
	function upFailed(info){
		...
		setTimeout(function(){
			upbtn.show().removeClass('disabled').text('重新上传');
		},1000)
	}
