$(function(){
		...
		//最大
			...
			if(type == 2){
				...
				if(val <= freeshi){
					...
					t.find(".total .del").html("含运费 &yen;"+freight);
				}else{
	...
	function checkTotal(){
		...
		$("#totalPrice").html("&yen;"+price.toFixed(2));
	...
	$(".section .delete").bind("click", function(){
		...
		var t = $(this), tr = t.closest("tr"), id = tr.data("id");
		var newCartData = [];

		if(cartData != "" && cartData != null){
			cartData = cartData.split("|");
			for(var i = 0; i < cartData.length; i++){
				var data = cartData[i].split(",");
				if(id != data[0]){
		...
		$.cookie(cookiePre+"tuan_cart", newCartData.join("|"), {expires: 7, domain: cookieDomain, path: '/'});
	...
	$("#submit").bind("click", function(){
		...
		//提交
		cartData = $.cookie(cookiePre+"tuan_cart");
		var t = $(this), data = [], action = t.closest("form").attr("action"), cartDataArr = cartData.split("|"), newCartData = cartDataArr;
		$(".section tbody tr").each(function(){
