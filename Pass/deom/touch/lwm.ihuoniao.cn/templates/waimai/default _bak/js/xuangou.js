...
$(function(){
	...
	//商品分类切换
	$(".main_left li").bind("click", function(){
		var t = $(this), id = t.data("id");
		t.addClass("ml_bac").siblings("li").removeClass("ml_bac");
		$(".main_item").hide();
		$("#item"+id).show();
		$(".main_right").scrollTop(0);
	});
	...
	var cartInit = {
		...
		init: function(){
			...
			if(cartHistory){
				...
				for (var i = 0; i < cartHistory.length; i++) {
					...
					list.push('</div>');
					list.push('<span class="sale-price">&yen;'+Number(cartHistory[i].price+cartHistory[i].nprice)+'</span>');
				...
				if(dabaoPrice > 0){
					...
					list.push('<h3>打包费</h3>');
					list.push('</div>');
					list.push('<span class="sale-price">&yen;'+dabaoPrice+'</span>');
					list.push('</div>');
				}
		...
		,updateFood: function(type, btn, id, title, pic, unit, price, dabao, ntitle, nprice){
				...
				if(!lb.hasClass("dabao")){
					if(lid == id && lntitle == ntitle){
						...
						if(account == 0 || (type == "reduce" && account == 1)){
							...
							lb.find(".sale-price").html("&yen;"+ Number((account * Number(nprice + price))));
						}
			...
			if(!has && type == "plus"){
				...
				list.push('</div>');
				list.push('<span class="sale-price">&yen;'+Number(price+nprice)+'</span>');
				list.push('<span class="r">');
				list.push('<em class="num-rec">－</em>');
