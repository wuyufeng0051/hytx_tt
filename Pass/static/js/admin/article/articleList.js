	...
	huoniao.operaJson("articleJson.php?dopost="+action, data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < articleList.length; i++){
				...
				list.push('  <td class="row20 left">'+img+'<span><a href="'+articleList[i].url+'" target="_blank"'+color+'>'+articleList[i].title+'</a>'+append+'</span></td>');
				...
				list.push('  <td class="row5">'+(recycle != "" ? "&nbsp;" : articleList[i].sort)+'</td>');
				var state = "";
