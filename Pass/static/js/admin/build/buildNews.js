$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("buildNews.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#typeManage").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("buildNewsType", type, "建材资讯分类", "build/"+href);
		} catch(e) {}
	});
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("buildNewsAdd", type, "新增建材资讯", "build/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("buildNewsEdit"+id, "build", title, "build/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("buildNews.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, buildNewsList = val.buildNewsList;
		if(val.state == "100"){
			huoniao.hideTip();
			...
			for(i; i < buildNewsList.length; i++){
				list.push('<tr data-id="'+buildNewsList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row35 left">'+buildNewsList[i].title+'</td>');
				list.push('  <td class="row5">'+buildNewsList[i].weight+'</td>');
				list.push('  <td class="row15"><a href="javascript:;" data-id="'+buildNewsList[i].typeid+'" class="type">'+buildNewsList[i].type+'</a></td>');
				list.push('  <td class="row15">'+buildNewsList[i].date+'</td>');
				list.push('  <td class="row12">'+buildNewsList[i].state+'</td>');
				list.push('  <td class="row15"><a data-id="'+buildNewsList[i].id+'" data-title="'+buildNewsList[i].title+'" href="buildNews.php?dopost=Edit&id='+buildNewsList[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
