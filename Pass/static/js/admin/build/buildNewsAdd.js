...
$(function () {
	...
	$("#btnSubmit").bind("click", function(event){
		...
		$.ajax({
			type: "POST",
			url: "buildNews.php?dopost="+$("#dopost").val(),
				...
				if(data.state == 100){
					...
					if($("#dopost").val() == "Add"){
						...
						$.dialog({
							...
							ok: function(){
								try{
									$("body",parent.document).find("#nav-buildNewsphp").click();
									parent.reloadPage($("body",parent.document).find("#body-buildNewsphp"));
									$("body",parent.document).find("#nav-buildNewsEdit"+id+" s").click();
								}catch(e){
									location.href = thisPath + "buildNews.php";
								}
							},
							cancel: false
						});
