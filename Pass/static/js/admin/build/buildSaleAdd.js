...
$(function () {
	...
	$("#btnSubmit").bind("click", function(event){
		...
		$.ajax({
			type: "POST",
			url: "buildSale.php?dopost="+$("#dopost").val(),
				...
				if(data.state == 100){
					...
					if($("#dopost").val() == "Add"){
						...
						$.dialog({
							...
							ok: function(){
								try{
									$("body",parent.document).find("#nav-buildSalephp").click();
									//parent.reloadPage($("body",parent.document).find("#body-houseNewsListphp")[0].contentWindow);
									parent.reloadPage($("body",parent.document).find("#body-buildSalephp"));
									$("body",parent.document).find("#nav-buildSaleEdit"+id+" s").click();
								}catch(e){
									location.href = thisPath + "buildSale.php";
								}
							},
							cancel: false
						});
