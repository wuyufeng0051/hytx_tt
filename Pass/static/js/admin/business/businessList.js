	...
	huoniao.operaJson("businessList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < businessList.length; i++){
				...
				var img = '<img src="'+cfg_attachment+businessList[i].logo+'" class="litpic" />';
				...
				var order = [];
				for(var o = 0; o < businessList[i].order.length; o++){
					order.push('<span class="badge badge-success" style="font-size: 10px;" title="过期时间：'+huoniao.transTimes(businessList[i].order[o]['expired'], 1)+'">'+businessList[i].order[o]['name']+'</span>');
				}

				list.push('  <td class="row15 left">'+img+'<span><a href="'+businessList[i].url+'" target="_blank">'+businessList[i].title+'</a><br />'+auth.join("")+'</span></td>');
				list.push('  <td class="row15 left"><a href="javascript:;" data-id="'+businessList[i].uid+'" class="userinfo">'+businessList[i].user+'</a><br />'+order.join(" ")+'<a href="javascript:;" data-id="'+businessList[i].uid+'" class="manage" style="margin: 0 0 0 5px;" title="管理开通模块"><i class="icon-list-alt" style="margin-top: 7px;"></i></a></td>');
				list.push('  <td class="row10 left">'+businessList[i].typename+'</td>');
				list.push('  <td class="row10 left">'+businessList[i].addrname+'</td>');
				list.push('  <td class="row12 left">'+businessList[i].phone+'<br />'+businessList[i].email+'</td>');
				list.push('  <td class="row15 left">'+businessList[i].pubdate+'</td>');
				var state = "";
				...
				list.push('  <td class="row10"><a data-id="'+businessList[i].id+'" data-title="'+businessList[i].title+'" href="businessAdd.php?dopost=edit&id='+businessList[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
