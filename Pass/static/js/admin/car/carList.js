$(function(){
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("carListAdd", "car", "添加车辆", "car/"+href);
		} catch(e) {}
	});
	...
	$("#stateBtn, #pageBtn, #paginationBtn").delegate("a", "click", function(){
		...
		if(obj.attr("id") == "paginationBtn"){
			...
			$("#typeBtn")
				.attr("data-id", "")
				.find("button").html('全部级别<span class="caret"></span>');
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("carListEdit"+id, "car", title, "car/"+href);
		} catch(e) {}
	});
	
	//删除
	$("#delBtn").bind("click", function(){
		$.dialog.confirm('确定后将删除此车辆的所有信息<br />此操作不可恢复，您确定要删除吗？', function(){
			init.del();
		});
	});
	...
	huoniao.operaJson("carList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < carList.length; i++){
				...
				list.push('  <td class="row25 left"><img src="'+cfg_attachment+carList[i].litpic+'&type=small" width="100" class="litpic" /><a href="/carList.php?id='+carList[i].id+'" target="_blank">'+carList[i].title+'</a></td>');
				list.push('  <td class="row15 left">'+carList[i].brandName+'</td>');
				list.push('  <td class="row7 left">'+carList[i].typeName+'</td>');
				list.push('  <td class="row13 left">'+carList[i].guide+'</td>');
				list.push('  <td class="row12 left">'+carList[i].pubdate+'</td>');
				var state = "";
				...
				list.push('<a data-id="'+carList[i].id+'" data-title="'+carList[i].title+'" href="carList.php?dopost=edit&id='+carList[i].id+'" title="修改" class="edit">修改</a>');
