...
$(function(){
	...
	//选择模板
	$("#tplList .choose").bind("click", function(){
		var t = $(this), li = t.closest("li"), ul = t.closest(".tpl-list"), img = li.find(".img"), id = img.attr("data-id");
	...
	//编辑模板
	$("#tplList .edit").bind("click", function(event){
		var t = $(this), obj = t.parent().prev(".img"), template = obj.attr("data-id"), touch = t.closest(".tpl-list").hasClass("touch") ? "touch" : "", title = obj.attr("data-title");
		try {
	...
	$("#tplList").delegate(".img", "click", function(){
		...
		$.dialog({
			id: 'tplPic',
			title: $(this).next("span").attr("title"),
			lock: true,
	...
	//删除模板
	$("#tplList a.del").bind("click", function(){
		var t = $(this), obj = t.parent().prev(".img"), floder = obj.attr("data-id"), type = t.closest(".tpl-list").hasClass("touch") ? "touch/" : "";
	...
	$("#checkFtpConn").bind("click", function(){
		var t = $(this), custome = $("input[name=articleFtp]:checked").val(), type = $("input[name=ftpType]:checked").val(), data = $("#ftpType"+type).find("input").serialize();
		if(t.text() == "正在连接...") return false;
		t.html("<font class='muted'>正在连接...</font>");
		var action = type == 1 ? "checkOssConn" : "checkFtpConn";
		if(custome == 0){
			action = "checkSystemConn";
			data = "";
		}
		huoniao.operaJson("../inc/json.php?action="+action, data, function(val){
			...
			huoniao.operaJson("../inc/json.php?action=getMemberInfo", "id="+id, function(data){
				...
				if(data){
					...
					$.dialog({
						...
						fixed: false,
						title: "会员信息",
						content: '<table width="100%"border="0"cellspacing="1"cellpadding="5" style="line-height:2em;"><tr><td width="100"valign="top"><img src="'+cfg_attachment+data[0]["photo"]+'&type=small"width="100"/></td><td width="80"align="right"valign="top">会员名：<br />昵称：<br />'+(data[0]["company"] ? "公司名称：<br />" : "")+'帐户：<br />性别：<br />邮箱：<br />电话：<br />QQ：<br />生日：<br />注册时间：<br />注册IP：</td><td valign="top">'+data[0]["username"]+(data[0]["level"] ? '<font color="red">【'+data[0]["level"]+'】</font>' : '')+'<br />'+data[0]["nickname"]+(data[0]["company"] ? '<br />'+data[0]["company"] : "")+'<br />余额 '+data[0]["money"]+'&nbsp;&nbsp;&nbsp;积分 '+data[0]["point"]+'&nbsp;&nbsp;&nbsp;保障金 '+data[0]["promotion"]+'<br />'+(data[0]["sex"] == 0 ? "女" : "男")+'<br />'+data[0]["email"]+(data[0]["emailCheck"] == 0 ? "&nbsp;<font color='#f00'>[未验证]</font>" : "&nbsp;<font color='green'>[已验证]</font>")+'<br />'+data[0]["phone"]+(data[0]["phoneCheck"] == 0 ? "&nbsp;<font color='#f00'>[未验证]</font>" : "&nbsp;<font color='green'>[已验证]</font>")+'<br />'+data[0]["qq"]+'<br />'+huoniao.transTimes(data[0]["birthday"], 2)+'<br />'+huoniao.transTimes(data[0]["regtime"], 2)+'<br />'+data[0]["regip"]+'</td></tr></table>',
						width: 550,
						okVal: "修改会员信息",
						ok: function(){
