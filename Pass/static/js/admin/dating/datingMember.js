	...
	huoniao.operaJson("datingMember.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < datingMember.length; i++){
				...
				list.push('  <td class="row25 left"><a href="'+datingMember[i].url+'" target="_blank" class="userinfo" title="查看会员信息"><img style="width: 45px;height:60px;" src="'+cfg_attachment+datingMember[i].photo+'&type=middle" />&nbsp;'+datingMember[i].username+' ['+datingMember[i].sex+']</a></td>');
