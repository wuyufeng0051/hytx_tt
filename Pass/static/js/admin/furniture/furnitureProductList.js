$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("furnitureProduct.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("furnitureProduct.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("furnitureProductEdit"+id, "furniture", title, "furniture/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("furnitureProduct.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, furnitureProduct = val.furnitureProduct;
		...
		if(val.state == "100"){
			...
			for(i; i < furnitureProduct.length; i++){
				list.push('<tr data-id="'+furnitureProduct[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row25 left"><img src="'+cfg_attachment+furnitureProduct[i].litpic+'&type=large" class="litpic" /><span><a href="'+furnitureProduct[i].url+'" target="_blank">'+furnitureProduct[i].title+'</a></span></td>');
				var company = '<a href="'+furnitureProduct[i].companyurl+'" target="_blank">'+furnitureProduct[i].company+'</a>';
				if(furnitureProduct[i].companyid == 0){
					company = furnitureProduct[i].company;
				}
				list.push('  <td class="row20 left">'+company+'</td>');
				list.push('  <td class="row12 left"><a href="javascript:;" data-id="'+furnitureProduct[i].typeid+'" class="type">'+furnitureProduct[i].typename+'</td>');
				list.push('  <td class="row7 left">'+furnitureProduct[i].price+'</td>');
				var state = "";
				switch (furnitureProduct[i].state) {
					case "0":
				...
				list.push('  <td class="row13">'+furnitureProduct[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+furnitureProduct[i].id+'" data-title="'+furnitureProduct[i].title+'" href="furnitureProductAdd.php?dopost=edit&id='+furnitureProduct[i].id+'" title="修改" class="edit">修改</a>');
