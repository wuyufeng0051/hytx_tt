...
$(function () {
	...
	$("#btnSubmit").bind("click", function(event){
		...
		$.ajax({
			type: "POST",
			url: "furnitureSale.php?dopost="+$("#dopost").val(),
				...
				if(data.state == 100){
					...
					if($("#dopost").val() == "Add"){
						...
						$.dialog({
							...
							ok: function(){
								try{
									$("body",parent.document).find("#nav-furnitureSalephp").click();
									//parent.reloadPage($("body",parent.document).find("#body-houseNewsListphp")[0].contentWindow);
									parent.reloadPage($("body",parent.document).find("#body-furnitureSalephp"));
									$("body",parent.document).find("#nav-furnitureSaleEdit"+id+" s").click();
								}catch(e){
									location.href = thisPath + "furnitureSale.php";
								}
							},
							cancel: false
						});
