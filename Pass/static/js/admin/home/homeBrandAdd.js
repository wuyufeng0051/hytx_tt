$(function () {
	...
	$("#btnSubmit").bind("click", function(event){
		...
		$.ajax({
			type: "POST",
			url: "homeBrand.php?dopost="+$("#dopost").val(),
				...
				if(data.state == 100){
					if($("#dopost").val() == "Add"){
						...
						$.dialog({
							...
							ok: function(){
								try{
									$("body",parent.document).find("#nav-homeBrandphp").click();
									//parent.reloadPage($("body",parent.document).find("#body-homeBrandphp")[0].contentWindow);
									parent.reloadPage($("body",parent.document).find("#body-homeBrandphp"));
									$("body",parent.document).find("#nav-homeBrandEdit"+id+" s").click();
								}catch(e){
									location.href = thisPath + "homeBrand.php";
								}
							},
							cancel: false
						});
