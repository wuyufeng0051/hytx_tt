$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("homeNews.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#typeManage").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("homeNewsType", type, "家居资讯分类", "home/"+href);
		} catch(e) {}
	});
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("homeNewsAdd", type, "新增家居资讯", "home/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("homeNewsEdit"+id, "home", title, "home/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("homeNews.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, homeNewsList = val.homeNewsList;
		if(val.state == "100"){
			huoniao.hideTip();
			...
			for(i; i < homeNewsList.length; i++){
				list.push('<tr data-id="'+homeNewsList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row35 left">'+homeNewsList[i].title+'</td>');
				list.push('  <td class="row5">'+homeNewsList[i].weight+'</td>');
				list.push('  <td class="row15"><a href="javascript:;" data-id="'+homeNewsList[i].typeid+'" class="type">'+homeNewsList[i].type+'</a></td>');
				list.push('  <td class="row15">'+homeNewsList[i].date+'</td>');
				list.push('  <td class="row12">'+homeNewsList[i].state+'</td>');
				list.push('  <td class="row15"><a data-id="'+homeNewsList[i].id+'" data-title="'+homeNewsList[i].title+'" href="homeNews.php?dopost=Edit&id='+homeNewsList[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
