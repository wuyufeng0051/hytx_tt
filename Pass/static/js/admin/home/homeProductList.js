$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("homeProduct.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("homeProduct.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("homeProductEdit"+id, "home", title, "home/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("homeProduct.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, homeProduct = val.homeProduct;
		...
		if(val.state == "100"){
			...
			for(i; i < homeProduct.length; i++){
				list.push('<tr data-id="'+homeProduct[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row25 left"><img src="'+cfg_attachment+homeProduct[i].litpic+'&type=large" class="litpic" /><span><a href="'+homeProduct[i].url+'" target="_blank">'+homeProduct[i].title+'</a></span></td>');
				var company = '<a href="'+homeProduct[i].companyurl+'" target="_blank">'+homeProduct[i].company+'</a>';
				if(homeProduct[i].companyid == 0){
					company = homeProduct[i].company;
				}
				list.push('  <td class="row20 left">'+company+'</td>');
				list.push('  <td class="row12 left"><a href="javascript:;" data-id="'+homeProduct[i].typeid+'" class="type">'+homeProduct[i].typename+'</td>');
				list.push('  <td class="row7 left">'+homeProduct[i].price+'</td>');
				var state = "";
				switch (homeProduct[i].state) {
					case "0":
				...
				list.push('  <td class="row13">'+homeProduct[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+homeProduct[i].id+'" data-title="'+homeProduct[i].title+'" href="homeProductAdd.php?dopost=edit&id='+homeProduct[i].id+'" title="修改" class="edit">修改</a>');
