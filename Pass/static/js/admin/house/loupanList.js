	...
	huoniao.operaJson("loupanList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < loupanList.length; i++){
				...
				list.push('  <td class="row10"><a href="javascript:;" data-id="'+loupanList[i].addrid+'" class="addr">'+loupanList[i].addr+'</a></td>');
				list.push('  <td class="row10">'+loupanList[i].username+'</td>');
