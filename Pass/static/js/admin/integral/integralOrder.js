	...
	huoniao.operaJson("integralOrder.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < integralOrder.length; i++){
				var level = '';
				if(integralOrder[i].userLevel.id > 0){
					level = '<div style="color:#f60;">'+integralOrder[i].userLevel.name+'</div>';
					if(integralOrder[i].courier != '0'){

					}
					level += '<div style="color:#f60;">'+integralOrder[i].courier+'</div>';
				}
				...
				list.push('  <td class="row10 left"><a href="javascript:;" data-id="'+integralOrder[i].userid+'" class="userinfo">'+integralOrder[i].username+'</a>'+level+'</td>');
