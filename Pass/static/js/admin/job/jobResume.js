	...
	huoniao.operaJson("jobResume.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < jobResume.length; i++){
				...
				list.push('  <td class="row15 left"><img src="'+cfg_attachment+jobResume[i].photo+'" style="width: 45px; height: 60px;" />&nbsp;&nbsp;<a href="'+jobResume[i].url+'" target="_blank">'+jobResume[i].name+'</a></td>');
