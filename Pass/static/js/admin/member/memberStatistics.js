	...
	huoniao.operaJson("memberStatistics.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < memberList.length; i++){
				...
				list.push('  <td class="row37 left"><a href="javascript:;" data-id="'+memberList[i].userid+'" class="userinfo">'+memberList[i].username+'</a></td>');
				list.push('  <td class="row30 left">'+memberList[i].amount+'</td>');
				list.push('  <td class="row30 left">'+huoniao.transTimes(memberList[i].lastdate, 1)+'</td>');
				list.push('</tr>');
			}
