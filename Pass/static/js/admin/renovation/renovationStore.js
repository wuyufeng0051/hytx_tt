	...
	huoniao.operaJson("renovationStore.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < renovationStore.length; i++){
				...
				list.push('  <td class="row25 left"><img src="'+cfg_attachment+renovationStore[i].logo+'&type=large" class="litpic" /><span><a href="'+renovationStore[i].url+'" target="_blank">'+renovationStore[i].company+'</a></span></td>');
