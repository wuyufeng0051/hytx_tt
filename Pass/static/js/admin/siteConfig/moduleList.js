			...
			for(var i = 0; i < l; i++){
				(function(){
					...
					if(jsonArray["parentid"] == 0){
						...
						typeList.push('  <div class="row40 left"><span class="plus-icon" style="margin-left:'+cl+'px;"></span><img width="40" src="../../static/images/admin/nav/'+jsonArray["icon"]+'" /> <strong>'+jsonArray["title"]+'</strong><sup>'+jsonArray["version"]+'</sup> ('+jsonArray["name"]+')&nbsp;&nbsp;<a href="javascript:;" class="explain">说明</a></div>');
			...
			if(id == ""){
				...
				huoniao.operaJson("moduleList.php?dopost=getDetail", "id="+id, function(data){
					if(data != null){
						...
						$.dialog({
							...
							ok: function(){
								...
								var title      = self.parent.$("#title").val(),
									...
									domainRules  = self.parent.$("#domainRules").val(),
									catalogRules = self.parent.$("#catalogRules").val(),
						...
						self.parent.$("#domainRules").val(data.domainRules);
						self.parent.$("#catalogRules").val(data.catalogRules);
