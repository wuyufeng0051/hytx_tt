		...
		quickEdit: function(id){
			...
			if(id !== "" && id != undefined){
				...
				huoniao.operaJson("smsAccount.php?action=getInfo&token="+$("#token").val(), "id="+id, function(data){
					if(data != null){
						...
						if (data.title == '阿里云' || data.title == '阿里大于' || data.title == '阿里大鱼' || data.title == '大于' || data.title == '大鱼') {
							...
							self.parent.$("input[name=international]").each(function(){
								if($(this).val() == data.international){
									$(this).attr("checked", true);
								}
							});
						}
			...
			$.dialog({
				...
				ok: function(){
					...
					if (typeid == '0') {
						serialize += 'username='+username+'&password='+password+'&signCode='+signCode+'&title=阿里云&website=https://www.aliyun.com/&contact=95187';
					}else if (typeid == '1') {
						serialize += 'username='+username+'&password='+password+'&signCode='+signCode+'&title=阿里大于&website=https://dayu.aliyun.com/&contact=95187';
					}else {
