$(function(){
	...
	function getComment(filter){
		...
		combox.html('').after('<div class="loading">正在加载，请稍后</div>');
		var data = [];
		filter = filter || '';
		data.push('id='+detailID);
		data.push('filter='+filter);
		$.ajax({
			url: masterDomain + "/include/ajax.php?service=bulid&action=common",
			data : data.join("&"),
			type: "GET",
			dataType: "jsonp",
			...
			error: function(){
				...
				$('.comment-box .loading').text('网络错误，加载失败！');
			}
		})
	}
