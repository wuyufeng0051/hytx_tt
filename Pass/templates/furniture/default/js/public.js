var isgoing,furnitureInit,glocart,cartbtn,carcountObj,cartwrap,cartlist,cartul,onClass,ishow;
$(function(){
...
furnitureInit = {
	...
	add: function(data){
		...
		//furnitureInit.list();  //新增前先更新，避免其他页面删除之后，当前页面直接新增还会保留删除前的数据
	...
	,update: function(){
		...
		$.cookie(cookiePre+"furniture_cart", data.join("|"), {expires: 7, domain: masterDomain.replace("http://", ""), path: '/'});
	...
	//删除全部
	,deleteAll: function(){
		$.cookie(cookiePre+"furniture_cart", null);
		cartul.html("");
		this.update();
	}
	...
	,list: function(){
		...
		var thi = this;
		var cartData = $.cookie(cookiePre+"furniture_cart");
		if(cartData == null || cartData == ""){
			...
			//异步获取信息详细
			$.ajax({
				url: masterDomain+'/include/ajax.php?service=furniture&action=getCartList',
				type: "GET",
				dataType: "jsonp",
