$(function(){
	...
	// 	if (winTop > tabTop) {
	// 		$('.select-box ul').addClass('top');
	// 	}
	// 	else{
	// 		$('.select-box ul').removeClass('top');
	// 	}
	// })
	...
	function getList(tr){
		...
		$(".command-list-con").append('<div class="loading">加载中...</div>');
		...
		$.ajax({
			url: "/include/ajax.php?service=furniture&action=flist",
			data: data.join("&"),
			type: "GET",
			dataType: "jsonp",
			success: function (data) {
				if(data){
					if(data.state == 100){
						...
						//没有数据
							...
							isload = true;
							$(".command-list-con").append('<div class="loading">暂无相关信息</div>');
						}
				...
				//加载失败
					...
					$(".command-list-con .loading").html('加载失败');
				}
			},
			error: function(){
				isload = false;
				$(".command-list-con .loading").html('网络错误，加载失败！');
			}
		});
	}
