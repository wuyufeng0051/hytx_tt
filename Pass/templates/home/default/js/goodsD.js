$(function(){
	...
	/*小图滚动*/
	$(".but .prev").on("click",function(){
		var ul = $("#thumblist"),top=parseInt(ul.css("top"));
		if(top == 0){
			//alert("已经是第一张了");
		}else{
			top = top + 75;
	...
	$(".but .next").on("click",function(){
		var ul = $("#thumblist"),top=parseInt(ul.css("top")),height=parseInt((ul.find("li").length)*75),
		h = parseInt(300-height);
		if(top == h){
			//alert("已经是最后一张了");
		}else{
			top = top-75;
