$(function(){
  ...
  var objId = $('#listCon'), atpage = 1, totalPage = 1; pageSize = 10, isload = false;
    ...
    function getList(){
        ...
        $.ajax({
            url: "/include/ajax.php?service=huangye&action=ilist&page="+atpage+'&pageSize='+pageSize,
            data: data.join("&"),
            type: "GET",
            dataType: "jsonp",
            success: function (data) {
                    ...
                    if(data.state == 100){
                        ...
                        if(list.length > 0){
                            for(var i = 0; i < list.length; i++){
                                ...
                                var rz = obj.rz1==1 || obj.rz2==1 || obj.rz3==1 || obj.rz4==1 ? '<em>已认证</em>' : '';

                                item.push(' <div class="li-right"><span class="title"'+color+'>'+obj.title+'</span>'+rz);
                                if(obj.top == "1"){
                                    item.push(' <i>置顶</i>');
                                }
                                item.push(' <p>'+obj.project+'</p>');
                                item.push(' </div>');
