$(function(){
	...
	function getCommentList(list){
		...
		for(var i = 0; i < list.length; i++){
			...
			html.push('  <a href="'+masterDomain+'/user/'+list[i].userinfo['userid']+'"><img onerror="javascript:this.src=\''+staticPath+'images/noPhoto_40.jpg\';" data-uid="'+list[i].userinfo['userid']+'" src="'+photo+'" alt="'+list[i].userinfo['nickname']+'"></a>');
			html.push('  <div class="c-body">');
			html.push('    <div class="c-header">');
			html.push('      <a href="'+masterDomain+'/user/'+list[i].userinfo['userid']+'" data-id="'+list[i].userinfo['userid']+'">'+list[i].userinfo['nickname']+'</a>');
