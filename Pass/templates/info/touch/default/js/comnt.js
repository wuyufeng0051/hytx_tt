$(function(){
  ...
  $('.wcmt_send, .submit_top').click(function(){
      ...
      alert("请输入您要评论的内容！");
		...
		$.ajax({
			url: "/include/ajax.php?service=info&action=sendCommon&aid="+newsid+"&id="+0,
	...
	$(window).scroll(function(){
      ...
      var orderby = 'time';
      var url = masterDomain + "/include/ajax.php?service=info&action=common&infoid="+newsid+"&page="+page+"&orderby="+orderby+"&pageSize=" + pageSize;
      $.ajax({
          url: url,
          type: "GET",
          dataType: "jsonp",
