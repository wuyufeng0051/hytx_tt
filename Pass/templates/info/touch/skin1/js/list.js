    ...
    $('#choose-info-second').delegate("li", "click", function(){
		...
		if(index == 1 && $t.index() > 0){
			$.ajax({
				...
				success: function (data) {
					if(data && data.state == 100){
						var item = data.info[0].item,box =[],html = [],list = [], f = 0;
						...
						if(item != undefined && item.length > 0){
							...
							for(var i = 0; i < item.length; i++){
                                    if(item[i].formtype != "text"){
                                        ...
                                        list.push('<div class="screen_list fn-clear fn-hide '+item[i].id+'" data-id="'+item[i].id+'"  data-type=""><ul>');
                                        list.push('<li data-id="0" data-name="'+item[i].title+'">不限</li>');
                                        for(var b = 0; b < item[i].options.length; b++){
                                            list.push('<li data-id="'+item[i].options[b]+'" data-name="'+item[i].options[b]+'">'+item[i].options[b]+'</li>');
                                        }
                                        list.push('</ul></div>');
                                    }
                                    if (list != "") {
			...
			success: function (data) {
				...
				if(data && data.state == 100){
					...
					for (var i = 0; i < list.length; i++) {
                        ...
                        if (list[i].litpic) {
                            html.push('<div class="Info_pic"><img src="'+pic+'" alt=""></div>');
                        }
