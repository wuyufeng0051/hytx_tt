		...
		$.ajax({
			...
			success: function(data){
				...
				if(id != ""){
					...
					$.ajax({
						url: "json.php?action=delProductFlags",
						data: "id="+id,
						type: "POST",
						dataType: "json",
						success: function(data){
						if(data && data.length > 0){
							...
							flagsHtml.push('<label style="width: auto; padding-top: 0;"><div class="checker" id="uniform-undefined"><span'+($.inArray(data[i].id, flagsArr) > -1 ? ' class="checked"' : '')+'><input type="checkbox" name="flags" value="'+data[i].id+'" style="opacity: 0;"'+($.inArray(data[i].id, flagsArr) > -1 ? ' checked' : '')+'></span></div> '+data[i].name+'</label>');
							}
							$("#flagsList").html(flagsHtml.join(""));
