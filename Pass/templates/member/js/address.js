$(function(){
	...
	$("#submit").bind("click", function(event){
		...
		if(address.val() == ""){
			address.focus();
			$.dialog.alert(langData['siteConfig'][20][69]);
			return false;
		}
		if(person.val() == ""){
			person.focus();
			$.dialog.alert(langData['siteConfig'][20][70]);
			return false;
		}
		if(mobile.val() == "" && tel.val() == ""){
			mobile.focus();
			$.dialog.alert(langData['siteConfig'][20][67]);
			return false;
		}
		...
		$.ajax({
			...
			success: function (data) {
				if(data && data.state == 100){
					...
					t.attr("disabled", false).html(id == 0 ? langData['siteConfig'][6][27] : langData['siteConfig'][6][41]);
				}
			},
			error: function(){
	...
	$("#list .edit").bind("click", function(){
		...
		if(id == "" || id == 0){
			$.dialog.alert(langData['siteConfig'][20][250]);
			return false;
		}
		...
		$("#submit").html(langData['siteConfig'][6][41]);
