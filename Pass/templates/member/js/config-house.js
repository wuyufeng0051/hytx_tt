$(function(){
  ...
  //选择区域
	$("#selAddr").delegate("a", "click", function(){
		if($(this).text() != langData['siteConfig'][22][96] && $(this).attr("data-id") != $(this).closest("dd").find("input").val()){
		...
		$.ajax({
			...
			success: function (data) {
				if(data && data.state == 100){
					...
					if(!selected){
						$("#addr").val(0);
						$("#addr").closest("dd").find(".tip-inline").removeClass().addClass("tip-inline error").html("<s></s>"+langData['siteConfig'][20][68]);
					}
  ...
  //选择小区
    ...
    var input = $(this).siblings("input"), valArr = input.val().split(",");
		...
		$.ajax({
          ...
          //已选小区
          content.push('<div class="selectedTags"><label>'+langData['siteConfig'][6][21]+'：</label></div>');
					...
					content.push('<div class="pinp_main"><div class="pinp_main_zm"><center style="line-height:335px;">'+langData['siteConfig'][20][128]+'</center></div></div>');
					content.push('</div></div>');
              ...
              //确定选择结果
  						var html = parent.$(".selectedTags").html().replace("<label>"+langData['siteConfig'][6][21]+"：</label>", ""), ids = [];
						...
						$.ajax({
							...
							success: function (data) {
								if(data && data.state == 100){
									...
									parent.$("#tCount").html("<small>"+list.length+langData['siteConfig'][13][50]+"</small>");
								}else{
  ...
  $("#submit").bind("click", function(event){
    ...
    if($.trim(company) == "" && zjcom == ""){
      $.dialog.tips(langData['siteConfig'][20][129], 2, "error.png");
      return false;
    }
    ...
    if(addr == "" || addr == 0){
      $.dialog.tips(langData['siteConfig'][20][131], 2, "error.png");
      return false;
    }

    if(litpic == ""){
      $.dialog.tips(langData['siteConfig'][20][133], 2, "error.png");
      return false;
    }

    t.addClass("disabled").val(langData['siteConfig'][6][35]+"...");
