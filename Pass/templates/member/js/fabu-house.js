...
$(function(){
	...
	if($('#community').size() > 0){
		$('#community').autocomplete({
			...
			onSelect: function(suggestion) {
				...
				$("#communityAddr").html(suggestion.address + "&nbsp;&nbsp;"+langData['siteConfig'][19][315]+"："+suggestion.price+echoCurrency('short')+"/㎡");
	...
	$('#community').bind("input", function(){
		$('#communityid').val(0);
		$("#selAddr button").html(langData['siteConfig'][7][2]+"<span class='caret'></span>");
		$("#communityAddr").html("");
		$("#address").val("");
		$(".community-addr").hide();
		unitPrice = 0;
					...
					$.dialog({
						...
						ok: function(){
							...
							if(id != undefined && title != undefined){
								...
								$("#community").val(title);
								$("#communityid").val(id);
								$("#communityAddr").html(address + "&nbsp;&nbsp;"+langData['siteConfig'][19][315]+"："+price+echoCurrency('short')+"/㎡");
		...
		$.ajax({
			...
			success: function (data) {
				...
				if(data && data.state == 100){
					...
					//选楼盘
					content.push('<div class="choose-item" id="selectLoupan" style="width:230px;"><h2>'+langData['siteConfig'][19][113]+'：<span id="tCount"></span></h2><div class="choose-container fn-clear">');
					content.push('<div class="pinp_main"><div class="pinp_main_zm"><center style="line-height:335px;">'+langData['siteConfig'][20][19]+'</center></div></div>');
					content.push('</div></div>');

					$.dialog({
						id: "chooseData",
						fixed: false,
						title: langData['siteConfig'][19][113],
						content: '<div class="chooseData fn-clear">'+content.join("")+'</div>',
						width: 590,
						okVal: langData['siteConfig'][6][1],
						ok: function(){
							...
							if(loupan != undefined && address != undefined && addrid != undefined && addrName != undefined){
								...
								alert(langData['siteConfig'][20][524]);
								return false;
							}
					...
					function getLoupan(){
						...
						parent.$("#selectLoupan .pinp_main_zm").html('<center style="line-height:335px;">'+langData['siteConfig'][6][176]+'...</center>');
						...
						$.ajax({
							...
							success: function (data) {
								if(data && data.state == 100){
									...
									parent.$("#selectLoupan .pinp_main_zm").html('<center style="line-height:335px;">'+langData['siteConfig'][20][19]+'</center>');
									parent.$("#tCount").html("");
								}
							}
						});
	...
	if($("#getlnglat").size() > 0){
		...
		if(site_map == "baidu"){
			...
			$("#getlnglat").bind("click", function() {
				...
				$(this).unbind("click").html("<s></s>"+langData['siteConfig'][7][3]+"...");
			});
