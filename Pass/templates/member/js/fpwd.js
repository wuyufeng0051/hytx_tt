$(function(){
        ...
        success: function (data) {
          if(data){
            if(data.state == 100){
              ...
              t.html(langData['siteConfig'][6][55]);
            }
          }else{
            alert(langData['siteConfig'][20][526]);
            t.removeClass("disabled");
            t.html(langData['siteConfig'][6][55]);
          }
        }
  ...
  var verifyInput = function(t){
		...
		if($.trim(t.val()) == ""){
			...
			if(id == "email"){
				showTip(t, "error", langData['siteConfig'][21][36]);
			}else if(id == "phone"){
				showTip(t, "error", langData['siteConfig'][20][463]);
			}else if(id == "vericode1" || id == "vericode2"){
				showTip(t, "error", langData['siteConfig'][20][176]);
			}else if(id == "vdimgck"){
				showTip(t, "error", langData['siteConfig'][20][28]);
			}
			return false;

		}else{
			if(id == "email" && !/^[a-z0-9]+([\+_\-\.]?[a-z0-9]+)*@([a-z0-9]+\.)+[a-z]{2,6}$/i.test($.trim(t.val()))){
				showTip(t, "error", langData['siteConfig'][20][511]);
				return false;
			}else if(id == "phone"){
				...
				$.ajax({
					...
					success: function (data) {
						if(data && data.state == 100){
							if(data.info == "error"){
								t.addClass("err");
								showTip(t, "error", langData['siteConfig'][21][99]);
								$("#verifycode").click();
							}
						}
					}
  ...
  //倒计时（开始时间、结束时间、显示容器）
	function countDown(time, obj, type){
		...
		obj.addClass('disabled').text(langData['siteConfig'][20][5].replace('1', time));
		mtimer = setInterval(function(){
			obj.text(langData['siteConfig'][20][5].replace('1', (--time)));
			if(time <= 0) {
				clearInterval(mtimer);
				obj.removeClass('disabled').text(langData['siteConfig'][6][55]);
			}
		}, 1000);
	}
