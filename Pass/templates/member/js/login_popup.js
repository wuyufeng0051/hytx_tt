$(function(){
	...
	function info(typeCount,v){
		var t = typeCount == 2 ? langData['siteConfig'][3][0] : langData['siteConfig'][23][112];
	}
	...
	var vdimgck = {
		username : function(){
			...
			if(typeCount == 3){
				if(v == ''){
					$('#registerBox .error').text(langData['waimai'][3][84]);
					return false;
				}else{
					return true;
				}
			...
			if(typeCount == 2){
				if(v == ''){
					$('#registerBox .error').text(langData['siteConfig'][20][538]);
					return false;
				}else{
					var reg = !!v.match(/^([\.a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(\.[a-zA-Z0-9_-])+/);
					if(!reg) {
						$('#registerBox .error').text(langData['siteConfig'][20][319]);
						return false;
					}else{
						return true;
					}
			...
			if (typeCount == 1) {
				if(v == ''){
					$('#fwdBox .error').text(langData['waimai'][3][84]);
					return false;
				}else{
					return true;
				}
		...
		password : function(){
			...
			if(v == ''){
				$('#registerBox .error').text(langData['siteConfig'][20][502]);
				return false;
			}else{
				var o2 = $('.repassword'+typeCount),v2 = o2.val(),e2 = o2.closest('.inpbox').siblings('.error');
				if(v2 == ''){
					$('#registerBox .error').text(langData['siteConfig'][20][539]);
					return false;
				}else{
					if(v != v2){
						$('#registerBox .error').text(langData['siteConfig'][20][381]);
						return false;
					}else{
						return true;
					}
		...
		yzm : function(){
			...
			if (typeCount == 1) {
				if(v == ''){
					$('#fwdBox .error').text(langData['siteConfig'][20][540]);
					return false;
				}else{
					return true;
				}
				...
				if(v == ''){
					$('#registerBox .error').text(langData['siteConfig'][20][540]);
					return false;
				}else{
					return true;
				}
	...
	$("#fwdBox .login-btn").click(function(){
		...
		if (password.val() == "") {
			$('#fwdBox .error').text(langData['siteConfig'][20][502]);
			return false;
		}
		if (repassword.val() == "") {
			$('#fwdBox .error').text(langData['siteConfig'][20][539]);
			return false;
		}
		if (password.val() != repassword.val()) {
			$('#fwdBox .error').text(langData['siteConfig'][20][381]);
			return false;
		}
	...
	function countDown(time, obj, func){
		...
		mtimer = setInterval(function(){
			...
			if(time <= 0) {
				clearInterval(mtimer);
				obj.text(langData['siteConfig'][4][1]).removeClass('disabled');
			}
		}, 1000);
	}
	...
	$("#loginForm li input").bind("blur", function(){
		...
		if($.trim(t.val()) == ""){
			...
			if(id == "loginuser"){
				showTip($("#loginuser"), "error", langData['siteConfig'][20][166]);
			}else if(id == "loginpass"){
				showTip($("#loginpass"), "error", langData['siteConfig'][20][164]);
			}else if(id == "logincode"){
				showTip($("#logincode"), "tip", langData['siteConfig'][20][176]);
			}
		}else{
			if(id == "loginpass" && !/^.{5,}$/.test($.trim($("#loginpass").val()))){
				showTip($("#loginpass"), "error", langData['siteConfig'][21][103]);
			}else{
				t.removeClass("err");
			}
		}
	...
	$("#submitLogin").bind("click", function(){
		...
		t.addClass("disabled").html(langData['siteConfig'][2][5]+"...");
			...
			success: function (data) {
				if(data){
					if(data.indexOf("100") > -1){
						...
						t.removeClass("disabled").html(langData['siteConfig'][2][0]);
