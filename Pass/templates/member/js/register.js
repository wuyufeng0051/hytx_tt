$(function(){
	...
	var verifyInput = function(t){
		...
		if($.trim(t.val()) == ""){
			...
			if(id == "username"){
				showTip(t, "error", "请输入用户名！");
			}else if(id == "password"){
				showTip(t, "error", "请输入密码！");
			}else if(id == "nickname"){
				showTip(t, "error", "请输入您的真实姓名！");
			}else if(id == "email"){
				showTip(t, "error", "请输入邮箱地址！");
			}else if(id == "phone"){
				showTip(t, "error", "请输入手机号码！");
			}else if(id == "company" && $("input[name=mtype]:checked").val() == "2"){
				showTip(t, "error", "请输入公司名称！");
				return false;
			}else if(id == "answer"){
				showTip(t, "error", "请输入安全问题答案！");
			}else if(id == "vericode"){
			...
			if(id == "username" && !/^[a-zA-Z]{1}[0-9a-zA-Z_]{4,15}$/.test($.trim(t.val()))){
				showTip(t, "error", "用户名格式：英文字母、数字、下划线以内的5-20个字！<br />并且只能以字母开头！");
				return false;
			}else if(id == "password" && !/^.{5,}$/.test($.trim(t.val()))){
				showTip(t, "error", "密码长度最少为5位！");
				return false;
			}else if(id == "nickname" && !/^[a-z\/ ]{2,20}$/i.test($.trim(t.val())) && !/^[\u4e00-\u9fa5 ]{2,20}$/.test($.trim(t.val()))){
				showTip(t, "error", "姓名格式：中文、英文字母、空格、反斜线(/)以内的2-20个字！<br />如：刘德华、刘 德华、Last/Frist Middle");
				return false;
		...
		$.ajax({
			...
			success: function (data) {
				if(data){
					...
					t.removeClass("disabled").html("注册");
				}else{
					alert("注册失败，请重试！");
					t.removeClass("disabled").html("注册");
				}
