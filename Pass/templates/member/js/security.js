$(function(){
	...
	function animation(data){
		var term = setTimeout(function(){
			if(i < interval.length){
				clearTimeout(term);
				if(i == 0){
					title.html(langData['siteConfig'][20][563]+"···");
					if(data.paypwd == "ok"){
						p1.addClass("ok");
					}else{
						p1.addClass("fail");
	...
	$("#chnickname").bind("click", function(){
		modifyFun(langData['siteConfig'][6][180], "chnicknameEdit", "nick", 500, langData['siteConfig'][6][181], function(){
			...
			var name = $("#name");
			if($.trim(name.val()) == ""){
				popTip(langData['siteConfig'][20][568], "error");
				name.focus();
				return "false";
			}
		});
	...
	$("#chCertify").bind("click", function(){
		modifyFun(langData['siteConfig'][19][255], "chCertifyAdd", "certify", 580, langData['siteConfig'][6][60], function(){
			...
			return "realname="+realname.val()+"&idcard="+idcard.val()+"&front="+front.val()+"&back="+back.val();
		}, function(){
			var realname = $("#realname"), idcard = $("#idcard"), front = $("#front"), back = $("#back");
	...
	$("#shCertify").bind("click", function(){
		modifyFun(langData['siteConfig'][23][117], "chCertify", "certify", 580, false);
	...
	$("html").delegate("#getPhoneVerify", "click", function(){
		...
		if(phone.val() == ''){
			popTip(langData['siteConfig'][20][463], "error");
			phone.focus();
		}else{
	...
	function countDown(t) {
		if (wait == 0) {
			...
			t.addClass("disabled");
			t.html(langData['siteConfig'][2][234].replace('1', wait));
			wait--;
			setTimeout(function() {
				countDown(t)
			}, 1000);
	...
	$("#chphone").bind("click", function(){
		modifyFun(langData['siteConfig'][3][7], "chphoneAdd", "chphone", 500, langData['siteConfig'][6][60], function(){
			...
			return "areaCode="+$(".areaCodeBox .code").text()+"&phone="+phone.val()+"&vdimgck="+vdimgck.val();
		}, function(){
			var phone = $("#phone"), vdimgck = $("#vdimgck");
