...
$(function(){
  ...
  $('.fans_list ul li .attention').click(function(){
    ...
    if (x.hasClass('guanzhu_btn')) {
			follow(x, function(){
				x.removeClass('guanzhu_btn').text(langData['siteConfig'][19][846]);
			});
    }else{
			follow(x, function(){
				x.addClass('guanzhu_btn').text(langData['siteConfig'][19][845]);
			});
  ...
  function set_focus(el){
			...
			success: function (data) {
				...
				if(data && data.state == 100){
					...
					for(var i = 0; i < list.length; i++){
						...
						html.push('<div class="com_btn">'+langData['siteConfig'][6][29]+'</div>');
					...
					loadmore.remove();
					if(page == 1){
						$("#leave_message .messages_box").html('<div class="empty">'+langData['siteConfig'][19][387]+'</div>');
					}
				}
			},
			error: function(){
				alert(langData['siteConfig'][19][388]);
				loadmore.removeClass("disabled").html(langData['siteConfig'][6][148]);
			}
		});
	}
	...
	$('body').delegate('.com_btn', "click", function(){
		...
		var rid = 0;
		if(txt == langData['siteConfig'][6][29]){
...
function getList(){
	...
	$('.'+mainBox).html('<div class="loading">'+langData['siteConfig'][20][184]+'...</div>');
	...
	$.ajax({
		...
		success: function(data){
			if (data && data.state != 200) {
				if (data.state == 101) {
					$('.loading').html(langData['siteConfig'][21][64]);
				}else {
					...
					for (var i = 0; i < list.length; i++) {
						...
						// 活动列表
							...
							if (feetype == 1) {
								huodongHtml.push('<li><em>'+langData['siteConfig'][23][122]+'</em>'+echoCurrency('symbol')+ list[i].mprice + '/人</li>');
							}else {
						...
						// 贴吧列表
							...
							if(group.length > 0){
								...
								for(var g = 0; g < group.length; g++){
									if(g < 5){
										var total = g == 4 ? '<div class="Pic_Number">'+group.length+langData['siteConfig'][13][17]+'</div>' : '';
							...
							tiebaHtml.push('<div class="tie_foot"><em>' + transTimes(list[i].pubdate, 3) + '</em><span> '+langData['siteConfig'][16][114] + list[i].reply + '</span></div>');
							tiebaHtml.push('</div>');
							tiebaHtml.push('</div>');
						...
						// 房源
							...
							if (curr == 'saleList') {
								...
								houseHtml.push('<div class="house_price">'+langData['siteConfig'][19][428]+'：¥ <em>'+list[i].unitprice+'</em>/m²</div>');
							...
							// 写字楼
								...
								if (list[i].type == '0') {
									houseHtml.push('<div class="house_price">'+langData['siteConfig'][19][428]+'：'+echoCurrency('symbol')+' <em>'+price+'</em>/m²•'+langData['siteConfig'][13][18]+'</div>');
									tprice = price * area, unit = langData['siteConfig'][13][18];
								}else {
									houseHtml.push('<div class="house_price">'+langData['siteConfig'][19][428]+'：'+echoCurrency('symbol')+' <em>'+tprice+'</em>'+langData['siteConfig'][13][27]+'</div>');
