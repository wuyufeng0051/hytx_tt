  ...
  $(".tab-box a").bind("click", function(){
	...
	var bidDefaultDay = parseInt($(".day .on").text()),               //默认时长为选中的天数
	  ...
	  $('.paybtn').click(function(){
	    var t = $(this), type = $('.house-type .active').attr('data-type');
	    var inpPrice = bidPriceObj.val();
	    var inpPrice1 = bidPriceObj1.val();
	    ...
	    if(isIncrease){
	      ...
	      var url = t.data("url1").replace("$aid", bidID).replace("$type", type).replace("$price", inpPrice1).replace("$paytype", paytype);
	      t.attr("href", url);
	    ...
	    //正常竞价
	      ...
	      var url = t.data("url").replace("$aid", bidID).replace("$type", type).replace("$price", inpPrice).replace("$day", bidDefaultDay).replace("$paytype", paytype);
	      t.attr("href", url);
	    }
    			...
    			if(data.state == 101){
                    ...
                    waitpay     = list[i].waitpay,
    						...
    						//二手房
    							...
    							html.push('<div class="house-box item" data-id="'+id+'">');
