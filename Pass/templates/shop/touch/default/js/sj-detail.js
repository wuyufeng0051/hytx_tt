$(function(){
	...
	var chooseScroll = function(obj){return new iScroll(obj, {vScrollbar: false, mouseWheel: true, click: true});};

	chooseScroll('choose0');
	chooseScroll('choose1');
	...
	$('.chooseType').click(function(){
		if($('#info-box').hasClass('dn')){
			$('#info-box').removeClass('dn');
			$('.disk').show();
		}else{
			$('#info-box').addClass('dn');
			$('.disk').hide();
		}
	})
	// 选中分类
	$("#info-box li").click(function(){
		...
		// 一级
		if(pid == 'choose0'){
			var obj = $('#choose1 .sub_box').eq(index);
			obj.show().siblings().hide();
			chooseScroll(obj.attr('id'));
		}else{
	...
	// 上滑下滑导航隐藏
	var upflag = 1, downflag = 1, fixFooter = $(".select-box ul");
	//scroll滑动,上滑和下滑只执行一次！
	scrollDirect(function (direction) {
		var dom = fixFooter.hasClass('top');
