...
$(function(){
  ...
  function getList(tr){
    ...
    if (action == "article") {
      ...
      url = masterDomain + "/include/ajax.php?service=waimai&action=store&page="+page+"&pageSize=10&title="+keywords;
    }else if (action == "shop") {
      url = masterDomain + "/include/ajax.php?service=shop&action=slist&page="+page+"&pageSize=10&title="+keywords;
          ...
          if (data.state == 101) {
            ...
            for (var i = 0; i < list.length; i++) {
              ...
              // 外卖
                ...
                html.push('<p class="tag"><span class="source">配送费：'+echoCurrency('symbol')+list[i].peisong+'</span><span class="time">营业时间'+list[i].start1+'-'+list[i].start2+'</span></p>');
                html.push('</div>');
                html.push('</a>');
                html.push('</div>');
