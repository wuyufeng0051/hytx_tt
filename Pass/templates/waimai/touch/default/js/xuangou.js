...
$(function(){
	...
	//关闭公告
	$(".notice a").bind("click", function(){
		$(".notice").remove();
		$(".main").removeClass("show_notice");
	});

	// 活动统计
	$('.saleBtn i').text($('.saleBox .gray').length);

	// 展开更多活动
	$('.saleBox .saleBtn').click(function(){
		var t = $('.saleBox');
		if (t.hasClass('open')) {
			t.removeClass('open');
		}else {
	...
	var cartInit = {
		...
		init: function(){
			...
			if(cartHistory && foodIds.length > 0){
				...
				for (var i = 0; i < cartHistory.length; i++) {
					...
					list.push('</div>');
					list.push('<span class="sale-price">'+echoCurrency("symbol")+Number(cartHistory[i].price+cartHistory[i].nprice).toFixed(2)+'</span>');
					...
					if (parsentCount > 0) {
						numAccount.siblings('.reduce').addClass('show');
					}
				...
				if(dabaoPrice > 0){
					...
					list.push('</div>');
					list.push('<span class="sale-price">'+echoCurrency("symbol")+dabaoPrice.toFixed(2)+'</span>');
					list.push('</div>');
				}
		...
		,updateFood: function(type, btn, id, title, pic, unit, price, dabao, ntitle, nprice){
			...
			if(!has && type == "plus"){
				...
				list.push('</div>');
				list.push('<span class="sale-price">'+echoCurrency("symbol")+Number(price+nprice).toFixed(2)+'</span>');
				list.push('<span class="r">');
				list.push('<em class="num-rec">－</em>');
		...
		,plusORreduce: function(type, btn){
			...
			if(stock != "" && (stock == 0 || stock - count < 0)){
				...
				if (count != 0) {
					...
					par.find('.reduce').removeClass('show');
		...
		,clean: function(){
			if(confirm(langData['siteConfig'][20][449])){
				...
				$('.num-account, .reduce').removeClass('show');
				cartInit.statistic();
			}
		}
